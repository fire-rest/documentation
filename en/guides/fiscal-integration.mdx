...
<Info>
  **One numbering contract, every gateway country.** Fire resolves internally which provider
  matches each country. The numbering integration is the same in every country served by the
  gateway (today Ecuador and Colombia): what changes is which fields come filled in the
  response, not how you ask for it. Brazil does not number through this path — it issues
  through PlugNotas after the sale; see [How fiscal works in Fire](/en/fiscal/overview).
</Info>
...
| Event | When |
|---|---|
| [`order.invoiced`](/en/events/order-invoiced) | The authority **approved** the document — the authority's identifiers arrive in `countryData` |
...
| **Country** | `countryData` | The vocabulary of the authority that numbered: `claveAcceso` in Ecuador, `cufe` in Colombia. **Iterate it, do not index it** |
...
<Check>You iterate `countryData` instead of indexing fixed keys: the numbering country sets them, and in Colombia there is no `claveAcceso`.</Check>
