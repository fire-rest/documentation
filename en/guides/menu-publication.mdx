...
Emitted when Fire needs to propagate menus for multiple stores at once. Fire splits large syncs automatically and emits multiple events sequentially. Each event carries one menu in `data.menu`, with the same structure as [`menu.updated`](/en/webhook-reference/menu-updated).
...
Process each event independently. Apply `data.menu` to the downstream system identified by `menu.list.storeId` and `menu.list.channelId`. Each event is retried independently on failure. See [`menus.sync`](/en/webhook-reference/menus-sync) for the full schema.
