...
If your integration injects orders, you need to map Fire channel IDs to your aggregator identifiers.
...
Cache the channel list and use it to resolve the correct channel when injecting orders.
...
If you operate in Brazil and have fiscal emission enabled for a store, add a second flow with trigger `fiscal.authorized.br` (and another with `fiscal.cancelled.br`) — same HTTPS endpoint, same template — to receive the fiscal document context.
...
<CardGroup cols={2}>
  ...
  <Card title="API reference" icon="code" href="/en/api-reference/introduction">
    Inject orders and query sales channels.
  </Card>
</CardGroup>
