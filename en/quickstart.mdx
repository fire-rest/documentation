...
## Step 5: Add fiscal events

If you have fiscal emission enabled for a store, in any country, add a second flow with trigger `order.invoiced` (and another with `order.reversed`) to receive the fiscal document context. Use the same HTTPS endpoint and the same template; the country travels in `fiscal.countryCode`.
