...
**Format:** ISO 8601 with **second** precision and a `Z` suffix — `"2026-08-04T12:30:00Z"`. Always in **UTC**, never in the store's zone: the value is the same fact for every store in the menu, which can be in different countries. To display it in local time, use the `timezone` carried in `list`. No milliseconds — unlike `event.createdAt`, which does carry them.
...
There is no `assignedAt` at the `list` level: the field describes when the entity joined the **menu**, not when a store started receiving it.
...
- **`list.schedules`:** time windows when the channel is active for that store. An empty array (`[]`) means the channel operates 24 hours — no restrictions.
...
<Info>
  Looking for a lighter webhook plus on-demand fetch instead of the full payload?
  See [Hybrid delivery](/en/hybrid-delivery/overview) — a proposed alternative for
