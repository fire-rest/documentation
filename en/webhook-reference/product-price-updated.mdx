...
A price change does **not** alter the product's menu assignment date (`additionalInfo.assignedAt`): that value is membership data, not edit data. That's why this event doesn't carry it — the `assignedAt` you already have on record for that product stays valid. It arrives in [`product.updated`](/en/webhook-reference/product-updated#menu-assignment-date) and in [`menu.updated`](/en/webhook-reference/menu-updated-v2).
