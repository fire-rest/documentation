...
<Tabs>
  <Tab title="v0 · deprecated">
    <Warning>**Deprecated.** This is the **v0** `order.completed` contract, kept only as a **historical reference**. Use the current **v1** contract for new integrations.</Warning>
  </Tab>
  <Tab title="v1 · current">
    Current contract → [order.completed — v1](/en/events/order-completed) (adds discounts, granular taxes, full `itemType` enum, `deliveryConfirmationCode`).
  </Tab>
</Tabs>
