...
<Tabs>
  <Tab title="v0 · deprecated">
    <Warning>**Deprecated.** This is the **v0** `order.completed` contract, kept only as a **historical reference**. Use the current **v1.1** contract for new integrations.</Warning>
  </Tab>
  <Tab title="v1 · current">
    Current contract → [order.completed — v1.1](/en/events/order-completed) (adds discounts, granular taxes, full `itemType` enum, `deliveryConfirmationCode`).
  </Tab>
</Tabs>
...
<ResponseField name="payments" type="object">
  ...
  <Expandable title="payments">
    <ResponseField name="totals" type="object[]">
      Order-level totals — usually one entry. **Casing notice:** these objects are pass-through from the source. Top-level keys mix camelCase (`taxes`, `discounts`) and snake_case (`taxes_value`, `currency_code`, `discount_value`, `subtotal_before_taxes`). Build your handler to read both.

      Each entry has: `total`, `subtotal`, `taxes[]`, `discounts[]`, `taxes_value`, `currency_code`, `discount_value`, `subtotal_before_taxes`.

      `taxes[n]` shape: `{ base, name, rate, amount, metadata }`. `name` examples: `ICMS`, `PIS`, `COFINS`, `IBS_UF`, `IBS_MUN`, `IBS_FED`. `metadata` carries country-specific tax codes (`cst`, `cBenef`, `cClassTrib`, `reducao`, `rateNominal`, `rateEffective`).
    </ResponseField>

    <ResponseField name="shippingCost" type="object[]">Shipping cost line items. Empty for in-store orders.</ResponseField>
    <ResponseField name="extraCharges" type="object[]">Tips, packing fees, and other add-ons.</ResponseField>
    <ResponseField name="discounts"    type="object[]">Per-product or per-reward discount entries.</ResponseField>

    <ResponseField name="paymentMethods" type="object[]">
      Methods used to pay. Most fields are camelCase (`totalBill`, `currencyCode`, `transactionId`, `transactionStatus`, `paymentMethodCode`, …) but some sub-objects are pass-through with snake_case (`card.last_four_digits`, `card.card_country`).

      Each entry: `processor`, `currencyCode`, `paymentMethodCode`, `transactionType`, `transactionId`, `transactionStatus`, `exactPayment`, `totalBill`, `acquirer`, `card`, `transactionDate`, `metadata`, `mid`, `tid`, `voucher`, `referenceNumber`, `authorizationCode`, `customerCashAmount`.
    </ResponseField>
...
<Tabs>
  ...
  <Tab title="Argentina (AR)">
    Argentine stores carry the V4 shape but **no fiscal aggregates today** (no `fiscal.*` flow trigger fires for AR). Compact sample of the country-specific fields:
    ...
    `store.locationInfo.currencyCode` is populated (`ARS`) — same as BR's behavior. You can also read it from `payments.totals[0].currency_code` or `orderLines[0].selectedCurrency` if needed; they all match.
  </Tab>
