...
<ParamField body="lines" type="object[]" required>
  Lines to add (at least 1). Same fields as on order creation: `item_id` / `external_item_id` / `supplier_sku`, `qty`, `qty_base`, `base_unit_id`, `unit_code`, `unit_label`, `unit_cost`, `tax_amount`, `delivery_amount`, `total_amount`, `markup_amount`, `notes`, `metadata`.
</ParamField>
