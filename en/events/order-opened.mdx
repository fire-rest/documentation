...
<Tabs>
  <Tab title="v2.1 · current">
    You're viewing the **current (v2.1)** `order.opened` contract. This event is **new in v2.1** — there is no earlier version of it. It was born with `data.policy` and `data.lastKnown` already in the snapshot.
  </Tab>
</Tabs>
