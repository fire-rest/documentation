...
`fiscal.status` values you may see: `pending`, `processing`, `fiscal_graphic`, `authorized`, `cancelling`, `cancelled`, `rejected`, `denied`, `error`. Fire also tracks two internal states for orders with no document yet — those are reported as `null` here rather than leaking bookkeeping into your contract.
