...
<Tabs>
  <Tab title="v1.1 · current">
    You are reading the **current (v1.1)** contract for `order.opened`. **v1.1 adds** `data.fiscalRepresentation`: the fiscal numbering the point of sale obtained before injecting the order. It is present when numbering **was attempted** — `numberingStatus` tells you how it ended — and **omitted** when no attempt was made. Its presence does **not** mean the document is authorized. Additive only — nothing you already read changed.
  </Tab>
...
**The block being present means numbering was attempted, not that it succeeded.**
`numberingStatus` tells you how the attempt ended, and `failure` why, when it did
not end well.

**It is omitted** — the key is absent, not `null` — only when no attempt was
made: aggregators, countries without fiscal representation, or merchants with
numbering turned off. Branch on the key's presence to know whether there was an
attempt, and on `numberingStatus` to know how it went.
