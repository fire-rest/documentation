...
| [`order.cancelled`](/en/events/order-cancelled) | V4 order snapshot + `cancellation` audit block, with `cancellation.fiscal` (`status`, `authority`) when the order had an authorized document |
| [`order.invoiced`](/en/events/order-invoiced) | V4 order snapshot + `fiscal` block: `authority` (canonical fields, `amounts`, `countryData`, `graphic`), `history`, `compensates` |
| [`order.reversed`](/en/events/order-reversed) | V4 order snapshot + `cancellation` audit block + `cancellation.fiscal` with the cancelled document, `history` and `compensates` |
...
<CardGroup cols={2}>
  ...
  <Card title="order.invoiced" icon="file-invoice" href="/en/events/order-invoiced">
    The fiscal authority authorized the document, in any country, through your fiscal provider.
  </Card>
</CardGroup>
