...
flowchart LR
    ...
    FIRE -- "⑨ store.day-closed (day close)" --> DOWN

    style FIRE fill:#1E293B,color:#F8FAFC,stroke:#1E293B
```

Notice the two **round trips**: Fire notifies the biller (②), the biller resolves with the authority (③) and **answers back to Fire** through the callback (④) — only then does Fire emit `order.invoiced` / `order.reversed` (⑤). Same with the kitchen: Fire dispatches the order to the KDS (⑥), the KDS **reports progress back to Fire** (⑦) and Fire emits `order.status-updated` (⑧). Nothing learns anything "on its own": everything goes through Fire and comes out of Fire.

<Steps>
  <Step title="order — enters through any channel">
    An order is received from POS, kiosk, app or aggregator. Fire normalizes it and **takes control of the entire lifecycle** — from here on, every milestone flows out of Fire.
  </Step>
  <Step title="order.completed — outbound">
    The order is paid and completed. This is the signal for the billing system to start issuing the electronic document. [Reference →](/en/events/order-completed)
  </Step>
  <Step title="order.cancelled — outbound">
    The order was cancelled. Downstream systems void any pending process. [Reference →](/en/events/order-cancelled)
  </Step>
  <Step title="POST /webhooks/fiscal/callback — inbound">
    The fiscal provider returns the authority's result (NFC-e, SRI, DIAN…). Fire **validates that the callback references an event Fire emitted for that order**, responds `202` and attaches the document to the order. [Reference →](/en/api-reference/fiscal-callback)
  </Step>
  <Step title="order.invoiced — outbound">
    Fire confirms the order has the authorized invoice attached. Downstream can display or archive the receipt. [Reference →](/en/events/order-invoiced)
  </Step>
  <Step title="order.reversed — outbound">
    Fire confirms the authorized cancellation receipt. The fiscal reversal is complete. [Reference →](/en/events/order-reversed)
  </Step>
  <Step title="order.status-updated — outbound (in parallel)">
    The kitchen advances the order (`preparing` → `ready` → `dispatched`). The KDS reports to Fire through an inbound webhook and Fire emits the validated advance. [Reference →](/en/events/order-status-updated)
  </Step>
  <Step title="store.day-closed — scheduled">
    Emitted at business-day close. Feeds ERP and Back of House integrations — inventory, cost of sales, accounting and cash reconciliation. [Reference →](/en/events/store-day-closed)
  </Step>
</Steps>
...
<ResponseField name="event" type="object">
  ...
  <Expandable title="event">
    ...
    <ResponseField name="type" type="string">
      Event type. One of: `order.completed`, `order.cancelled`, `order.invoiced`, `order.reversed`, `order.status-updated`, `store.day-closed`.
    </ResponseField>
