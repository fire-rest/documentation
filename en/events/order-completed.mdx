...
description: "An injected order has been fully received and paid. The most common Fire event — carries the full V4 order snapshot and the store's fiscal configuration."
---
...
<ResponseField name="payments" type="object">
  ...
  <Expandable title="payments">
    ...
    <ResponseField name="paymentMethods" type="object[]">
      ...
      Each entry: `processor`, `currencyCode`, `paymentMethodCode`, `transactionType`, `transactionId`, `transactionStatus`, `exactPayment`, `totalBill`, `acquirer`, `card`, `transactionDate`, `metadata`, `mid`, `tid`, `voucher`, `referenceNumber`, `authorizationCode`, `customerCashAmount`, `changeAmount`, `idAuth`, `receiptCustomer`, `receiptMerchant`.
      ...
      - **`customerCashAmount`** (`number | null`) — Cash the customer handed over, in decimals: for a `22.9` bill paid with a `50` note, `50`. `0` on methods that are not cash. Orders injected before 24 September 2026 carry the string `"0"` here — read it as a number. `null` when it was not sent.
      - **`changeAmount`** (`number | null`) — Change the till gave back, in decimals. The real change on cash payments, `0` on other methods, `null` when it was not sent.
    </ResponseField>
...
<ResponseField name="orderLines" type="object[]">
  ...
  <Expandable title="orderLines[n]">
    ...
    <ResponseField name="rewardId" type="string | null">Redemption code of the coupon behind this line’s discount — it matches an entry in `marketing.coupons[].rewardId`. `null` when the discount had no coupon (a catalog markdown), and populated with `discountValue: "0"` when the coupon changed the list price instead of discounting it. Both are valid. Only one coupon per line: a second one applied to the same item appears only in `marketing.coupons[]`.</ResponseField>
    ...
    <ResponseField name="modifierGroups" type="object[]">
      Selected modifier groups. Each: `{ uid, description, selectedModifiers[] }`. `selectedModifiers[n]` is recursively shaped like an order line (price, modifierGroups, itemDescription, …) up to a few levels deep, and carries its own `rewardId` — on a combo the discount usually lives on the modifiers, not on the container.
    </ResponseField>
    <ResponseField name="metadata" type="object">
...
<ResponseField name="marketing" type="object | null">
  Loyalty and coupons. `null` when the order carried neither.

  <Expandable title="marketing.coupons[n]">
    <ResponseField name="rewardId" type="string">Redemption code. Joins with `orderLines[].rewardId` and with the `rewardId` of any nested selected modifier.</ResponseField>
    <ResponseField name="name" type="string">Promotion name (e.g. `VERAO6`). Not unique across promotions.</ResponseField>
    <ResponseField name="metadata" type="object">`{ id, scope, type, itemId, description, percentage, value, baseValue, amount }`. `scope` is `product`, `order` or `shipping`; `type` is `DISCOUNT_PERCENTAGE`, `DISCOUNT_FIXED`, `DISCOUNT_POINTS` or `PRODUCT`. `amount` is the money it took off; `value` is the rule’s nominal figure (a percentage, or points — **not** currency). `baseValue: null` means the list price is unknown, which is not the same as zero.</ResponseField>
  </Expandable>

  A coupon with `scope: "order"` is the only trace of itself in the payload: its amount arrives already spread across the lines, and nothing links the slices back to it. Read it here, not from the lines.
</ResponseField>
...
- **SEFAZ-specific blocks — Brazil only.** The NF-e aggregate `payments.metadata.fiscal` (vBC / vNF / vICMS …) and the per-line `orderLines[].metadata.fiscal` (ncm / cfop / csosn) exist **only for BR**. Non-BR countries still carry their `taxes[]` breakdown, but these SEFAZ-specific blocks are absent. The `order.invoiced` / `order.reversed` events are not Brazil-only: they fire in **every country**, and the country travels in `fiscal.countryCode`.
...
- **`client` may be a populated "FINAL_CONSUMER" placeholder** in any country — it is not `null`. Treat `govIdType === "FINAL_CONSUMER"` as anonymous for analytics.
...
<CardGroup cols={2}>
  ...
  <Card title="order.invoiced" icon="file-invoice" href="/en/events/order-invoiced">
    Fires once the tax authority authorizes the order's fiscal document, in any country.
  </Card>
</CardGroup>
...
| Field | What it is |
|---|---|
| `numberingStatus` | How the **numbering act** ended: `GENERATED` or `NOT_GENERATED` — only two, and the gateway's internal states collapse into them. **Not the authority's verdict** — that lives in `lastKnown.fiscal.status`. |
