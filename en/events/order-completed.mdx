...
<Tabs>
  <Tab title="v1 · current">
    You're viewing the **current (v1)** `order.completed` contract — adds order- and product-level **discounts** (`Discount` object), **granular taxes**, the full `itemType` enum (`PRODUCT` / `COMBO` / `MODIFIER` / `PACKAGING`), and `fulfillment.delivery.deliveryConfirmationCode`.
  </Tab>
...
together with the deferred-payment cycle and are **additive to v1**: existing
