...
<Tabs>
  <Tab title="v2.1 · current">
    You're viewing the **current (v2.1)** `order.completed` contract. **v2.1 adds** `data.policy.deferredPayment` (may this order be acted on before payment?) and `data.lastKnown` (advisory kitchen/fiscal state), plus the new [`order.opened`](/en/events/order-opened) event. Additive only — nothing you already read changed.
  </Tab>
