...
<Tabs>
  <Tab title="v1 · current">
    You're viewing the **current (v1)** contract. `order.cancelled` carries the same v1 order snapshot as [`order.completed`](/en/events/order-completed) — order/product **discounts** (`Discount` object), **granular taxes**, the full `itemType` enum, and `deliveryConfirmationCode` — plus the `cancellation` audit block.
  </Tab>
  <Tab title="v0 · deprecated">
    <Warning>The **v0** contract is **deprecated** — kept only as a historical reference. Open it here: [order.cancelled — v0](/en/events-v0/order-cancelled).</Warning>
  </Tab>
</Tabs>
