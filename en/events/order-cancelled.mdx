...
`policy`, `lastKnown` and `fiscalRepresentation` travel exactly as on `order.completed`. In Brazil (PlugNotas) `fiscalRepresentation` is `null`: there is no numbering step.
...
When the order had no authorized document (it was cancelled before the authority answered, the store does not issue documents, or the document has no provider id), the **`fiscal` key is absent** — not `null`. Here, an Ecuador order numbered at the till and cancelled before the authority answered: `fiscalRepresentation` carries the invoice that was printed, `lastKnown.fiscal` still says `processing`, and `cancellation` has no `fiscal`. No `order.reversed` follows.
...
```json
{
  ...
  "data": {
    ...
    "fiscalRepresentation": {
      ...
      /* rest of the block — see data.fiscalRepresentation below */
    },
