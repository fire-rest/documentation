...
`order.invoiced` fires once SEFAZ (the Brazilian tax authority) authorizes the fiscal document associated with an order. It is emitted by the Brazilian fiscal pipeline that integrates with your fiscal provider as the document provider.

This event is **separate from** [`order.completed`](/en/events/order-completed): the order is paid first (`order.completed`), then Fire requests fiscal emission via your fiscal provider, and `order.invoiced` fires only when SEFAZ returns an authorization protocol.
