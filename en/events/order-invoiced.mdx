...
<Tabs>
  <Tab title="v1.1 · current">
    You're viewing the **current (v1.1)** `order.invoiced` contract. **v1.1 adds** `data.policy.deferredPayment` (may this order be acted on before payment?) and `data.lastKnown` (advisory kitchen/fiscal state), plus the new [`order.opened`](/en/events/order-opened) event. Additive only — nothing you already read changed.
  </Tab>
