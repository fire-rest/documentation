...
| `rejected`, `denied`, `error` | `rejected`, `denied`, `error` | none — visible only in `lastKnown.fiscal.status` of the next event for that order |
...
| `authority.countryData` | from `document` | `chaveAcesso`, `numero`, `serie`, `modelo`, `protocolo` |
...
- **There is no guard by provider.** An order holds a single document. An `authorized` whose `eventId` does not match the order's existing document ends in `notFound` and changes nothing. A `cancelled` resolves the order's most recent document and updates it, whichever provider issued it — PlugNotas included.
