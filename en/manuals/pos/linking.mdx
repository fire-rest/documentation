...
<Info>
  **FIRE POS is not installed.** It is a web application opened in the terminal browser at **[pos.fire.rest](https://pos.fire.rest)** — there is no executable or installer. Optionally, it can be installed as a PWA from the browser itself so it opens full screen.

  These are two different addresses: `app.fire.rest` is the backoffice (where you generate the code) and `pos.fire.rest` is the terminal (where you type it in).
</Info>
...
Sign in to the backoffice at **[app.fire.rest](https://app.fire.rest)**. From the top-right corner, open the selector and choose the country and vendor you will operate under. Every action in this guide happens inside that context.
...
In the device browser, open **[pos.fire.rest](https://pos.fire.rest)**. If the terminal has never been linked, the home screen shows the **Link terminal** title and a 6-digit numeric keypad.
