---
title: "Cancellations"
description: "How a cashier cancels an approved order from FIRE POS V2 with manager authorization."
---

Cancellation lets a cashier reverse an **approved order** directly from the POS terminal. Each cancellation requires a manager authorization code, issues a **credit note** and is recorded in the audit trail. It cannot be undone.
...
The manager generates the code in the backoffice from **Fire POS → Authorization codes**, choosing the **Order cancellation access** purpose.
...
- Use **Refresh** (<Icon icon="arrows-rotate" />) to reload the list if a recent order is not visible yet.
