...
FIRE POS is a web application (PWA) served from the cloud: the terminal does not run the application, it only opens it in a browser. The only component installed locally is **Fire Agent**, required to print receipts and open the cash drawer.
...
|---|---|---|
| **FIRE POS** | Terminal browser (served from the cloud) | Yes |
...
<Steps>
  <Step title="Install FIRE POS as an app (PWA)">
    Install it from the browser so it opens full screen, without an address bar.
  </Step>
