...
<Note>
  Kitchen print from fire-kds tries the URL for the **detected platform first** (Windows `wss://localhost:9101` vs Android `wss://kds-agent.fire.rest:9101`) and **falls back to the other** if that connect fails. You do not pick the URL on the KDS screen. Tablets: [Android installation](/en/manuals/fire-agent/android).
</Note>
