...
Fire Agent is a local agent that lets any browser (FIRE POS, XMART, fire-kds, etc.) print to thermal POS printers over WebSocket. Once installed, it runs in the background and starts automatically with Windows.

Install it once per Windows device that has a printer attached. Kitchen tablets on Android use a different app: [Android installation](/en/manuals/fire-agent/android).

<Card title="Android KDS tablet" icon="mobile" href="/en/manuals/fire-agent/android">
  Fire Agent APK for the same device that runs fire-kds in Chrome.
</Card>
