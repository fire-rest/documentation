...
description: "How to define the configuration fields that enrich each payment, by brand, store, or device, in PayBridge (account)."
---

Beyond deciding **where** a method is turned on ([Availability](/en/manuals/paybridge/availability)), in PayBridge you define **what data** it operates with. That data —credentials, identifiers, and other fields— is stored per level and combined to enrich every payment that goes through PayBridge.
...
- Backoffice access with permissions on the account.
...
<CardGroup cols={2}>
  ...
  <Card title="Create a payment method" icon="credit-card" href="/en/manuals/paybridge/payment-methods">
    General credentials at the method level.
  </Card>
</CardGroup>
