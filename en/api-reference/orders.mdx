...
<ParamField body="payments" type="object" required>
  ...
  <Expandable title="payments">
    ...
    <ParamField body="shippingCost" type="object[]">
      Shipping fee lines. Each entry uses the **same price-band shape** as `totals[]`—currency, subtotals, discount fields, tax percentages, and line total.
    ...
    <ParamField body="discounts" type="object[]">
      Discount lines—each row is one applied discount with the **same price-band fields** as `totals[]` and `shippingCost[]` (amounts, discount rate/value, taxes, total).
