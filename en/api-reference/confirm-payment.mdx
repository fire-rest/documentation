...
<Note>
  **A charge can arrive in several tenders, and in several calls.** If you split the bill between
  cash and card you may send both pieces together, or each one as it happens. Fire keeps the count:
  it adds up what has been approved and settles the order when the sum reaches the total exactly.
  Until then the order stays open, and `settlement.paidSoFar` tells you how much it has collected —
  see [Payment progress](/en/api-reference/list-orders#payment-progress-settlement-and-payments).

  Within **a single call** the batch is atomic: either every tender is recorded, or none is.
</Note>
...
| --- | --- | --- |
| the approved sum **equals** the total and the order is open | `settled` | yes — becomes `COMPLETED` and invoicing starts |
| the sum is **less** than the total (some tenders declined) | `recorded` | no — the money collected is recorded and the order stays open |
...
| the sum **exceeds** the total | — | **`400`** — nothing is recorded |

<Warning>
  **The order settles only on an exact sum.**

  If you charge **less** than the total, what came in is recorded and the order stays open waiting for
  the rest — this happens when some tenders decline. Settling for less would invoice the full total
  while only part was collected, and a wrongly issued invoice cannot be corrected.

  If you charge **more**, the call fails with `400` and **nothing is recorded**, so you can fix it and
  resend without leaving stray rows. The message gives you both amounts.
</Warning>
...
| the approved sum does not exceed the order total | `400` |
| valid API key | `401` |
