...
<Tip>
  Need to correct the order before charging it —customer billing details, locator—? That goes
  through [Update order](/en/api-reference/update-order), not this endpoint. This one only records
  the money; the difference between the two is explained there.
</Tip>
...
```json 400 — the charge does not add up to the total
{
  "success": false,
