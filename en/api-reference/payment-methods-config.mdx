...
<Info>
  There is now a [v2 of this endpoint](/en/api-reference/payment-methods-config-v2) that resolves
  the configuration **per store**, with channel, fulfillment and terminal levels, and returns the
  configuration values. **This v1 stays available and has no sunset date** — migrate when it suits
  you.
</Info>
