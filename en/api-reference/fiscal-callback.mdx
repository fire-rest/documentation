...
For the whole fiscal picture — numbering, callback and events, and how they fit together — see the [fiscal overview](/en/fiscal/overview).
...
<ParamField header="Authorization" type="string">
  Legacy alias: the same Fire API key, as `Bearer pk_live_...`. Prefer `x-api-key`; if both are sent, `x-api-key` is used.
</ParamField>
...
<ParamField body="providerEventId" type="string" required>
  Your provider's own id for this delivery. Stored for audit/forensics — **it is not the idempotency key**. Fire deduplicates on `(orderId, eventId)` — a different `eventType` on the same pair returns `409`, except `fiscal_graphic` and retries after a failure (see [Retries and rejected cancellations](#retries-and-rejected-cancellations)) — so you may safely send a fresh `providerEventId` on every retry without creating duplicates. Use the provider's native event id if available; otherwise a UUID.
</ParamField>
...
Fire accepts **exactly two** values in `documentType`. There is no third type: any other value returns `400`.

| `documentType` | What it is | Callback `eventType` | Numbering operation |
|---|---|---|---|
| `SALE_INVOICE` | The sale document: invoice, NFC-e / NF-e, boleta… | `authorized` | `INVOICE` |
| `CREDIT_NOTE` | The document that annuls a sale | `cancelled` | `CANCEL` |

What each regime calls its document goes in `docSubtype`, and that one is **open**: `nfce`, `nfe` (Brazil), `factura` (Ecuador, Colombia), `boleta` (Chile), `factura_a` (Argentina) and any other name your authority uses. Fire does not validate it against a list and never branches on it.

<Note>
  Do not confuse these with the [Document types catalog](/en/api-reference/fiscal-document-types): that endpoint lists the **buyer identification** documents (`CEDULA`, `CPF`, `NIT`…), not fiscal document types.
</Note>
...
<ParamField body="provider" type="object">
  Who issued it and with what version. It reaches the order and the events as `authority.providerIdentity` — the same name as in the fiscal numbering — with the three keys always present. If you do not send `provider`, or send it empty, `providerIdentity` is `null`.
  ...
  **Formerly named `error`**, and it now carries the same shape as the fiscal numbering response.
  ...
  <Expandable title="failure">
    ...
    <ParamField body="details" type="array">
      Field-by-field detail, when the authority provides it: a list of `{ "field": "…", "issue": "…" }`, both optional.
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="metadata" type="object">
  Free-form bag for provider-specific extras (raw response, internal IDs, etc.). Not validated. It reaches the order and the events as `authority.providerMetadata`, as is and opaque — just like in the fiscal numbering. Absent or `{}` becomes `null`. Do not send `null`: omit it or send `{}` — `null` returns `400`.
...
| Field | `authorized` | `cancelled` | `fiscal_graphic` | `rejected` · `denied` · `error` |
|---|---|---|---|---|
...
<Tabs>
  <Tab title="Brazil (BR)">
    ...
    | `cStat` | int / string \| null | | SEFAZ status code: `100` authorized, `135` cancellation registered, `865` rejected. Reaches the events as `authority.countryData.cStat` |

    The example below is a complete `authorized` payload: every root field it shows is **required** for that event, except `provider` and `metadata`, which are optional and reach the order as `authority.providerIdentity` and `authority.providerMetadata` — see [What is required, per event](#what-is-required-per-event).
...
`fiscal_graphic` reports that your provider is delivering the **representación gráfica** — what the till prints for the document. It is a **state of its own**, not an outcome:
...
For non-authorized states, omit `document` (or send it as `null`) and provide `failure`. `country` still applies (it validates routing); the document itself is not required because there is no authorized artifact — and neither are `documentType`/`docSubtype`, since there is no document to type.

**A negative outcome stores no receipt data.** If you send `documentNumber`, `providerDocId`, `issuedAt`, `authorizedAt`, amounts, `pdfUrl`, `xmlUrl` or `graphic` with `rejected`, `denied` or `error`, Fire ignores them: only `documentType`, `docSubtype`, the country's `document` keys, `failure`, `provider` and `metadata` are kept. A document that already had receipt data keeps it.
...
### Retries and rejected cancellations

**A failed issuance can be retried.** If you reported `rejected`, `denied` or `error` and then retry with the authority, send the new result with the **same `eventId`** — the issuance's. Fire accepts it and the flow continues normally: if the new result is `authorized`, `order.invoiced` is dispatched. If the retry fails again, the new reason replaces the previous one.

```text
rejected (eventId E1)  →  authorized (eventId E1)   ✓ applied, order.invoiced dispatched
rejected (eventId E1)  →  error (eventId E1)        ✓ new reason stored
```

**Cancellation rejected by the authority.** When Fire asks you to void an authorized invoice (`order.cancelled` event), the order is left "cancelling". If the authority **does not accept the cancellation**, send `rejected`, `denied` or `error` with the `eventId` of **that** cancellation and the reason in `failure`. Fire reads it as a failed cancellation: **the invoice remains valid** (the order goes back to `authorized`, with the reason in `error`) and no event is dispatched. If you later retry and the authority accepts it, send `cancelled` with the same `eventId` and `order.reversed` is dispatched.

```text
order.cancelled (eventId C1)  →  rejected (C1)   → the invoice stays authorized
                              →  cancelled (C1)  → voided, order.reversed dispatched
```

<Note>
  A `rejected` on an authorized invoice that is **not** being cancelled still means what it always did: the authority rejected the invoice.
</Note>
...
<ResponseField name="duplicate" type="boolean">
  `true` when this `(orderId, eventId)` was already ingested **with the same `eventType`** — the existing record is returned and nothing is re-enqueued. `false` for a fresh event. A different `eventType` on the same pair never reaches this field: it returns `409`, except `fiscal_graphic`.
</ResponseField>
...
Fire deduplicates callbacks on **`(orderId, eventId)`** — *not* on `providerEventId` (which you may regenerate freely). The same pair resent with the **same** `eventType` is a benign replay; the same pair with a **different** `eventType` returns `409` — an attempt to ride one action on another's event — except `fiscal_graphic`, which may share the `eventId` with its outcome.
...
| Same `(orderId, eventId)` with the same `eventType` resent (any `providerEventId`) | **`202`** · `duplicate: true` — existing record returned, **not** re-enqueued |
...
| A failure (`rejected`, `denied` or `error`) then another outcome with the **same `eventId`** (e.g. `authorized` after a retry) | **`202`** — it is a retry of the same issuance and is applied |
| The same failure resent with the **same `eventId`** (e.g. a second `rejected` with another reason) | **`202`** · `duplicate: false` — processed, the new reason is stored. A failure dispatches no events, so reprocessing it is harmless |
...
When the order already has a document, a callback is matched to it by `eventId`. A callback with a different valid `eventId` ends in `notFound`, not `regression`; resending the document's own `eventId` with an earlier state is a same-pair replay. Neither changes the document.
...
<CardGroup cols={2}>
  <Card title="From the callback to the events" icon="arrow-right-arrow-left" href="/en/fiscal/callback-to-events">
    How each field you send here reaches `order.invoiced` and `order.reversed`.
  </Card>
  <Card title="Fiscal overview" icon="file-invoice" href="/en/fiscal/overview">
    Numbering, callback and events in one picture.
  </Card>
</CardGroup>
...
The root fields land in `fiscal.authority` (or `cancellation.fiscal.authority`) with the same names, except the three amounts, which are grouped in `authority.amounts`. `document` becomes `authority.countryData`, and `provider` and `metadata` become `providerIdentity` and `providerMetadata` (`null` when absent or empty). The field-by-field table is in [From the callback to the events](/en/fiscal/callback-to-events#where-each-field-of-the-callback-lands).
