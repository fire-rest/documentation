...
| `issuedAt` | string (ISO 8601) \| null | | UTC date/time the document was issued. **Formerly named `emittedAt`** — see the notice below |
...
<Warning>
  **Renamed: `emittedAt` → `issuedAt`**

  This field is now `issuedAt`, the same name **the same fact** carries in the fiscal
  numbering response. They were two names for one thing, and the difference forced consumers
  to know which path a document came in through just to read its date.

  **If you still send `emittedAt`, nothing is lost**: it is still accepted on ingestion. But
  it is no longer part of the contract and will stop being accepted — migrate to the new name.
</Warning>
