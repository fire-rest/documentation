...
`revision` is the order's version: it starts at `1` and goes up by one each time the order is [corrected](/en/api-reference/update-order). It is what you send as `expectedRevision` when correcting it. Payment does not move it.
