...
**Available fields**: `id`, `orderCode`, `orderExternal`, `accountId`, `vendorId`, `storeId`, `stationId`,
...
The order DTO mirrors **every column of the `orders` table**. Two of them carry the order's
lifecycle beyond Fire:

| Field | What it holds |
|---|---|
| `kitchen` | KDS mirror — `{ status, occurredAt, stationName, history[] }`: the order's journey through the kitchen (`preparing` → `ready` → `dispatched`). |
| `aggregator` | Aggregator mirror — `{ channelCode, status, occurredAt, history[] }`: the delivery journey reported by Rappi / Uber / Didi. |
| `orderExternal` | Your own order id, canonical and unique per vendor. |

`channel` is the only renamed field: it is the `catalog_id` column, which stores the channel code.
`deletedAt` is always `null` — soft-deleted orders are never returned.
