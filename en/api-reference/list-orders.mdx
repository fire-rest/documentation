...
sub-object always has the **same wide shape** — it holds the union of every country's identifiers —
but **only the fields for `fiscal.countryCode` are populated; the rest are `null`**. Read
`fiscal.countryCode` to know which identifiers to expect.
...
`pdfUrl`, `xmlUrl`, `emittedAt`, `cancelledAt`, `totalAmount`, `taxAmount`, `currencyCode`. The
country-specific identifiers below are added on top; the identifiers of the **other** countries are
present but `null`.
