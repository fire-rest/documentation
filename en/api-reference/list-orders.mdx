...
When an order is charged **after** it opens, the money can arrive in parts — cash now, the card when
the customer comes back. `status` and `paymentStatus` cannot express that: they say `OPEN` and
`PENDING` both for an order that paid nothing and for one that still owes 15.90 of 35.90.

| the order | `status` | `paymentStatus` | `settlement.status` |
| --- | --- | --- | --- |
| opened, nothing charged | `OPEN` | `PENDING` | `pending` |
| **charged 20.00 of 35.90** | `OPEN` | `PENDING` | **`recorded`** |
| a tender was declined | `OPEN` | `PENDING` | `declined` |
| fully charged | `COMPLETED` | `SUCCEEDED` | `settled` |

Read `settlement` to know **how much** has been collected, and `payments` to know **with what**.
...
`status` is one of `pending`, `recorded`, `declined`, `settled`. `paidSoFar` and `total` use the same
×10000 scale as `totals` — above, 20.00 collected of 35.90. `tenderCount` counts approved tenders
only; declined ones are in `declinedCount`.

`amountMismatch: true` means **what has been collected has not reached the total yet**. On a deferred
order that is the normal mid-collection state, not an error.
