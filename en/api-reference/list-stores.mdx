  ...
  <Expandable title="store">
    ...
    <ResponseField name="fiscal" type="object | null">
      ...
      <Expandable title="fiscal">
        ...
        <ResponseField name="secondaryGovIdType" type="string | null">Secondary tax ID type (BR: `INSCRICAO_ESTADUAL`).</ResponseField>
        <ResponseField name="secondaryGovIdNumber" type="string | null">Secondary tax ID number.</ResponseField>
        <ResponseField name="metadata" type="object | null">Country-specific extras. Today only BR is populated — see [The fiscal block](#the-fiscal-block).</ResponseField>
      </Expandable>
    </ResponseField>
...
`metadata` **varies by country** — the country is discriminated by `location.countryCode` (there is
no `country` field inside `fiscal`). Today only Brazil populates `metadata`:
