...
catalogue, and for each store where every method charges and with which values.
...
<ResponseField name="data" type="object">
  <Expandable title="data">
    ...
    <ResponseField name="methods" type="object[]">
      Each method's definition. Not repeated inside every store. It is the union of the catalogues
    ...
    <ResponseField name="warnings" type="object[]">
      ...
      the store has no country set, so its catalogue could not be resolved.
    </ResponseField>
...
- A vendor belongs to one country, but its stores are read by the country each one has set. That
  ...
  identify it**: two countries can define the same code with different `id`s.
- `fulfillmentCode` is returned exactly as stored, without normalising against the global
  catalogue.
