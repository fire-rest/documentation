...
<Info>
  **KDS consumer.** fire-kds calls this endpoint with the [resolved Fire credential](/en/guides/kds-fire-credentials) as Bearer. Brazil uses **v1**. Ecuador and later countries use **v2** (RIDE layout via `fiscalRepresentation`).
</Info>
...
```json 200 (v2) — numbered CO order (the blocks that change)
{
  "fiscal": {
    "countryCode": "CO",
...
<CardGroup cols={2}>
  ...
  <Card title="order.invoiced event" icon="file-invoice" href="/en/events/order-invoiced">
    Fires when the tax authority authorizes, in any country; carries the same fiscal references.
  </Card>
