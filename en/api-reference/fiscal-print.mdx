...
<ParamField query="vendorId" type="string" deprecated>
  **Deprecated — no longer needed.** The vendor is resolved from your API key (vendor-scoped). Still accepted for backward compatibility during a deprecation window: if you send it, it must match your API key's vendor or Fire returns `403`. Drop it from your requests.
</ParamField>
...
```json 403 — vendorId does not match the API key
{
  "error": {
    "code": "forbidden",
