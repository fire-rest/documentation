---
title: "Request fiscal numbering (v2)"
...
<Note>
  **[v1](/en/api-reference/fiscal-documents) is still alive, with no sunset date.** If your
  integration already consumes it, you do not have to do anything: same contract, same
  behavior.

  v2 differs in one thing only: a `CANCEL` is checked against the account's **cancellation
  policy** first — if the policy does not allow it, nothing is numbered. `INVOICE` behaves
  **identically** to v1, down to the HTTP status codes.
</Note>

## Migrating from v1

What your channel has to handle before switching the URL:

1. **A `CANCEL` can come back without `document`.** That is the whole migration. Check
   `requestStatus === "BLOCKED_BY_POLICY"` (or simply `document === null` with
   `printing.mode: "NONE"`, which you already handle for `NOT_APPLICABLE`).
2. **Show the person why.** `policy.cancellation.reason` in one line, `reasonDetail` as the
   paragraph. Branch on `code`, never on the text.
3. **Do not retry.** `retryable: false` — the policy will answer the same thing.
...
<ResponseField name="requestStatus" type="string">
  ...
  Six possible values. The first four describe how the attempt ended; the last two say there
  was no attempt — because this store does not number, or because the cancellation policy did
  not allow the void.
  ...
  <Note>
    `NOT_APPLICABLE` and `BLOCKED_BY_POLICY` are **not stored**: neither creates a fiscal
    ...
    the table reconciliation reads from.
  </Note>
</ResponseField>
...
<ResponseField name="cancellation" type="object | null">
  What the cancellation policy said about THIS void. In v1 it is always `null`.

  **`null` means "it was not asked", not "it could not be evaluated"**: issuing is not
  ...
  <Expandable title="policy.cancellation">
    <ResponseField name="canCancel" type="boolean">
      The verdict. `false` is the only thing that blocks: the request came back with
      `BLOCKED_BY_POLICY` and no document.
    </ResponseField>
    <ResponseField name="outcome" type="string">
      `ALLOW` or `DENY`. Redundant with `canCancel`, explicit on purpose.
    </ResponseField>
    <ResponseField name="code" type="string | null">
      **The stable contract for deciding in code.** A machine-readable reason such as
      `ORDER_NOT_CANCELLABLE` or `CANCELLATION_POLICY_DENIED` (the code every
      account-written rule gets when it does not carry its own). `null` when the void is
      allowed.
    </ResponseField>
    <ResponseField name="reason" type="string | null">
      The name of the rule that decided, **to show to a person** — it fits in one line. It
      is editable and translatable text: never branch on it, that is what `code` is for.
    </ResponseField>
    <ResponseField name="reasonDetail" type="string | null">
      The long note of that rule — the paragraph. It travels separately from `reason`
      because there is not always room for both on a screen.
    </ResponseField>
    <ResponseField name="source" type="string">
      Who decided: `baseline` (Fire's own rules, which always run first), `account` (a rule
      the account configured) or `default` (no rule objected).
    </ResponseField>
    <ResponseField name="threshold" type="object | null">
      Present only when the rule compared against a numeric threshold: `field`, `threshold`
    ...
    <ResponseField name="resolvedAt" type="string">
      When the verdict was resolved, ISO 8601.
    </ResponseField>
    <ResponseField name="resolvedFrom" type="object">
      **The receipt**: the context the policy evaluated — order status, fiscal status,
      country, minutes elapsed. With this, the decision can be reconstructed even if the
      account's configuration changes afterwards.
    </ResponseField>
  </Expandable>
</ResponseField>
</ResponseField>
...
### The policy can say no

In v1 a `CANCEL` always numbers; here Fire first asks the account's **cancellation policy**,
and if it does not allow it, nothing is numbered. Two steps, in this order:

<Steps>
  <Step title="Fire finds the original document">
    By the natural key, the same way v1 already did. If there is no document to void, there is
    no point in going further.
  </Step>
  <Step title="Fire asks the cancellation policy">
    It looks up the order — the one that has the context the policy evaluates: country, fiscal
    status, whether the buyer identified themselves, how many minutes since authorization —
    and runs Fire's baseline rules plus the rules the account configured.
  </Step>
  <Step title="It numbers, or it blocks">
    If the policy allows it, everything proceeds as usual: credit note, `201`,
...
<Note>
  **Which rules run here, and which do not.**

  The ones answerable **from the order**: its status, the fiscal status, the country, whether
  the buyer identified themselves, the minutes since authorization, and whatever the account
  configured.

  **The credit-note rule does not run.** The cancellation policy has a baseline rule that
  denies when it is missing — and it does apply to `POST /cancel` — but here it would ask this
  endpoint for a credit note to exist before issuing it: that is exactly what it is about to
  create.

  The business-day rule does not run either, for the same reason: it is a condition for
  cancelling the order, not for voiding the receipt.

  The question asked here is **"can this sale be voided?"**, not "can this order be
  cancelled?". That one is answered by
  [Cancellation eligibility](/en/api-reference/cancellation-eligibility).
</Note>


<Note>
  **The policy fails open.** If Fire cannot evaluate it — the order is not found, the
  account's configuration cannot be read — it numbers anyway. A legitimate void is never
  blocked because Fire could not answer its own question. Only an explicit `DENY` blocks.
</Note>

**Why a policy rejection is a `200` and not a `4xx`.** In this endpoint the HTTP code
...
- **`retryable: false`.** Insisting does not change the policy's answer. What has to change is
  the account's configuration, or the order — not the request.
- **`policy.numberingFailure.lostSaleReason` is `null`.** A blocked void is NOT a lost sale:
  the sale stands and so does its money. Reporting it would put money in the cash close that
  was never given back.

<ParamField query="locale" type="string" default="es">
  Language for `policy.cancellation.reason` and `reasonDetail`: `es`, `en` or `pt`.

  It only affects **Fire's own rules**, which ship with labels in the three languages. The
  text of a rule the account configured comes back exactly as the account wrote it, in
  whatever language it was written.
</ParamField>

<Warning>
  **`BLOCKED_BY_POLICY` is never persisted.** Nothing was numbered, so there is no document to
  reconcile: it does not appear in the queries by
  [`fiscalRequestId`](/en/api-reference/fiscal-document-get) or
  [`orderCode`](/en/api-reference/fiscal-documents-query). If you need the verdict, it is in
  the response of this call — and only there.
</Warning>
...
<Note>
  Both queries exist under `/api/v2/…` too, re-exported from v1 unchanged: querying does not
...
```json 201 Voided — the policy allowed it
{
  "success": true,
  "data": {
