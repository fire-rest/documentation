...
<ParamField body="createdAt" type="string" required>
  ...
  It is only the **fallback** for the issue date: the primary source is the store's open business
  day. A sale made after midnight belongs to the day that is still open, not to the clock's.

  With no timezone (`2026-08-12 17:26:09`), it is read as **UTC**.
</ParamField>
...
<ResponseField name="environment" type="string">
  ...
  vendor and per country, so the same account can have Ecuador in production and Venezuela in
...
<ResponseField name="document" type="object">
  ...
  <Expandable title="document">
    ...
    <ResponseField name="documentNumber" type="string">
      The visible number, composed per the country's convention (`001-020-000000123`). It is for
      **printing**: to search or reconcile, use the identifiers in `countryData`.
    </ResponseField>
  ...
  **The country's identifiers, in its authority's vocabulary and ready to print.**
  ...
  <Warning>
    **Do not look for fixed fields: iterate the keys that arrive.** Ecuador brings `claveAcceso`,
    Brazil `chaveAcesso`, Colombia `cufe`, Venezuela `numeroControl`. A channel reading
...
Today the gateway numbers in **Ecuador**. Every country that comes in adds its tab here — and
...
<Tabs>
  <Tab title="Ecuador (EC) · available">
    **SRI** receipts.
    ...
    <Note>
      **`documentNumber` is not here because the SRI does not issue it.** Fire composes it from
      these pieces —`establecimiento-puntoEmision-secuencial`— so you can print it without
      knowing the country's convention.
    </Note>
  </Tab>
  ...
  <Tab title="Other countries · when they arrive">
    ...
    What does change is the vocabulary, which is why indexing fixed keys is a bad idea: Brazil
    says `chaveAcesso`, Colombia `cufe`, Venezuela `numeroControl`. Those are examples of how each
    regime names things, not a contract already available.

    <Warning>
      **If your integration operates in more than one country, iterate the keys.** A channel
      reading `countryData.claveAcceso` outright works in Ecuador and breaks with the first one
      added.
    </Warning>
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="FAILED_FINAL — there is none, and retrying will not help" icon="circle-xmark">
    Some value is not accepted. Read `failure.message`, which carries the real reason.

    Print the provisional ticket, inject the order and escalate with the `fiscalRequestId`.
    **The sale was charged without a fiscal receipt** — that also travels in the order's events, so
