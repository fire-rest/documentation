...
<ParamField header="Accept-Language" type="string">
  The language you want the rejection reason in: `es`, `en` or `pt`, with or without a region
  (`pt-BR` works). The first entry in the list wins; `q=` weights are ignored.

  Without this header — or with a language we don't handle — the reason comes back in
  **Spanish**. Fire's own rules carry their text in all three languages; a rule written by an
  account may carry only one, and in that case it comes back as is no matter what you ask for.
</ParamField>
