...
<ParamField body="eventId" type="string" required>
  Correlation UUID (parity with the fiscal callback). **Fire validates that it references an event Fire emitted for this same order** — echo the `event.id` of the envelope Fire used to dispatch the order to your KDS. An `eventId` that does not correspond to a Fire event for that order is rejected with `400` before the `202`. Never invent this value.
</ParamField>
