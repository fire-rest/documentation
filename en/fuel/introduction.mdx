...
- **Single order** — Build one order from the store's real menu (POS-style) and inject it into FIRE, with automatic fiscal enrichment for Brazil.
...
<Warning>
  All five levels must be selected before FUEL can send any request. **Single order** reads the store's real menu (the environment needs a catalog host configured); **Batch** still requires a test menu for the selected vendor.
</Warning>
