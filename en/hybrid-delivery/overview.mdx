...
2. An on-demand **fetch** —
   [`GET /v1/menus/{listId}`](/en/hybrid-delivery/get-menu) — your system calls this
   when it actually needs the menu, using the `listId` from the notify event.
...
| # | Method | Description |
|---|---|---|
| 1 | [`menu.list_ready`](/en/hybrid-delivery/menu-list-ready) | Webhook. Notifies your system that a menu is ready to be fetched for a given store and channel. |
| 2 | [`GET /v1/menus/{listId}`](/en/hybrid-delivery/get-menu) | API endpoint. Returns the full menu — same shape as `data.menu` in `menu.updated` — for the `listId` you received in the notify event. |
...
[`GET /v1/menus/{listId}`](/en/hybrid-delivery/get-menu) authenticates differently —
