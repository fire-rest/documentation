...
description: "Returns the full menu for a listId received in a menu.list_ready notification."
authMethod: "bearer"
tag: "Proposal"
---
...
<ParamField path="listId" type="string" required>
  The menu's identifier, received as `data.listId` in the
  [`menu.list_ready`](/en/hybrid-delivery/menu-list-ready) event.
</ParamField>
...
<ResponseField name="list" type="object">
  ...
  <Expandable title="list fields">
    <ResponseField name="listId" type="string">
      Composite identifier: `{storeNumber}-{channelCode}-{fulfillmentType}`.
    </ResponseField>
...
- A `listId` that doesn't exist, or that has been removed, should return `404`.
