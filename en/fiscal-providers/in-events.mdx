...
<Warning>
  **Amounts travel multiplied by 10,000, in the numbering request and in the order event alike.**

  Everything monetary — `totals` in the numbering request and `data.payments` in the event —
  travels as an **integer in a string, multiplied by 10,000**. It is the scale FIRE stores with,
  so it can do arithmetic with integers and not drag floating-point error when summing taxes.
  ...
  **Divide by 10,000 wherever you read an amount** — in the numbering request or in the event —
  before declaring it to the authority or computing a hash such as the CUFE. Percentages are not
  scaled: a rate travels as is. See [the contract](/en/fiscal-providers/contract).

  Finding out late is expensive: if you do not divide, you declare **500,000,000 COP** for a sale of
