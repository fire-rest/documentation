...
## 25 September 2026 — Cash handed over and change in the order events

The two cash fields of `payments.paymentMethods[]` in the events that carry the V4 order snapshot (`order.opened`, `order.completed`, `order.invoiced`, `order.cancelled`, `order.reversed`). The event versions do not change.

- **`customerCashAmount` is a number and carries the real amount.** Until 24 September 2026 it always travelled as the string `"0"`, even on cash payments. Now it is the cash the customer handed over: for a `22.9` bill paid with a `50` note, `50`; `0` on methods that are not cash.
- **`changeAmount` is listed** — the change the till gave back, as a number: `27.1` in the example above, `0` on methods that are not cash, `null` when the till did not send it. It already travelled in the events; it was missing from the reference.
- **If your consumer checked that `customerCashAmount` is a string, it now fails.** Read it as a number. Events of orders injected before 24 September still carry `"0"`, so a replay of an older order does too.

Updated in **EN / ES / PT**.

## 16 September 2026 — `cStat` is emitted again, inside `countryData`

**This supersedes what a later entry says.** On 15 September we announced that `cStat` was no longer emitted. Consumers asked for it back — without it a cancellation does not say whether the authority accepted it — so it returns. Only `cStat`: `docType`, `dataAutorizacao` and `sefazCancellation` stay out.

- **Brazil: `authority.countryData.cStat`.** The code the authority resolved **this** document with: `100` on an authorization, `135` on a cancellation. It is the authority's own vocabulary, so it travels beside `chaveAcesso` and `protocolo`, not at the root of the block.
- **A cancellation carries its own code, not the invoice's.** `order.reversed` reads `135` — the cancellation event registered at SEFAZ — while the `order.invoiced` that preceded it keeps `100`. Verified end to end in staging.
- **It travels as the provider sends it**, `number` or `string`. We do not normalise a type the authority did not choose.
- **Through the fiscal callback it already worked**: anything inside `document` that is not canonical reaches `countryData` untouched. Send `cStat` there and it arrives — no change was needed on your side.
- **`mensagemSefaz` stays out.** Only the code comes back.
- **Rejections still emit no event.** A `rejected` document stores its code but fires neither `order.invoiced` nor `order.reversed`, so `cStat` does not reach you for that case.

Updated in **EN / ES / PT**.

## 15 September 2026 — KDS bump printing, zoom, and Android Fire Agent

Kitchen print is now a store rule (comanda and/or fiscal on one station). Tablets need their own Fire Agent. Zoom and the operator reprint printer are separate from that rule.

- **[Print on bump](/en/manuals/kds/peripherals-printing)** — **By screen** or **By station**, documents and fiscal timing. Arm the queue on the KDS screen. A dashboard alert fires if the print station has no active screen.
- **[Screen actions](/en/manuals/kds/screen-actions)** — station/ticket **zoom** (timer double-tap, default key **Q**). If the printer fails after a fiscal-ready bump, the ticket shows **Queued: printer failed**.
- **[Operator board](/en/manuals/kds/operator-board)** — **Operator fiscal printer** on the supervisor PC (not the kitchen queue).
- **[Android installation](/en/manuals/fire-agent/android)** — APK on the KDS tablet. `kds-agent.fire.rest` must resolve to `127.0.0.1`. Token and printer **name** must match fire-kds.
- **[Stations](/en/manuals/kds/stations)** / **[Routing](/en/manuals/kds/routing)** — station item-type veto is gone. Categories live only in the routing strategy.

Updated in **EN / ES / PT**.

## 15 September 2026 — Cancellation dates and rejected documents

Two fixes to what the [fiscal callback](/en/api-reference/fiscal-callback) stores, and therefore to what [`order.invoiced`](/en/events/order-invoiced) and [`order.reversed`](/en/events/order-reversed) carry. The event versions do not change.

- **A cancellation keeps the invoice's dates.** `authority.issuedAt` and `authority.authorizedAt` now stay as the invoice had them, even when the `cancelled` callback sends its own. Before, they took the cancellation's time while `compensates.issuedAt` still pointed at the invoice, so the same block told two stories. Use `authority.cancelledAt` for when the document was annulled.
- **A negative outcome stores no receipt data.** On `rejected`, `denied` and `error`, Fire ignores `documentNumber`, `providerDocId`, `issuedAt`, `authorizedAt`, the amounts, `pdfUrl`, `xmlUrl` and `graphic` even if the provider sends them; `documentType`, `docSubtype`, the country's `document` keys, `failure`, `provider` and `metadata` are kept. **A rejected document used to read like an authorized one**, with a number and an `authorizedAt` on it. A document that already had receipt data keeps it.

Updated in **EN / ES / PT**.

## 15 September 2026 — Structured fiscal block and new fiscal event versions

The fiscal block of the three fiscal events is now what Fire stores on the order, with one shape for every country: the authority's verdict in `authority`, plus the document's journey and what it replaces. Two new guides explain the whole cycle.

- **Breaking — the fiscal block of [`order.invoiced`](/en/events/order-invoiced), [`order.cancelled`](/en/events/order-cancelled) and [`order.reversed`](/en/events/order-reversed) is structured.** Canonical fields move into `authority` (`documentType`, `documentNumber`, `issuedAt`, `cancelledAt`…), amounts into `authority.amounts`, and the authority's own identifiers (`chaveAcesso`, `claveAcceso`, `cufe`, `protocolo`…) into `authority.countryData`. `dataAutorizacao` becomes `authority.authorizedAt`; `docType`, `cStat` and `sefazCancellation` are no longer emitted. Every previous path has exactly one new path: see [Path changes](/en/events/order-invoiced#path-changes-from-the-previous-version).
- **Breaking — on a cancellation the block moves from `cancellation.metadata.fiscal` to `cancellation.fiscal`**, and `cancellation.metadata` is no longer emitted. When the order had no authorized document, the `fiscal` key is **absent** — before it was `null`, so a check for `=== null` stops matching.
- **Breaking — the [fiscal callback](/en/api-reference/fiscal-callback) moved every canonical field out of `document` to the root** (`documentType`, `docSubtype`, `documentNumber`, `issuedAt`, `authorizedAt`, `cancelledAt`, `authorizationMode`, `providerDocId`, `totalAmount`, `taxAmount`, `currencyCode`, `pdfUrl`, `xmlUrl`, `graphic`); `document` now holds only the country's identifiers. Renames: `countryCode` → `country`, `docType` → `documentType` (`SALE_INVOICE` or `CREDIT_NOTE`, the only two accepted), `error` → `failure` (with `scope` and `details`), `providerSpecific` → `metadata`, `emittedAt` → `issuedAt`. A sender using the previous names receives `400` naming the field — for `docType`, on `authorized` and `cancelled`. In Ecuador `numeroAutorizacion` is retired: the authorized identifier is `claveAcceso`. `cnpjEmitente` is not part of the contract.
- **`order.cancelled` carries the fiscal block in every country** with an authorized document; before it was Brazil only.
- **`history` and `compensates`** — `order.invoiced` and `order.reversed` carry every stop of the document's journey, oldest first. `compensates` points at the document the current one replaces, so it is filled on `order.reversed` and `null` on `order.invoiced`.
- **`authority.providerIdentity` and `authority.providerMetadata`** — the callback's `provider` and `metadata` now reach the order and the events, with the same names as in the [fiscal numbering](/en/api-reference/fiscal-documents). `null` when not sent; PlugNotas (Brazil) never sends them.
- **[How fiscal works in Fire](/en/fiscal/overview)** — the two acts (numbering and authorization), the two blocks on the order, and the two ways a document reaches Fire.
- **[From the callback to the events](/en/fiscal/callback-to-events)** — which event fires for each callback and where every field lands.

<Warning>Flows and handlers that read the previous paths — `fiscal.chaveAcesso`, `fiscal.protocolo`, `cancellation.metadata.fiscal.*` — read `undefined` on the new versions, without an error. Update them before relying on these events.</Warning>

<Note>In Brazil `authority.authorizedAt` carries the SEFAZ date with a conventional midnight time, so it can read earlier than `issuedAt`. Use `issuedAt` for the moment of the sale.</Note>

| Event | Version |
|---|---|
| [`order.invoiced`](/en/events/order-invoiced) | v1.2 → **v2** |
| [`order.cancelled`](/en/events/order-cancelled) | v2.2 → **v3** |
| [`order.reversed`](/en/events/order-reversed) | v2 → **v3** |

Not backwards compatible for the fiscal block; the rest of the order snapshot is unchanged. The previous versions stay available from each event's version tabs.

Updated in **EN / ES / PT**.

## 4 September 2026 — Coupons on order lines

Orders can now arrive with coupons. [`order.completed`](/en/events/order-completed) carries who financed each discount, on the line and on the order.

- **`orderLines[].rewardId`** — the coupon’s redemption code, or `null`. Selected modifiers carry it too: on a combo the discount usually sits on the modifiers, not on the container. Two shapes are both valid and neither is a bug: `null` with a discount (a catalog markdown, no coupon), and a code with `discountValue: "0"` (the coupon changed the list price instead of discounting it).
- **[`data.marketing.coupons[]`](/en/events/order-completed)** — was reserved for future use; now carries `{ rewardId, name, metadata }` per coupon. `metadata.scope` is `product`, `order` or `shipping`, and `metadata.amount` is the money it took off. A coupon with `scope: "order"` appears **only** here: its amount reaches the lines already spread across them, and nothing links the slices back to it.

<Warning>On kiosk orders `totals.discountValue` is the total across **every** level — the order coupon and the per-line discounts together, because the source spreads the order coupon into the lines before sending. Adding it to the sum of `orderLines[].discountValue` counts the same money twice. Use one or the other, not both.</Warning>
...
## 3 September 2026 — KDS Fire credentials, ticket rows, and station categories

Kitchen print and cancel need the right Fire host and key. Ticket cards can now sit on one to three rows. Products get a routing category and a kitchen thumbnail.

- **[Fire API credentials](/en/manuals/kds/fire-credentials)** — account default vs vendor override. The base URL is the Fire host only; fire-kds appends print and cancel paths.
- **[Station categories](/en/manuals/kds/station-categories)** — codes used as `item_type` in routing, plus product thumbs on the category view.
- **[KDS Fire credentials (technical)](/en/guides/kds-fire-credentials)** — vendor row → account default → env. Print reads `FISCAL_PRINT_*` only. Cancel may fall back to `FIRE_*`.
- **[Screen actions](/en/manuals/kds/screen-actions)** — **Ticket rows** (1–3) reserve space for notes. If Fire rejects a cancel, the operator sees the rejection and the policy detail; the job stays failed.

Updated in **EN / ES / PT**.

## 3 September 2026 — The catalog: products, categories and modifier groups

Three new manuals for the screens that feed everything else. A menu invents nothing — it picks
from the catalog — so this is where the work actually starts.

- **[Products](/en/manuals/backoffice/products)** — the four types, the switches that decide
  whether something sells, price, modifiers, combos and suggestions.
- **[Categories](/en/manuals/backoffice/categories)** — the blocks customers browse with,
  their schedule, and channel exclusivity.
- **[Modifier Groups](/en/manuals/backoffice/modifier-groups)** — the questions asked at order
  time, and the options that answer them.
- **Modifier options are catalog products.** To offer extra cheese there has to be an extra
  cheese product; its price is read from that product, never typed on the group.
- **You only set two numbers on a group: minimum and maximum.** Required/optional and
  single/multiple are derived from them, so a group cannot contradict itself.
- **Channel exclusivity cascades.** Marking a category exclusive takes it *and its products*
  out of the Golden Menu.
- **A combo has no pricing tab**, and its listed price is a calculated reference, not what you
  charge. The real price comes from the destination's price list.

<Note>
  The pricing tab is country-dependent: in Brazil it is simplified to the final price plus the
  fiscal category (NCM). Product variants are planned but have no screen yet.
</Note>

Updated in **EN / ES / PT**.

## 3 September 2026 — Menus: building the menu and getting it to the channel

Two new manuals for the pair of screens that decide **what** is sold and **where**: the Golden
Menu and the custom menus that inherit from it, and the matrix that puts each menu into each
store, channel and fulfillment type.

- **[Menus](/en/manuals/backoffice/menus)** — Golden and custom, inheritance, the create
  dialog, the Structure and Overrides tabs, and the price and day lenses.
- **[Menu assignment](/en/manuals/backoffice/menu-assignments)** — the store x channel x
  fulfillment matrix, bulk assignment, Sync and its warnings, and the sync preview.
- **The fulfillment type is chosen once and cannot be changed.** It is the only field on the
  menu with no way back: getting it wrong means creating the menu again.
- **Saving a menu does not publish it — there is no publish button in the editor.** The menu
  reaches a channel only when somebody syncs the corresponding row on the assignment screen.
- **Sync flattens the menu fresh, with no versioning.** It sends the menu and the price list
  exactly as they are saved at that second; an earlier state cannot be republished.
- **A row can end up Synced and still have dropped products.** A product with no price in the
  assigned list is left out of the payload rather than published at zero, and only the warnings
  chip says so.

<Note>
  Aggregator channels receive their events, but their outcome is not yet reflected in the sync
  status on this screen. The menu History tab is also still empty.
</Note>

Updated in **EN / ES / PT**.

## 2 September 2026 — Out of stock: turning products off during service

New manual for the **Out of stock** screen: taking a product off sale in one store when the
kitchen runs out, without editing the menu and without anyone having to undo it later.

- **[Out of stock](/en/manuals/backoffice/out-of-stock)** — the turn-off dialog, scope by
  channel, the mandatory reason, restoring, and the Audit tab.
- **A turn-off never outlives the store's closing time.** The ceiling is the end of the
  store's business day, not a fixed number of hours: a product turned off today is selling
  again when the store opens tomorrow.
- **Scope is a channel and fulfillment type pair.** A product can be turned off for the kiosk
  while it keeps selling at the till.
- **As a combo option, the product stays selectable.** If it is also sold inside modifier groups,
  the dialog names them and the turn-off records them, but channel availability is per product —
  there is nowhere to put an option in either contract.
- **The reason is mandatory and immutable.** Changing the expiry moves the time only — the
  reason stays as the record of why the product was turned off.
- **Publishing the menu closes the turn-off — but only if it covered the whole scope.** A store-wide
  turn-off survives a publication that reached one channel, because the others really are still out
  of sale. The outcome is **Replaced by a sync**.
- **A turn-off is saved even when the channel is not notified.** Notifying is reported separately
  from saving, and turn-offs whose notification did not land are counted in the header and flagged
  on their row.

<Note>
  The turn-off does propagate to the sales channels: `products/listing` per product for X-Mart
  channels, and [`product.availability_changed`](/en/webhook-reference/product-availability-changed)
  per channel for the aggregators — in all three directions (turning off, restoring, expiring). The
  row stays as Fire's operational record — what stopped being sold, when and why — and never enters
  the menu snapshot: that separation is deliberate, not a pending stage. What does not propagate are
  products sold only as a modifier option: neither contract has anywhere to put them.
</Note>
...
- **`categories[n].assignedAt`** and **`products[n].additionalInfo.assignedAt`** — the date the entity joined the menu, ISO 8601 UTC, no milliseconds. It's **membership** data, not edit data: untouched by price, name, image, or order edits; gets a **new** value when an item is removed from the menu and re-added. Full edge cases in [`menu.updated` → Menu assignment date](/en/webhook-reference/menu-updated#menu-assignment-date). Not carried by [`product.price_updated`](/en/webhook-reference/product-price-updated) or [`product.availability_changed`](/en/webhook-reference/product-availability-changed) — they only ship their delta.
- **[`menu.updated`](/en/webhook-reference/menu-updated) / [`product.updated`](/en/webhook-reference/product-updated) use `taxInfo`** (not `taxesInfo`), **`type: PRODUCTO`** for standard items, and **`modifierGroups[n].type: RADIO`** for single-select groups (`CHECKBOX` for multiple).
