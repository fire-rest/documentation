...
- **Inject order**: `payments.discounts[n].code` added — string identifying the discount type. Known values: `FIXED`, `PERCENTAGE`, `KEETA_DISCOUNT`. Any other string is accepted. Request example updated. EN/ES/PT.
...
- **New event [`order.status-updated`](/en/events/order-status-updated)** — the KDS advances the order in the kitchen (`preparing` → `ready` → `dispatched`), with a `kitchen` block and the full journey in `history`.
