...
- **`data.fiscalRepresentation`** — series, sequential, document number and access key,
  exactly as printed at the till. **Omitted** (key absent, not `null`) when the sale was
  not numbered: aggregators, countries without fiscal representation, or numbering
  turned off. Its presence does **not** mean the document is authorized — the
  authority's verdict stays in `lastKnown.fiscal.status`. It never changes, even after
  the authority authorizes or rejects.
