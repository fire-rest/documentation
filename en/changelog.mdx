...
- **English and Portuguese [`menu.updated`](/en/webhook-reference/menu-updated-v2) / [`product.updated`](/en/webhook-reference/product-updated) were on outdated field names** — `taxesInfo` is now `taxInfo`, `type: PRODUCT` is now `type: PRODUCTO`, and `modifierGroups[n].type: single` is now `RADIO`. If your integration reads either language version of these events, check it against the current names.
- **`storeId` in `menu.updated`'s English docs said `store_number`; it's actually the internal UUID** (`stores.id`), same convention as every other event.
