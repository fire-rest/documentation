...
{/*
  Buttondown's own form, embedded in an iframe.

  It is not styled like the rest of the page on purpose. Buttondown blocks
  cross-origin AJAX, so a form of our own can only submit by sending the reader
  to buttondown.com. The iframe keeps them here AND carries Buttondown's
  Turnstile captcha and firewall, which a hand-rolled form would skip.

  Repeated verbatim in es/ and pt/ — the embed has no language of its own.
*/}
...
<div style={{border: '1px solid rgb(128 128 128 / 0.25)', borderRadius: '0.75rem', padding: '1.25rem', marginBottom: '2.5rem'}}>
  ...
  {/* Tall enough for the captcha when the firewall decides to show it; it is
      invisible the rest of the time. Left scrollable so nothing is ever cut. */}
  <iframe src="https://buttondown.com/fire-docs?as_embed=true" title="Subscribe to updates" style={{width: '100%', height: '175px', border: 'none', borderRadius: '0.5rem', marginTop: '0.75rem', colorScheme: 'light'}} />
</div>
