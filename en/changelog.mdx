...
## 4 August 2026 — Menu assignment date (`assignedAt`) and webhook reference fixes

- **New `assignedAt` field** on `menu.updated v2` (`categories[n]`, `products[n].additionalInfo`) and on `product.updated` (same fields) — the date the category or product **joined the menu**, in ISO 8601 UTC without milliseconds. It's **membership** data, not edit data: it doesn't change on price/name/image/order edits; it does change (new date) when the item is removed from the menu and re-added. Full edge-case table and canonical reference in [`menu.updated` → Menu assignment date](/en/webhook-reference/menu-updated-v2#menu-assignment-date). It does **not** travel in `product.price_updated` or `product.availability_changed` — those events only carry their delta.
- **Fixed a documentation error**: clearing a menu for external deletion (`menu.updated` with empty `categories`/`products`/`modifierGroups`) now correctly states that items get a **new** `assignedAt` when re-added on a later publish — the text previously said the opposite.
- **Clarified the `storeId` / `channelId` convention**: both are the internal UUID (`stores.id` / `channels.id`), not the `store_number` or the aggregator's external `channel_id` — with cross-references across `menu.updated`, `product.updated`, `product.price_updated`, and `product.availability_changed`.
- **New warnings (⚠️)** about two real payload inconsistencies between events: `channelReferenceName` means **fulfillment** in `menu.updated` but **sales channel** in the product events; `product.price_updated`'s `priceInfo` doesn't share a shape with the catalog `priceInfo` of `menu.updated` / `product.updated`.
- **`product.updated`**: removed a row suggesting the event could "delete" a product — it can't — replaced with a note pointing to `menu.updated`.
- Fixed 7 internal links that pointed to the legacy `menu-updated` version instead of `menu-updated-v2`.
- Synced the content of all 4 events, which was still on the legacy schema (`taxesInfo`, `type: PRODUCT`, `modifierGroups[n].type: single`).
