...
- **Breaking — the fiscal block of [`order.invoiced`](/en/events/order-invoiced), [`order.cancelled`](/en/events/order-cancelled) and [`order.reversed`](/en/events/order-reversed) is structured.** Canonical fields move into `authority` (`documentType`, `documentNumber`, `issuedAt`, `authorizedAt`, `cancelledAt`…), amounts into `authority.amounts`, and the authority's own identifiers (`chaveAcesso`, `claveAcceso`, `cufe`, `protocolo`…) into `authority.countryData`. On a cancellation the block moves up from `cancellation.metadata.fiscal` to **`cancellation.fiscal`**. `docType`, `dataAutorizacao`, `cStat` and `sefazCancellation` are no longer emitted. Every previous path has exactly one new path: see [Path changes](/en/events/order-invoiced#path-changes-from-the-previous-version).
- **Breaking — the [fiscal callback](/en/api-reference/fiscal-callback) takes `documentType`** (`SALE_INVOICE` or `CREDIT_NOTE`, the only two accepted) instead of `docType`; a callback that still sends only `docType` receives `400`. `documentNumber`, `authorizedAt` and `graphic` are root fields, and what is required depends on `eventType`. `cnpjEmitente` is not part of the contract.
- **`history` and `compensates`** — `order.invoiced` and `order.reversed` carry every stop of the document's journey, oldest first, and a pointer to the document the current one replaces.
- **`authority.providerIdentity` and `authority.providerMetadata`** — the callback's `provider` and `metadata` now reach the order and the events, with the same names as in the fiscal numbering. `null` when not sent; PlugNotas (Brazil) never sends them.
- **`order.invoiced` and `order.reversed` fire in every country** — the country travels in `fiscal.countryCode`, not in the event name.
