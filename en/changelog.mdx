...
{/*
  Subscribe form, styled to match the docs in both themes.

  It posts into the hidden iframe below instead of navigating, so the reader
  never leaves the page. Buttondown blocks cross-origin AJAX, and its API key
  cannot live in the browser, so this is the only way to keep both the styling
  and the reader in place.

  The trade-off: the response lands in an iframe we cannot read across origins,
  so the confirmation below is optimistic — it says "check your inbox", never
  "you are subscribed". Buttondown's double opt-in and firewall still apply;
  its Turnstile captcha does not, since that lives on their own page.

  Repeated in es/ and pt/ with translated copy. The iframe `name` must stay
  unique per page.
*/}
...
<div style={{border: '1px solid rgb(128 128 128 / 0.25)', borderRadius: '0.75rem', padding: '1.25rem', marginBottom: '2.5rem'}}>
  ...
  <form action="https://buttondown.com/api/emails/embed-subscribe/fire-docs" method="post" target="fire-docs-subscribe-en" onSubmit={(e) => { const f = e.currentTarget; setTimeout(() => { f.style.display = 'none'; f.nextElementSibling.style.display = 'block' }, 0) }} style={{display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '0.875rem'}}>
    <input type="email" name="email" required placeholder="you@company.com" style={{flex: '1 1 16rem', minWidth: 0, padding: '0.5rem 0.75rem', borderRadius: '0.5rem', border: '1px solid rgb(128 128 128 / 0.4)', background: 'transparent', color: 'inherit', fontSize: '0.875rem'}} />
    <button type="submit" style={{padding: '0.5rem 1.25rem', borderRadius: '0.5rem', border: 'none', background: '#E6293D', color: '#fff', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer'}}>Subscribe</button>
  </form>
  <div style={{display: 'none', fontSize: '0.875rem', marginTop: '0.875rem'}}>Almost there — check your inbox and confirm your subscription.</div>
  <iframe name="fire-docs-subscribe-en" title="Subscription result" style={{display: 'none'}} />
</div>
