...
Each time Fire sends an event, it uses that secret to compute a signature of the request body and attaches it as the `X-Fire-Signature` header, together with an `X-Fire-Timestamp` header. Your system does the same calculation on the body it receives and compares the result:
...
<Note>
  The header carrying the signature is `X-Fire-Signature` by default, but it's
  configurable per integration — if your dashboard setup shows a different header
  name, use that one instead.
</Note>
...
<Steps>
  <Step title="Read the signature header">
    Extract the value from the `X-Fire-Signature` header on the incoming request. It
    has the form `sha256=<hex digest>` — a prefix, not a bare hex string.
  </Step>
  <Step title="Recompute the signature">
    Using your webhook secret, compute HMAC-SHA256 of the **raw request body** (before any JSON parsing), then prepend `sha256=` to the resulting hex digest.
  </Step>
  <Step title="Compare">
    Compare the computed value (with the `sha256=` prefix) against the full header value using a constant-time comparison. If they match, process the event. If not, return `400` and discard it.
  </Step>
</Steps>
...
Every delivery also carries an `X-Fire-Timestamp` header (Unix seconds — when Fire
signed the request). Fire does not enforce a delivery window on its side today: if you
want protection against a captured request being replayed later, check that the
timestamp is within a tolerance you choose (e.g. 5 minutes) before accepting the event.
