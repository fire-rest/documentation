...
Calling the Fire API requires three headers on every request:
...
The `x-api-key` identifies your integration. The `x-client-channel` header tells Fire the request comes from an external integration. The Bearer token proves your session is authenticated. You obtain the token by calling the login endpoint.

### Step 1 — Get an access token

Call [`POST /api/authentication/login`](/en/api-reference/login) with your API key in the header and your credentials in the body:
