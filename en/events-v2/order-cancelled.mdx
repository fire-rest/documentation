...
<Warning>
  **Deprecated (v2.2).** Previous contract, kept only as a historical reference. The current version is [order.cancelled — v3](/en/events/order-cancelled).
</Warning>
...
```json
{
  ...
  "data": {
    ...
    "marketing": null,  /* same as order.completed — carries coupons[] when the order had any */
...
<Note>Every block above that is elided or marked *same as `order.completed`* — `orderLines`, `marketing`, `payments`, `client`, `store`, `fulfillment`, `kds`, `device`, `channel`, `operator` — is byte-for-byte the snapshot `order.completed` carries, because all order events are built from the same v4 snapshot. That includes **`orderLines[].rewardId`** and **`marketing.coupons[]`**. See the [`order.completed` field reference](/en/events/order-completed#field-reference).</Note>
