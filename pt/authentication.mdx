...
Cada vez que o Fire envia um evento, usa esse secret para calcular uma assinatura do corpo da requisição e a anexa como header `X-Fire-Signature`, junto com um header `X-Fire-Timestamp`. Seu sistema faz o mesmo cálculo sobre o body recebido e compara o resultado:
...
<Note>
  O header que leva a assinatura é `X-Fire-Signature` por padrão, mas é configurável
  por integração — se o dashboard mostrar um nome diferente para sua integração, use
  esse.
</Note>
...
Ao adicionar um endpoint de webhook em uma [integração de agregadores](/pt/configuration/backoffice-integrations), o painel exibe o signing secret desse endpoint (muitas vezes apenas uma vez). Copie e guarde com segurança — você precisa dele para verificar cada entrega.

### Passos de verificação

<Steps>
  <Step title="Leia o header de assinatura">
    Extraia o valor do header `X-Fire-Signature` da requisição recebida. Tem a forma
    `sha256=<hex digest>` — é um prefixo, não um hex puro.
  </Step>
  <Step title="Recalcule a assinatura">
    Usando seu secret de webhook, calcule HMAC-SHA256 do **body bruto** da requisição (antes de qualquer parse de JSON), e adicione `sha256=` antes do hex resultante.
  </Step>
  <Step title="Compare">
    Compare o valor calculado (com o prefixo `sha256=`) com o valor completo do header usando uma comparação de tempo constante. Se coincidirem, processe o evento. Se não, retorne `400` e descarte.
  </Step>
</Steps>
...
Cada entrega também traz um header `X-Fire-Timestamp` (Unix seconds — momento em que o
Fire assinou a requisição). O Fire não impõe hoje uma janela de tolerância do lado
servidor: se você quiser proteção contra uma requisição capturada e reenviada depois,
valide que o timestamp está dentro de uma tolerância que você escolher (ex.: 5
minutos) antes de aceitar o evento.
