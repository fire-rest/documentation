...
Não existe `assignedAt` no nível `list`: o campo descreve quando a entidade entrou no **menu**, não quando uma loja passou a recebê-lo.
...
- **`list.schedules`:** janelas de tempo em que o canal está ativo para aquela loja. Um array vazio (`[]`) indica que o canal opera 24 horas — sem restrições.
...
<Info>
  Procurando um webhook mais leve mais uma busca sob demanda em vez do payload
  completo? Veja [Entrega híbrida](/pt/hybrid-delivery/overview) — uma alternativa
  proposta para menus, ainda não implementada.
</Info>
