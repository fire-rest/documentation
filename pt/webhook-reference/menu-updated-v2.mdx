...
| `stores` | string[] | Números das lojas às quais este menu se aplica (ex.: `["805", "806"]`) |
| `timezone` | string \| null | Fuso horário IANA da loja (ex.: `America/Guayaquil`). `null` se não configurado |
| `channels` | object[] | Canais de vendas nos quais este menu é publicado |
| `schedules` | object[] \| null | Janelas de tempo em que este menu está ativo. Sempre presente; `null` se não houver horário aplicável |
...
| `productModifiers` | object[] | Referências de grupos de modificadores — `{ modifierId, position }` |
...
- **`list.schedules`:** herança fulfillment → canal → `salesSchedule` → `operationSchedule` → `null`.
