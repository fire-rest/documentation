...
description: "Retorna o menu completo para um listId recebido em uma notificação menu.list_ready."
authMethod: "bearer"
tag: "Proposta"
---
...
<ParamField path="listId" type="string" required>
  Identificador do menu, recebido como `data.listId` no evento
  [`menu.list_ready`](/pt/hybrid-delivery/menu-list-ready).
</ParamField>
...
<ResponseField name="list" type="object">
  ...
  <Expandable title="campos de list">
    <ResponseField name="listId" type="string">
      Identificador composto: `{storeNumber}-{channelCode}-{fulfillmentType}`.
    </ResponseField>
...
- Um `listId` que não existe, ou que foi removido, deveria retornar `404`.
