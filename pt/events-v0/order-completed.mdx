...
<Warning>
  **Descontinuado (v0).** Contrato anterior, mantido apenas como referência histórica. A versão atual é [order.completed — v1.1](/pt/events/order-completed).
</Warning>
...
<ResponseField name="payments" type="object">
  ...
  <Expandable title="payments">
    <ResponseField name="totals" type="object[]">
      Totais a nível de pedido — geralmente uma entrada. **Aviso de casing:** estes objetos são pass-through da origem. Chaves top-level misturam camelCase (`taxes`, `discounts`) e snake_case (`taxes_value`, `currency_code`, `discount_value`, `subtotal_before_taxes`). Construa seu handler para ler ambos.

      Cada entrada tem: `total`, `subtotal`, `taxes[]`, `discounts[]`, `taxes_value`, `currency_code`, `discount_value`, `subtotal_before_taxes`.
    ...
    <ResponseField name="discounts"    type="object[]">Entradas de desconto por produto ou recompensa.</ResponseField>

    <ResponseField name="paymentMethods" type="object[]">
      Métodos usados para pagar. A maioria dos campos é camelCase (`totalBill`, `currencyCode`, `transactionId`, `transactionStatus`, `paymentMethodCode`, …) mas alguns sub-objetos são pass-through com snake_case (`card.last_four_digits`, `card.card_country`).
...
<Tabs>
  ...
  <Tab title="Argentina (AR)">
    ...
    `store.locationInfo.currencyCode` está populado (`ARS`) — igual ao comportamento de BR. Você também pode lê-lo de `payments.totals[0].currency_code` ou `orderLines[0].selectedCurrency` se precisar; todos coincidem.
  </Tab>
