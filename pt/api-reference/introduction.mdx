---
title: "Referência da API"
description: "Referência REST da API do Fire — injete pedidos e consulte a configuração da sua integração."
---

A API do Fire permite que seu sistema injete pedidos no Fire e obtenha a configuração necessária para isso (canais de venda, lojas). Todos os endpoints são REST, aceitam e retornam JSON, e exigem autenticação via Bearer token.

<CardGroup cols={2}>
  <Card title="Injetar pedido" icon="paper-plane" href="/pt/api-reference/orders">
    Envie um pedido de qualquer agregador ou origem para o Fire processar.
  </Card>
  <Card title="Listar canais de venda" icon="list" href="/pt/api-reference/sales-channels">
    Obtenha os IDs de canal necessários ao injetar pedidos.
  </Card>
</CardGroup>

## URL base

```
https://api.fire.rest/v1
```

Todos os caminhos abaixo são relativos a esta URL base.

## Autenticação

Cada requisição deve incluir sua API key como Bearer token:

```http
Authorization: Bearer <your_api_key>
```

Veja [Autenticação](/pt/authentication) para saber como obter e gerenciar API keys.

## Formato de requisição e resposta

- Todos os bodies de requisição devem ser JSON com `Content-Type: application/json`
- Todas as respostas são JSON
- Datas e timestamps usam formato ISO 8601 (`2024-11-15T14:30:00Z`)

## Erros

O Fire usa códigos de status HTTP padrão. Todas as respostas de erro incluem um body JSON com um `code` legível por máquina e uma `message` legível por humanos.

```json
{
  "error": {
    "code": "validation_error",
    "message": "channel_id is required"
  }
}
```

| Status | Significado |
|---|---|
| `200` | Sucesso |
| `400` | Bad request — campos inválidos ou faltando no payload |
| `401` | Não autorizado — API key faltando ou inválida |
| `404` | Recurso não encontrado |
| `409` | Conflito — `external_order_id` duplicado |
| `429` | Limite de taxa excedido — aguarde e tente novamente |
| `5xx` | Erro do lado do Fire — tente novamente com backoff exponencial |

## Limites de taxa

As requisições têm limite de taxa por API key. Se você exceder o limite, o Fire retorna `429 Too Many Requests`. Implemente backoff exponencial começando em 1 segundo.
