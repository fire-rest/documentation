...
Cada peça recusada volta em `declines[]`, com o motivo exatamente como você enviou e o código do
catálogo para o qual resolveu.
...
`resolvedTo: null` significa que aquele código **não casou com nenhum alias** do catálogo. A recusa
foi registrada mesmo assim e o texto original ficou guardado, mas esses registros não aparecem em
nada agrupado por motivo.

<Warning>
  Confira este bloco na primeira integração. Se o seu adquirente usa códigos que não reconhecemos,
  todas as recusas entram sem classificação e o seu painel de motivos fica vazio — sem que nada
  falhe. Quando vir um `resolvedTo: null` recorrente, peça para adicionarmos esse alias.
</Warning>
