...
<ParamField body="providerEventId" type="string" required>
  O id próprio do seu provedor para esta entrega. Armazenado para auditoria/forense — **não é a chave de idempotência**. O Fire deduplica por `(orderId, eventId)` — um `eventType` diferente no mesmo par retorna `409`, exceto `fiscal_graphic` e as retentativas após uma falha (veja [Retentativas e cancelamento rejeitado](#retentativas-e-cancelamento-rejeitado)) — então você pode enviar um `providerEventId` novo a cada retentativa sem criar duplicatas. Use o ID nativo do evento do provedor se disponível; caso contrário, um UUID.
</ParamField>
...
<Tabs>
  <Tab title="Brasil (BR)">
    ...
    | `cStat` | int / string \| null | | Código de status da SEFAZ: `100` autorizado, `135` cancelamento registrado, `865` rejeitado. Chega aos eventos como `authority.countryData.cStat` |
...
### Retentativas e cancelamento rejeitado

**Uma emissão com falha pode ser retentada.** Se você reportou `rejected`, `denied` ou `error` e depois retenta junto ao órgão, envie o novo resultado com o **mesmo `eventId`** — o da emissão. O Fire aceita e o fluxo segue normal: se o novo resultado for `authorized`, `order.invoiced` é despachado. Se a retentativa falhar de novo, o motivo novo substitui o anterior.

```text
rejected (eventId E1)  →  authorized (eventId E1)   ✓ aplicado, sai order.invoiced
rejected (eventId E1)  →  error (eventId E1)        ✓ motivo novo salvo
```

**Cancelamento rejeitado pelo órgão.** Quando o Fire pede para anular uma nota autorizada (evento `order.cancelled`), o pedido fica "cancelando". Se o órgão **não aceitar o cancelamento**, envie `rejected`, `denied` ou `error` com o `eventId` **desse** cancelamento e o motivo em `failure`. O Fire interpreta como cancelamento com falha: **a nota continua válida** (o pedido volta para `authorized`, com o motivo em `error`) e nenhum evento é despachado. Se depois você retentar e o órgão aceitar, envie `cancelled` com o mesmo `eventId` e `order.reversed` é despachado.

```text
order.cancelled (eventId C1)  →  rejected (C1)   → a nota continua autorizada
                              →  cancelled (C1)  → anulada, sai order.reversed
```

<Note>
  Um `rejected` sobre uma nota autorizada que **não** está sendo cancelada continua significando o mesmo de sempre: o órgão rejeitou a nota.
</Note>
...
| Uma falha (`rejected`, `denied` ou `error`) e depois outro desfecho com o **mesmo `eventId`** (ex. `authorized` após retentar) | **`202`** — é a retentativa da mesma emissão, é aplicada |
| A mesma falha reenviada com o **mesmo `eventId`** (ex. um segundo `rejected` com outro motivo) | **`202`** · `duplicate: false` — processada, o motivo novo é salvo. Uma falha não dispara eventos, então reprocessá-la é inofensivo |
