...
<Tabs>
  ...
  <Tab title="Equador (EC)">
    ...
    <Note>
      **`numeroComprobante` não é `numeroAutorizacion`.** O primeiro é o número impresso no
      comprovante —quinze dígitos em três trechos, art. 18 do Reglamento de Comprobantes de
      Venta—; o segundo é a resposta do SRI. São fatos distintos e viajam em campos distintos.

      É **opcional**: se o seu provedor não o emite, não o envie. O Fire não o compõe a partir
      de `establecimiento`, `puntoEmision` e `secuencial` —essa regra é do regime— e o
      documento fica sem número visível em vez de mostrar um montado por nós.
    </Note>
