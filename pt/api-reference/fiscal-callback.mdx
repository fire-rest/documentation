...
| `issuedAt` | string (ISO 8601) \| null | | Data/hora UTC em que o documento foi emitido. **Antes se chamava `emittedAt`** — veja o aviso abaixo |
...
<Warning>
  **Renomeado: `emittedAt` → `issuedAt`**

  Este campo passou a se chamar `issuedAt`, que é como **o mesmo fato** se chama na resposta
  de numeração fiscal. Eram dois nomes para a mesma coisa, e essa diferença obrigava a saber
  por qual caminho o documento entrou só para poder ler sua data.

  **Se você ainda envia `emittedAt`, o dado não se perde**: continua sendo aceito na
  ingestão. Mas não faz parte do contrato e deixará de ser aceito — migre para o nome novo.
</Warning>
