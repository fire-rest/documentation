...
<ParamField body="authorizedAt" type="string">
  Data/hora UTC (ISO 8601) em que o ente **aprovou** o documento — um fato diferente de `issuedAt`. Podem distar segundos (autorização on-line) ou horas (contingência autorizada quando a conexão volta). Opcional; se ausente, os consumidores dos eventos veem `authority.authorizedAt: null`. Em um cancelamento use `cancelledAt`.
</ParamField>
...
<Tabs>
  <Tab title="Brasil (BR)">
    ...
    | `cnpjEmitente` | string \| null | | **Não faz parte do contrato.** Aceito e levado a `countryData` intacto; o Fire nunca o lê |

    O exemplo abaixo é um payload `authorized` completo: todo campo de raiz que ele mostra é **obrigatório** para esse evento — veja [O que é obrigatório, por evento](#o-que-é-obrigatório-por-evento).
...
Para estados não autorizados, omita `document` e forneça `failure`. `country` ainda se aplica (valida o roteamento); o documento não é exigido porque não há artefato autorizado — e `documentType`/`docSubtype` também não, já que não há comprovante a tipificar.
...
| Body do callback | Evento: `fiscal.authority` (ou `cancellation.fiscal.authority`) |
|---|---|
...
| `country` | `fiscal.countryCode` (fora de `authority`); se o pedido já tinha um país fiscal, mantém-se o armazenado |
| `occurredAt` | `fiscal.occurredAt` (fora de `authority`) |
| `provider`, `metadata`, `failure`, `providerEventId` | não passam para o evento |
