...
<Info>
  **Consumidor KDS.** O fire-kds chama este endpoint com a [credencial Fire resolvida](/pt/guides/kds-fire-credentials) como Bearer. O Brasil usa **v1**. O Equador e os países seguintes usam **v2** (layout RIDE via `fiscalRepresentation`).
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Evento order.invoiced" icon="file-invoice" href="/pt/events/order-invoiced">
    Dispara quando o órgão fiscal autoriza, em qualquer país; carrega as mesmas referências fiscais.
  </Card>
