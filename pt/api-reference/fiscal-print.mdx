...
<ParamField query="vendorId" type="string" deprecated>
  **Descontinuado — não é mais necessário.** O vendor é resolvido a partir da sua API key (vendor-scoped). Ainda é aceito por compatibilidade durante uma janela de descontinuação: se enviado, deve coincidir com o vendor da sua API key ou o Fire retorna `403`. Remova-o das suas requisições.
</ParamField>
...
```json 403 — vendorId não coincide com a API key
{
  "error": {
    "code": "forbidden",
