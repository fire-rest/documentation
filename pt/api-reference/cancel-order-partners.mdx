...
<ParamField header="Accept-Language" type="string">
  O idioma em que você quer ler o motivo de uma recusa: `es`, `en` ou `pt`, com ou sem região
  (`pt-BR` vale). Usa-se o primeiro da lista e os pesos `q=` são ignorados.

  Sem este header — ou com um idioma que não tratamos — o motivo volta em **espanhol**. As
  regras do Fire trazem seu texto nos três idiomas; as escritas por uma conta podem trazer só
  um, e nesse caso voltam como estão, não importa o que você peça.
</ParamField>
