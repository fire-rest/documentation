...
<Note>
  **Campos novos na resposta.** Desde que existe a [v2](/pt/api-reference/cash-reconciliations-v2), toda conciliação — inclusive as registradas aqui — devolve também `apiVersion`, `openingBalance`, `withdrawalsTotal`, `sessionExternalId` e `terminalUid`, e `details` pode trazer `withdrawals` e `operator_name`.

  Nas linhas da v1, `apiVersion` é `1`, `withdrawalsTotal` é `0` e `sessionExternalId` é `null`. São acréscimos: nenhum campo existente mudou de valor nem de significado.
</Note>
