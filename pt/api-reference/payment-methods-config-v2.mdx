...
Devolve a configuração de pagamentos que a Fire tem cadastrada para o vendor da sua API key: o
catálogo de formas de pagamento e, para cada loja, onde cada forma cobra e com quais valores.
...
<ResponseField name="data" type="object">
  <Expandable title="data">
    ...
    <ResponseField name="methods" type="object[]">
      A definição de cada forma de pagamento. Não se repete dentro de cada loja. É a união dos
      catálogos dos países presentes nesta página.
      <Expandable title="method">
        <ResponseField name="id" type="string">UUID da forma. **É a chave**: as lojas referenciam por `methodId`.</ResponseField>
        <ResponseField name="countryCode" type="string">ISO 3166-1 alpha-2. As formas são definidas por país.</ResponseField>
        <ResponseField name="code" type="string">Código da forma. Único apenas **dentro de um país**.</ResponseField>
        ...
        <ResponseField name="cardBrands" type="string[]">Bandeiras aceitas. Vazio = não é forma de cartão.</ResponseField>
        ...
        <ResponseField name="configFields" type="object[]">
          Os campos que a forma declara.
          <Expandable title="configField">
            <ResponseField name="key" type="string">Chave com que aparece em `config`.</ResponseField>
            ...
            <ResponseField name="scopeLevel" type="string">`account`, `store` ou `device`. Os `device` **não** aparecem no `config` da loja, apenas em `devices[].config`.</ResponseField>
            <ResponseField name="secret" type="boolean">Se o valor é sensível.</ResponseField>
            <ResponseField name="helpText" type="string | null">Ajuda opcional.</ResponseField>
          </Expandable>
        </ResponseField>
      </Expandable>
    </ResponseField>
      ...
      <Expandable title="store">
        ...
        <ResponseField name="methods" type="object[]">
          <Expandable title="storeMethod">
            <ResponseField name="methodId" type="string">Referência a `methods[].id`.</ResponseField>
            <ResponseField name="code" type="string">Código, por conveniência. Para identificar, use `methodId`.</ResponseField>
    ...
    "methods": [
      {
        ...
        "configFields": [
          ...
          { "key": "terminal_key", "label": "Chave do terminal", "required": true,
            "scopeLevel": "store", "secret": true, "helpText": null },
          { "key": "terminal_ip", "label": "IP da maquininha", "required": false,
...
- Um vendor pertence a um país, mas as lojas dele são lidas pelo país que cada uma tem cadastrado.
  Por isso `methods` pode trazer mais de um país, e por isso **o `code` de uma forma não basta
  para identificá-la**: dois países podem definir o mesmo código com `id` diferente. Indexar do seu
  lado por `code` funciona até o dia em que o cliente abre num segundo país.
