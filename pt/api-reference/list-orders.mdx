...
<ParamField query="fields" type="string">
  Lista de campos a retornar separados por vírgula (projeção). Veja [Projeção de campos](#projeção-de-campos).
...
`businessDayDate`, `createdAt`, `updatedAt`, `completedAt`, `revision`, `deletedAt`, `store`, `customer`,
...
`revision` é a versão do pedido: começa em `1` e sobe um a cada vez que ele é [corrigido](/pt/api-reference/update-order). É o que você envia como `expectedRevision` ao corrigi-lo. A cobrança não a altera.
