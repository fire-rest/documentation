...
Quando um pedido é cobrado **depois** de aberto, o dinheiro pode chegar em partes — o dinheiro agora,
o cartão quando o cliente voltar. `status` e `paymentStatus` não conseguem expressar isso: dizem
`OPEN` e `PENDING` tanto para um pedido que não pagou nada quanto para um que ainda deve 15,90 de
35,90.

| o pedido | `status` | `paymentStatus` | `settlement.status` |
| --- | --- | --- | --- |
| aberto, nada cobrado | `OPEN` | `PENDING` | `pending` |
...
Leia `settlement` para saber **quanto** entrou, e `payments` para saber **com o quê**.
...
`status` é um de `pending`, `recorded`, `declined`, `settled`. `paidSoFar` e `total` usam a mesma
escala ×10000 de `totals` — acima, 20,00 cobrados de 35,90. `tenderCount` conta apenas as peças
aprovadas; as recusadas estão em `declinedCount`.

`amountMismatch: true` significa que **o que entrou ainda não alcançou o total**. Num pedido diferido
esse é o estado normal no meio da cobrança, não um erro.
