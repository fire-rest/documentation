...
{/*
  Formulário de assinatura, com o estilo da doc nos dois temas.

  Envia para o iframe oculto abaixo em vez de navegar, então o leitor nunca sai
  da página. O Buttondown bloqueia AJAX cross-origin e sua API key não pode
  viver no navegador, então esta é a única forma de manter o design e o leitor
  no lugar.

  O custo: a resposta cai num iframe que não podemos ler por ser de outra
  origem, então a mensagem abaixo é otimista — diz "veja seu e-mail", nunca
  "você já está inscrito". O double opt-in e o firewall do Buttondown continuam
  valendo; o captcha Turnstile não, porque vive na página deles.

  Repetido em en/ e es/ com os textos traduzidos. O `name` do iframe precisa
  continuar único por página.
*/}
...
<div style={{border: '1px solid rgb(128 128 128 / 0.25)', borderRadius: '0.75rem', padding: '1.25rem', marginBottom: '2.5rem'}}>
  ...
  <form action="https://buttondown.com/api/emails/embed-subscribe/fire-docs" method="post" target="fire-docs-subscribe-pt" onSubmit={(e) => { const f = e.currentTarget; setTimeout(() => { f.style.display = 'none'; f.nextElementSibling.style.display = 'block' }, 0) }} style={{display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '0.875rem'}}>
    <input type="email" name="email" required placeholder="voce@empresa.com" style={{flex: '1 1 16rem', minWidth: 0, padding: '0.5rem 0.75rem', borderRadius: '0.5rem', border: '1px solid rgb(128 128 128 / 0.4)', background: 'transparent', color: 'inherit', fontSize: '0.875rem'}} />
    <button type="submit" style={{padding: '0.5rem 1.25rem', borderRadius: '0.5rem', border: 'none', background: '#E6293D', color: '#fff', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer'}}>Assinar</button>
  </form>
  <div style={{display: 'none', fontSize: '0.875rem', marginTop: '0.875rem'}}>Quase lá — veja seu e-mail e confirme a assinatura.</div>
  <iframe name="fire-docs-subscribe-pt" title="Resultado da assinatura" style={{display: 'none'}} />
</div>
