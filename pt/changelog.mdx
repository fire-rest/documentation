...
## 4 de agosto de 2026 — Data de atribuição ao menu (`assignedAt`) e correções na referência de webhooks

- **Novo campo `assignedAt`** em `menu.updated v2` (`categories[n]`, `products[n].additionalInfo`) e em `product.updated` (mesmos campos) — data em que a categoria ou o produto **entrou no menu**, em ISO 8601 UTC sem milissegundos. É um dado de **pertencimento**, não de edição: não muda com edições de preço, nome, imagem ou ordem; muda (data nova) quando o item é removido do menu e adicionado novamente. Tabela completa de casos extremos e referência canônica em [`menu.updated` → Data de atribuição ao menu](/pt/webhook-reference/menu-updated-v2#data-de-atribuição-ao-menu). **Não** viaja em `product.price_updated` nem em `product.availability_changed` — esses eventos transmitem apenas seu delta.
- **Corrigido erro de documentação**: o esvaziamento de menu para exclusão externa (`menu.updated` com `categories`/`products`/`modifierGroups` vazios) agora descreve corretamente que os itens chegam com `assignedAt` **novo** ao serem reincorporados em uma publicação posterior — o texto anterior dizia o contrário.
- **Esclarecida a convenção `storeId` / `channelId`**: ambos são o UUID interno (`stores.id` / `channels.id`), não o `store_number` nem o `channel_id` externo do agregador — com referências cruzadas entre `menu.updated`, `product.updated`, `product.price_updated` e `product.availability_changed`.
- **Novos avisos (⚠️)** sobre duas inconsistências reais do payload entre eventos: `channelReferenceName` significa **fulfillment** em `menu.updated`, mas **canal de vendas** nos eventos de produto; o `priceInfo` de `product.price_updated` não compartilha forma com o `priceInfo` de catálogo de `menu.updated` / `product.updated`.
- **`product.updated`**: removida uma linha que sugeria que o evento podia "excluir" um produto — não pode — substituída por uma nota que remete a `menu.updated`.
- Corrigidos 7 links internos que apontavam para a versão legada `menu-updated` em vez de `menu-updated-v2`.
- Sincronizado o conteúdo dos 4 eventos, que ainda seguia o esquema legado (`taxesInfo`, `type: PRODUCT`, `modifierGroups[n].type: single`).
