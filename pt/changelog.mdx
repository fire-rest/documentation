...
- **As versões em inglês de [`menu.updated`](/pt/webhook-reference/menu-updated-v2) / [`product.updated`](/pt/webhook-reference/product-updated) tinham nomes de campo desatualizados** — `taxesInfo` agora é `taxInfo`, `type: PRODUCT` agora é `type: PRODUCTO`, e `modifierGroups[n].type: single` agora é `RADIO`. Se sua integração lê esses eventos em inglês, confira se bate com os nomes atuais.
- **`storeId` na doc em inglês de `menu.updated` dizia `store_number`; na verdade é o UUID interno** (`stores.id`), mesma convenção do restante dos eventos.
