...
- **Novo evento [`order.status-updated`](/pt/events/order-status-updated)** — o KDS avança o pedido na cozinha (`preparing` → `ready` → `dispatched`), com bloco `kitchen` e o percurso completo em `history`.
...
- **store.day-closed** — `sales` agora inclui `product_discounts`, `gross_before_discounts`, `shipping`, `extra_charges`, `total_charged` e `taxes_by_type`.
