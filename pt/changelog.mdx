...
- **`data.fiscalRepresentation`** — série, sequencial, número do documento e chave de
  acesso, exatamente como foram impressos no caixa. **É omitido** (a chave não aparece,
  não vem `null`) quando a venda não foi numerada: agregadores, países sem representação
  fiscal, ou numeração desativada. Sua presença **não** significa que o comprovante
  esteja autorizado — o veredito do órgão continua em `lastKnown.fiscal.status`. Nunca
  muda, nem depois que o órgão autoriza ou rejeita.
