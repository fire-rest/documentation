...
## 15 de setembro de 2026 — Bloco fiscal estruturado e novas versões dos eventos fiscais
...
- **Breaking — o bloco fiscal de [`order.invoiced`](/pt/events/order-invoiced), [`order.cancelled`](/pt/events/order-cancelled) e [`order.reversed`](/pt/events/order-reversed) é estruturado.** Os campos canônicos passam para `authority` (`documentType`, `documentNumber`, `issuedAt`, `authorizedAt`, `cancelledAt`…), os valores para `authority.amounts` e os identificadores do órgão (`chaveAcesso`, `claveAcceso`, `cufe`, `protocolo`…) para `authority.countryData`. Num cancelamento o bloco sobe de `cancellation.metadata.fiscal` para **`cancellation.fiscal`**. `docType`, `dataAutorizacao`, `cStat` e `sefazCancellation` não são mais emitidos. Cada caminho anterior tem exatamente um caminho novo: veja [Mudanças de caminho](/pt/events/order-invoiced#mudanças-de-caminho-em-relação-à-versão-anterior).
- **Breaking — o [callback fiscal](/pt/api-reference/fiscal-callback) recebe `documentType`** (`SALE_INVOICE` ou `CREDIT_NOTE`, os únicos dois aceitos) em vez de `docType`; um callback que ainda envie só `docType` recebe `400`. `documentNumber`, `authorizedAt` e `graphic` vão na raiz, e o que é obrigatório depende do `eventType`. `cnpjEmitente` não faz parte do contrato.
- **`history` e `compensates`** — `order.invoiced` e `order.reversed` levam cada parada do percurso do documento, da mais antiga à mais nova, e um ponteiro para o documento que o vigente substitui.
- **`authority.providerIdentity` e `authority.providerMetadata`** — o `provider` e o `metadata` do callback agora chegam ao pedido e aos eventos, com os mesmos nomes da numeração fiscal. `null` quando não enviados; o PlugNotas (Brasil) nunca os envia.
- **`order.invoiced` e `order.reversed` saem em todos os países** — o país viaja em `fiscal.countryCode`, não no nome do evento.
