...
## 25 de setembro de 2026 — Dinheiro entregue e troco nos eventos do pedido

Os dois campos de dinheiro de `payments.paymentMethods[]` nos eventos que levam o snapshot V4 do pedido (`order.opened`, `order.completed`, `order.invoiced`, `order.cancelled`, `order.reversed`). As versões dos eventos não mudam.

- **`customerCashAmount` é um número e traz o valor real.** Até 24 de setembro de 2026 trafegava sempre como o texto `"0"`, inclusive em pagamentos em dinheiro. Agora é o dinheiro entregue pelo cliente: para uma conta de `22.9` paga com uma nota de `50`, `50`; `0` nos meios que não são dinheiro.
- **`changeAmount` entra na referência** — o troco entregue pelo caixa, como número: `27.1` no exemplo anterior, `0` nos meios que não são dinheiro, `null` quando o caixa não o enviou. Já trafegava nos eventos; faltava na referência.
- **Se o seu consumidor validava que `customerCashAmount` fosse texto, agora falha.** Leia como número. Os eventos de pedidos injetados antes de 24 de setembro continuam trazendo `"0"`, então um reenvio de um pedido antigo também.

Atualizado em **EN / ES / PT**.

## 16 de setembro de 2026 — `cStat` volta a ser emitido, dentro de `countryData`

**Isto corrige o que diz uma entrada posterior.** Em 15 de setembro anunciamos que `cStat` não era mais emitido. Os consumidores pediram de volta —sem ele um cancelamento não diz se o órgão o aceitou— então ele retorna. Apenas `cStat`: `docType`, `dataAutorizacao` e `sefazCancellation` continuam de fora.

- **Brasil: `authority.countryData.cStat`.** O código com que o órgão resolveu **este** documento: `100` numa autorização, `135` num cancelamento. É vocabulário do órgão, então viaja ao lado de `chaveAcesso` e `protocolo`, não na raiz do bloco.
- **O cancelamento leva o próprio código, não o da nota.** O `order.reversed` traz `135` —o evento de cancelamento registrado na SEFAZ— enquanto o `order.invoiced` que o precedeu mantém `100`. Verificado de ponta a ponta em staging.
- **Viaja como o provedor envia**, `number` ou `string`. Não normalizamos um tipo que o órgão não escolheu.
- **Pelo callback fiscal já funcionava**: tudo o que vai dentro de `document` e não é canônico chega a `countryData` sem alteração. Envie `cStat` ali e ele chega — não foi preciso mudar nada do seu lado.
- **`mensagemSefaz` continua de fora.** Volta apenas o código.
- **As rejeições continuam sem emitir evento.** Um documento `rejected` guarda seu código mas não dispara `order.invoiced` nem `order.reversed`, então nesse caso `cStat` não chega até você.

Atualizado em **EN / ES / PT**.

## 15 de setembro de 2026 — Impressão no envio do KDS, zoom e Agente Fire Android

A impressão da cozinha agora é uma regra da loja (comanda e/ou fiscal numa estação). Tablets precisam do próprio Agente Fire. O zoom e a reimpressão do operador são outra impressora.

- **[Impressão no envio](/pt/manuals/kds/peripherals-printing)** — **By screen** ou **By station**, documentos e timing fiscal. Arme a fila na tela KDS. O dashboard avisa se a estação de impressão não tem tela ativa.
- **[Ações na tela](/pt/manuals/kds/screen-actions)** — **zoom** de estação/ticket (toque duplo no temporizador, tecla **Q**). Se a impressora falhar com o fiscal pronto, o ticket mostra **Queued: printer failed**.
- **[Board do operador](/pt/manuals/kds/operator-board)** — **Operator fiscal printer** no PC do supervisor (não a fila da cozinha).
- **[Instalação no Android](/pt/manuals/fire-agent/android)** — APK no tablet do KDS. `kds-agent.fire.rest` deve resolver para `127.0.0.1`. Token e **nome** da impressora devem coincidir com o fire-kds.
- **[Estações](/pt/manuals/kds/stations)** / **[Roteamento](/pt/manuals/kds/routing)** — o veto de item type na estação sumiu. As categorias vivem só na estratégia de roteamento.

Atualizado em **EN / ES / PT**.

## 15 de setembro de 2026 — Datas do cancelamento e rejeições

Duas correções no que o [callback fiscal](/pt/api-reference/fiscal-callback) guarda e, por consequência, no que [`order.invoiced`](/pt/events/order-invoiced) e [`order.reversed`](/pt/events/order-reversed) carregam. As versões dos eventos não mudam.

- **O cancelamento mantém as datas da fatura.** `authority.issuedAt` e `authority.authorizedAt` continuam como a fatura os tinha, mesmo que o callback `cancelled` envie os seus. Antes assumiam a hora do cancelamento enquanto `compensates.issuedAt` seguia apontando para a fatura, então o mesmo bloco contava duas histórias. Para saber quando o documento foi anulado, use `authority.cancelledAt`.
- **Um resultado negativo não guarda dados de comprovante.** Em `rejected`, `denied` e `error`, o Fire ignora `documentNumber`, `providerDocId`, `issuedAt`, `authorizedAt`, os valores, `pdfUrl`, `xmlUrl` e `graphic` mesmo que o provedor os envie; são guardados `documentType`, `docSubtype`, as chaves do país em `document`, `failure`, `provider` e `metadata`. **Um documento rejeitado era lido como um autorizado**, com número e com `authorizedAt`. Um documento que já tinha dados de comprovante os mantém.

Atualizado em **EN / ES / PT**.

## 15 de setembro de 2026 — Bloco fiscal estruturado e eventos fiscais novos

O bloco fiscal dos três eventos fiscais agora é o que o Fire guarda no pedido, com uma única forma para todos os países: o veredito do órgão em `authority`, mais o percurso do documento e qual ele substitui. Dois guias novos explicam o ciclo completo.

- **Breaking — o bloco fiscal de [`order.invoiced`](/pt/events/order-invoiced), [`order.cancelled`](/pt/events/order-cancelled) e [`order.reversed`](/pt/events/order-reversed) é estruturado.** Os campos canônicos passam para `authority` (`documentType`, `documentNumber`, `issuedAt`, `cancelledAt`…), os valores para `authority.amounts` e os identificadores do órgão (`chaveAcesso`, `claveAcceso`, `cufe`, `protocolo`…) para `authority.countryData`. `dataAutorizacao` passa a ser `authority.authorizedAt`; `docType`, `cStat` e `sefazCancellation` não são mais emitidos. Cada caminho anterior tem exatamente um caminho novo: veja [Mudanças de caminho](/pt/events/order-invoiced#mudanças-de-caminho-em-relação-à-versão-anterior).
- **Breaking — num cancelamento o bloco passa de `cancellation.metadata.fiscal` para `cancellation.fiscal`**, e `cancellation.metadata` não é mais emitido. Se o pedido não tinha documento autorizado, a chave `fiscal` **não existe** — antes vinha `null`, então uma comparação com `=== null` deixa de bater.
- **Breaking — o [callback fiscal](/pt/api-reference/fiscal-callback) tirou todos os campos canônicos de `document` para a raiz** (`documentType`, `docSubtype`, `documentNumber`, `issuedAt`, `authorizedAt`, `cancelledAt`, `authorizationMode`, `providerDocId`, `totalAmount`, `taxAmount`, `currencyCode`, `pdfUrl`, `xmlUrl`, `graphic`); `document` agora leva só os identificadores do país. Renomeações: `countryCode` → `country`, `docType` → `documentType` (`SALE_INVOICE` ou `CREDIT_NOTE`, os únicos dois aceitos), `error` → `failure` (com `scope` e `details`), `providerSpecific` → `metadata`, `emittedAt` → `issuedAt`. Um emissor com os nomes anteriores recebe `400` indicando o campo — no caso de `docType`, em `authorized` e `cancelled`. No Equador `numeroAutorizacion` é retirado: o identificador autorizado é `claveAcceso`. `cnpjEmitente` não faz parte do contrato.
- **`order.cancelled` leva o bloco fiscal em todos os países** com documento autorizado; antes era só o Brasil.
- **`history` e `compensates`** — `order.invoiced` e `order.reversed` levam cada parada do percurso do documento, da mais antiga à mais nova. `compensates` aponta para o documento que o vigente substitui: vem preenchido em `order.reversed` e `null` em `order.invoiced`.
- **`authority.providerIdentity` e `authority.providerMetadata`** — o `provider` e o `metadata` do callback agora chegam ao pedido e aos eventos, com os mesmos nomes da [numeração fiscal](/pt/api-reference/fiscal-documents). `null` quando não enviados; o PlugNotas (Brasil) nunca os envia.
- **[Como o fiscal funciona no Fire](/pt/fiscal/overview)** — os dois atos (numerar e autorizar), os dois blocos do pedido e os dois caminhos pelos quais um documento chega ao Fire.
- **[Do callback aos eventos](/pt/fiscal/callback-to-events)** — qual evento sai para cada callback e onde cai cada campo.

<Warning>Fluxos e handlers que leem os caminhos anteriores — `fiscal.chaveAcesso`, `fiscal.protocolo`, `cancellation.metadata.fiscal.*` — leem `undefined` nas versões novas, sem erro. Atualize-os antes de depender destes eventos.</Warning>

<Note>No Brasil `authority.authorizedAt` traz a data da SEFAZ com um horário convencional de meia-noite, então pode ficar antes de `issuedAt`. Para o momento da venda use `issuedAt`.</Note>

| Evento | Versão |
|---|---|
| [`order.invoiced`](/pt/events/order-invoiced) | v1.2 → **v2** |
| [`order.cancelled`](/pt/events/order-cancelled) | v2.2 → **v3** |
| [`order.reversed`](/pt/events/order-reversed) | v2 → **v3** |

Não é retrocompatível no bloco fiscal; o resto do snapshot do pedido não muda. As versões anteriores continuam disponíveis nas abas de versão de cada evento.

Atualizado em **EN / ES / PT**.

## 4 de setembro de 2026 — Cupons nas linhas do pedido

Os pedidos já podem chegar com cupons. [`order.completed`](/pt/events/order-completed) leva quem financiou cada desconto, na linha e no pedido.

- **`orderLines[].rewardId`** — o código de resgate do cupom, ou `null`. Os modificadores selecionados também o levam: num combo o desconto costuma estar nos modificadores, não no contêiner. Há duas formas válidas e nenhuma é erro: `null` com desconto (uma remarcação de catálogo, sem cupom) e um código com `discountValue: "0"` (o cupom mudou o preço de tabela em vez de descontar).
- **[`data.marketing.coupons[]`](/pt/events/order-completed)** — estava reservado para uso futuro; agora leva `{ rewardId, name, metadata }` por cupom. `metadata.scope` é `product`, `order` ou `shipping`, e `metadata.amount` é o dinheiro que ele descontou. Um cupom com `scope: "order"` aparece **apenas** aqui: o seu valor chega às linhas já distribuído entre elas e nada liga as parcelas a ele.

<Warning>Em pedidos de quiosque `totals.discountValue` é o total de **todos** os níveis — o cupom de pedido e os descontos de linha juntos, porque a origem distribui o cupom de pedido dentro das linhas antes de enviar. Somá-lo com a soma de `orderLines[].discountValue` conta o mesmo dinheiro duas vezes. Use um ou outro, não os dois.</Warning>
...
## 3 de setembro de 2026 — Credenciais Fire do KDS, linhas de comandas e categorias de estação

A impressão e o cancelamento da cozinha precisam do host e da key certos da Fire. Os cartões podem ocupar de uma a três linhas. Os produtos ganham uma categoria de roteamento e uma miniatura de cozinha.

- **[Credenciais Fire API](/pt/manuals/kds/fire-credentials)** — padrão da conta versus override por vendor. A base URL é só o host da Fire; o fire-kds acrescenta as rotas de print e cancel.
- **[Categorias de estação](/pt/manuals/kds/station-categories)** — códigos usados como `item_type` no roteamento, mais miniaturas de produto na visualização da categoria.
- **[Credenciais Fire do KDS (técnico)](/pt/guides/kds-fire-credentials)** — linha de vendor → padrão da conta → env. Print lê só `FISCAL_PRINT_*`. Cancel pode cair em `FIRE_*`.
- **[Ações na tela](/pt/manuals/kds/screen-actions)** — **Linhas de comandas** (1–3) reservam espaço para notas. Se a Fire rejeitar um cancel, o operador vê a rejeição e o detalhe de policy; o trabalho fica falho.

Atualizado em **EN / ES / PT**.

## 3 de setembro de 2026 — O catálogo: produtos, categorias e grupos de modificadores

Três manuais novos para as telas que alimentam todo o resto. Um menu não inventa nada — escolhe
do catálogo —, então o trabalho começa de verdade aqui.

- **[Produtos](/pt/manuals/backoffice/products)** — os quatro tipos, os interruptores que decidem
  se algo vende, o preço, os modificadores, os combos e as sugestões.
- **[Categorias](/pt/manuals/backoffice/categories)** — os blocos com que o cliente percorre o
  cardápio, o horário deles e a exclusividade de canal.
- **[Grupos de modificadores](/pt/manuals/backoffice/modifier-groups)** — as perguntas feitas na
  hora do pedido, e as opções que as respondem.
- **As opções de um modificador são produtos do catálogo.** Para oferecer queijo extra tem que
  existir um produto queijo extra; o preço dele é lido daquele produto, nunca digitado no grupo.
- **Num grupo só se definem dois números: mínimo e máximo.** Obrigatório/opcional e
  única/múltipla são deduzidos deles, então um grupo não pode se contradizer.
- **A exclusividade de canal é herdada.** Marcar uma categoria como exclusiva a tira do Golden
  Menu *junto com os produtos dela*.
- **Um combo não tem aba de preço**, e o preço que a lista mostra é uma referência calculada, não
  o que você cobra. O preço real sai da lista de preços do destino.

<Note>
  A aba de preços depende do país: no Brasil ela é simplificada ao preço final mais a categoria
  fiscal (NCM). As variantes de produto estão previstas mas ainda não têm tela.
</Note>

Atualizado em **EN / ES / PT**.

## 3 de setembro de 2026 — Menus: montar o cardápio e fazê-lo chegar ao canal

Dois manuais novos para o par de telas que decidem **o que** se vende e **onde**: o menu Golden e
os menus custom que herdam dele, e a matriz que coloca cada menu em cada loja, canal e tipo de
fulfillment.

- **[Menus](/pt/manuals/backoffice/menus)** — Golden e custom, a herança, o diálogo de criação,
  as abas Structure e Overrides, e as lentes de preço e de dia.
- **[Atribuição de menus](/pt/manuals/backoffice/menu-assignments)** — a matriz loja x canal x
  fulfillment, a atribuição em massa, o Sync e seus avisos, e a pré-visualização.
- **O tipo de fulfillment é escolhido uma vez e não pode ser alterado.** É o único campo do menu
  sem volta: errar significa criar o menu de novo.
- **Salvar um menu não o publica — no editor não há botão de publicar.** O menu chega a um canal
  só quando alguém sincroniza a linha correspondente na tela de atribuições.
- **O Sync achata o menu fresco, sem versionamento.** Envia o menu e a lista de preços exatamente
  como estão salvos naquele segundo; não é possível republicar um estado anterior.
- **Uma linha pode ficar em Synced e ainda assim ter perdido produtos.** Um produto sem preço na
  lista atribuída fica fora do envio em vez de ser publicado em zero, e a única coisa que avisa é
  o chip de avisos.

<Note>
  Os canais agregadores recebem seus eventos, mas o resultado ainda não se reflete no status de
  sincronização desta tela. A aba History do menu também segue vazia.
</Note>

Atualizado em **EN / ES / PT**.

## 2 de setembro de 2026 — Em falta: desligar produtos durante a operação

Manual novo da tela **Em falta**: tirar um produto da venda em uma loja quando ele acaba na
cozinha, sem editar o cardápio e sem que ninguém precise desfazer depois.

- **[Em falta](/pt/manuals/backoffice/out-of-stock)** — a caixa de diálogo de desligamento, o
  alcance por canal, o motivo obrigatório, a restauração e a aba de auditoria.
- **Um desligamento nunca sobrevive ao fechamento da loja.** O teto é o fim da jornada
  operacional, não uma quantidade fixa de horas: um produto desligado hoje amanhece vendendo.
- **O alcance é um par de canal e tipo de entrega.** Um produto pode ficar desligado no quiosque
  e continuar sendo vendido no caixa.
- **Como opção de um combo, o produto continua selecionável.** Se ele também é vendido dentro de
  grupos de modificadores, a caixa de diálogo os nomeia e o desligamento os registra, mas o que os
  canais recebem é disponibilidade por produto: uma opção não tem onde entrar ali.
- **O motivo é obrigatório e não é editável.** Alterar a validade move apenas o horário — o
  motivo fica como o registro do porquê o produto foi desligado.
- **Publicar o cardápio encerra o desligamento, mas só se cobriu todo o seu alcance.** Um
  desligamento de toda a loja sobrevive a uma publicação que chegou a um canal, porque nos outros o
  produto continua fora de venda de verdade. O desfecho é **Substituído por um sync**.
- **O desligamento fica salvo mesmo que o canal não fique sabendo.** Avisar é informado separado de
  salvar, e os desligamentos cujo aviso não chegou são contados no cabeçalho e marcados na sua linha.

<Note>
  O desligamento se propaga sim aos canais de venda: `products/listing` por produto para os canais
  X-Mart e [`product.availability_changed`](/pt/webhook-reference/product-availability-changed) por
  canal para os agregadores, nas três direções (desligar, restaurar, vencer). A linha fica como
  registro operacional do Fire — o que deixou de ser vendido, quando e por quê — e nunca entra no
  snapshot do cardápio: essa separação é deliberada, não uma etapa pendente. O que não se propaga são
  os produtos vendidos apenas como opção de modificador: nenhum dos dois contratos tem onde colocá-los.
</Note>
