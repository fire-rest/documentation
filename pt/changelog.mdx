...
- **`data.fiscalRepresentation`** — série, sequencial, número do documento, chave de
  acesso e o QR (`graphic`), exatamente como foram impressos no caixa. **Sua presença
  significa que se tentou numerar**, não que foi numerado: `numberingStatus` diz como a
  tentativa terminou e `failure` por quê, quando não terminou bem — uma venda cobrada que
  ficou **sem comprovante fiscal** chega com os identificadores em `null` e o motivo em
  `failure`. **É omitido** (a chave não aparece, não vem `null`) apenas quando não houve
  tentativa: agregadores, países sem representação fiscal, ou numeração desativada. Sua
  presença **não** significa que o comprovante esteja autorizado — o veredito do órgão
  continua em `lastKnown.fiscal.status`. Nunca muda, nem depois que o órgão autoriza ou
  rejeita.
