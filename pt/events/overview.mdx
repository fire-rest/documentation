...
<Note>
  O Fire **não** assina requisições de saída com HMAC hoje. Autentique usando um Bearer token, API key ou OAuth2 client credentials configurado no nó HTTP do seu flow. Assinatura HMAC (`X-Fire-Signature`) está no roadmap; esta página será atualizada quando sair.
</Note>
