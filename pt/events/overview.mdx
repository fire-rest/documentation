...
<Note>
  O Fire emite **cinco** tipos de evento de pedido hoje: [`order.completed`](/pt/events/order-completed), [`order.cancelled`](/pt/events/order-cancelled), [`order.invoiced`](/pt/events/order-invoiced), [`order.reversed`](/pt/events/order-reversed) e [`order.status-updated`](/pt/events/order-status-updated), mais o evento agendado [`store.day-closed`](/pt/events/store-day-closed) no fechamento do dia.
</Note>
...
flowchart LR
    ...
    FIRE -- "⑨ store.day-closed (fechamento do dia)" --> DOWN

    style FIRE fill:#1E293B,color:#F8FAFC,stroke:#1E293B
```

Repare nos dois **circuitos de ida e volta**: o Fire avisa o faturador (②), o faturador resolve com a autoridade (③) e **responde ao Fire** pelo callback (④) — só então o Fire emite `order.invoiced` / `order.reversed` (⑤). O mesmo com a cozinha: o Fire despacha o pedido ao KDS (⑥), o KDS **reporta o avanço ao Fire** (⑦) e o Fire emite `order.status-updated` (⑧). Nada fica sabendo de nada "por conta própria": tudo passa pelo Fire e sai do Fire.

<Steps>
  <Step title="pedido — entra por qualquer canal">
    Um pedido é recebido de POS, kiosk, app ou agregador. O Fire o normaliza e **assume o controle de todo o ciclo de vida** — daqui em diante, todos os marcos saem do Fire.
  </Step>
  <Step title="order.completed — outbound">
    O pedido está pago e completado. É o sinal para o sistema de faturamento iniciar a emissão do documento eletrônico. [Referência →](/pt/events/order-completed)
  </Step>
  <Step title="order.cancelled — outbound">
    O pedido foi cancelado. Os sistemas downstream anulam qualquer processo pendente. [Referência →](/pt/events/order-cancelled)
  </Step>
  <Step title="POST /webhooks/fiscal/callback — inbound">
    O provedor fiscal devolve o resultado da autoridade (NFC-e, SRI, DIAN…). O Fire **valida que o callback referencia um evento que o Fire emitiu para esse pedido**, responde `202` e anexa o documento ao pedido. [Referência →](/pt/api-reference/fiscal-callback)
  </Step>
  <Step title="order.invoiced — outbound">
    O Fire confirma que o pedido tem a nota autorizada anexada. Downstream pode exibir ou arquivar o comprovante. [Referência →](/pt/events/order-invoiced)
  </Step>
  <Step title="order.reversed — outbound">
    O Fire confirma o comprovante de cancelamento autorizado. O estorno fiscal está completo. [Referência →](/pt/events/order-reversed)
  </Step>
  <Step title="order.status-updated — outbound (em paralelo)">
    A cozinha avança o pedido (`preparing` → `ready` → `dispatched`). O KDS reporta ao Fire por webhook de entrada e o Fire emite o avanço validado. [Referência →](/pt/events/order-status-updated)
  </Step>
  <Step title="store.day-closed — agendado">
    Emitido no fechamento do dia de negócio. Alimenta integrações com ERP e Back of House — estoque, custo de vendas, contabilidade e conciliação de caixa. [Referência →](/pt/events/store-day-closed)
  </Step>
</Steps>
...
<ResponseField name="event" type="object">
  ...
  <Expandable title="event">
    ...
    <ResponseField name="type" type="string">
      Tipo de evento. Um de: `order.completed`, `order.cancelled`, `order.invoiced`, `order.reversed`, `order.status-updated`, `store.day-closed`.
    </ResponseField>
...
| [`order.status-updated`](/pt/events/order-status-updated) | Snapshot V4 + bloco `kitchen` (status, previousStatus, history do percurso) |
