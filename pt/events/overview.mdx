...
| [`order.cancelled`](/pt/events/order-cancelled) | Snapshot V4 + bloco `cancellation` de auditoria, com `cancellation.fiscal` (`status`, `authority`) quando o pedido tinha um documento autorizado |
| [`order.invoiced`](/pt/events/order-invoiced) | Snapshot V4 + bloco `fiscal`: `authority` (campos canônicos, `amounts`, `countryData`, `graphic`), `history`, `compensates` |
| [`order.reversed`](/pt/events/order-reversed) | Snapshot V4 + bloco `cancellation` de auditoria + `cancellation.fiscal` com o documento cancelado, `history` e `compensates` |
...
<CardGroup cols={2}>
  ...
  <Card title="order.invoiced" icon="file-invoice" href="/pt/events/order-invoiced">
    A autoridade fiscal autorizou o documento, em qualquer país, via seu provedor fiscal.
  </Card>
</CardGroup>
