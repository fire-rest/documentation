...
Valores possíveis de `fiscal.status`: `pending`, `processing`, `fiscal_graphic`, `authorized`, `cancelling`, `cancelled`, `rejected`, `denied`, `error`. O Fire ainda mantém dois estados internos para pedidos sem documento — esses são reportados aqui como `null`, para não vazar contabilidade interna dentro do seu contrato.
