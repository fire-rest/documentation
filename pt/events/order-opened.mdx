...
<Tabs>
  <Tab title="v1 · atual">
    Você está vendo o contrato **atual (v1)** de `order.opened`. Este evento não tem v0 — nasceu com o snapshot enriquecido (`data.policy`, `data.lastKnown`).
  </Tab>
</Tabs>
...
Os blocos restantes — `store`, `client`, `channel`, `orderLines`, `fulfillment`, `kds`, `device`, `operator`, `marketing`, `metadata`, `payments.totals` — são idênticos a `order.completed`. Em vez de duplicá-los, veja a [referência de campos de `order.completed`](/pt/events/order-completed#field-reference).
