...
<Tabs>
  ...
  <Tab title="v1 · descontinuado">
    <Warning>O contrato **v1** está **descontinuado** — mantido como referência histórica. Abra aqui: [`order.completed` — v1](/pt/events-v1/order-completed).</Warning>
  </Tab>
...
<Note>
  Os valores monetários são **strings com o valor inteiro escalado ×10.000** — quatro casas decimais fixas. `"229000"` é **22,9 BRL**, não 229.000. Para ler, divida por 10.000.

  É a escala com que o FIRE armazena: evita o drift de ponto flutuante ao somar impostos através de várias integrações. Parseie com uma biblioteca decimal, nunca com `parseFloat`.

  A exceção é `paymentMethods[].totalBill`, que a origem às vezes envia como número JSON — trate os dois casos.

  <Warning>
    **Não é a escala de todos os caminhos.** A requisição de
    [numeração fiscal](/pt/api-reference/fiscal-documents) leva os valores **sem escalar**, tal como foram cobrados. Se você tirar um valor deste evento para declará-lo ao órgão, divida antes.
  </Warning>
</Note>
...
<ResponseField name="payments" type="object">
  ...
  <Expandable title="payments">
    ...
    <ResponseField name="paymentMethods" type="object[]">
      ...
      Cada entrada: `processor`, `currencyCode`, `paymentMethodCode`, `transactionType`, `transactionId`, `transactionStatus`, `exactPayment`, `totalBill`, `acquirer`, `card`, `transactionDate`, `metadata`, `mid`, `tid`, `voucher`, `referenceNumber`, `authorizationCode`, `customerCashAmount`, `changeAmount`, `idAuth`, `receiptCustomer`, `receiptMerchant`.
      ...
      - **`customerCashAmount`** (`number | null`) — Dinheiro entregue pelo cliente, em decimais: para uma conta de `22.9` paga com uma nota de `50`, `50`. `0` nos meios que não são dinheiro. Os pedidos injetados antes de 24 de setembro de 2026 trazem aqui o texto `"0"` — leia como número. `null` quando não foi enviado.
      - **`changeAmount`** (`number | null`) — Troco entregue pelo caixa, em decimais. O troco real em dinheiro, `0` nos demais meios, `null` quando não foi enviado.
    </ResponseField>
...
<ResponseField name="orderLines" type="object[]">
  ...
  <Expandable title="orderLines[n]">
    ...
    <ResponseField name="rewardId" type="string | null">Código de resgate do cupom que originou o desconto desta linha — corresponde a uma entrada de `marketing.coupons[].rewardId`. É `null` quando o desconto não veio de um cupom (uma remarcação de catálogo), e chega preenchido com `discountValue: "0"` quando o cupom mudou o preço de tabela em vez de descontar. As duas formas são válidas. Um cupom por linha: se ao mesmo item aplicaram um segundo, ele aparece apenas em `marketing.coupons[]`.</ResponseField>
    ...
    <ResponseField name="modifierGroups" type="object[]">
      Grupos de modificadores selecionados. Cada um: `{ uid, description, selectedModifiers[] }`. `selectedModifiers[n]` tem recursivamente a forma de uma linha de pedido (price, modifierGroups, itemDescription, …) até alguns níveis, e carrega seu próprio `rewardId` — num combo o desconto costuma estar nos modificadores, não no contêiner.
    </ResponseField>
    <ResponseField name="metadata" type="object">
...
<ResponseField name="marketing" type="object | null">
  Loyalty e cupons. É `null` quando o pedido não trouxe nenhum dos dois.

  <Expandable title="marketing.coupons[n]">
    <ResponseField name="rewardId" type="string">Código de resgate. Une-se a `orderLines[].rewardId` e ao `rewardId` de qualquer modificador aninhado.</ResponseField>
    <ResponseField name="name" type="string">Nome da promoção (por exemplo `VERAO6`). Não é único entre promoções.</ResponseField>
    <ResponseField name="metadata" type="object">`{ id, scope, type, itemId, description, percentage, value, baseValue, amount }`. `scope` é `product`, `order` ou `shipping`; `type` é `DISCOUNT_PERCENTAGE`, `DISCOUNT_FIXED`, `DISCOUNT_POINTS` ou `PRODUCT`. `amount` é o dinheiro que ele descontou; `value` é o valor nominal da regra (uma porcentagem, ou pontos — **não** moeda). `baseValue: null` significa que o preço de tabela não é conhecido, o que não é o mesmo que zero.</ResponseField>
  </Expandable>

  Um cupom com `scope: "order"` é o seu único rastro no payload: o valor chega já distribuído entre as linhas e nada liga as parcelas a ele. Leia-o aqui, não a partir das linhas.
</ResponseField>
...
- **Blocos específicos da SEFAZ — apenas Brasil.** `payments.metadata.fiscal` (vBC / vNF / vICMS …) e `orderLines[].metadata.fiscal` (ncm/cfop/csosn) existem **apenas no BR**. Os demais países ainda carregam seu `taxes[]`, mas esses blocos SEFAZ estão ausentes. Os eventos `order.invoiced` / `order.reversed` não são exclusivos do Brasil: disparam em **todos os países**, e o país viaja em `fiscal.countryCode`.
...
async function onOrderCompleted(data) {
  ...
  await db.orders.upsert({
    ...
    create: {
      ...
      // payments.totals[0].total é "229000" está escalado ×10.000 → 22,9
...
<CardGroup cols={2}>
  ...
  <Card title="order.invoiced" icon="file-invoice" href="/pt/events/order-invoiced">
    Dispara quando o órgão fiscal autoriza o documento fiscal do pedido, em qualquer país.
  </Card>
</CardGroup>
...
| Campo | O que é |
|---|---|
| `numberingStatus` | Como terminou o **ato de numerar**: `GENERATED` ou `NOT_GENERATED` — só dois: os estados internos do gateway colapsam neles. **Não é o veredito do órgão** — esse está em `lastKnown.fiscal.status`. |
