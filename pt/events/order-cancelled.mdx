...
`order.cancelled` dispara quando um pedido previamente injetado é cancelado — pelo backoffice do Fire, um adaptador externo ou a API de cancelamento. **Não** retrai um `order.completed` anterior do mesmo pedido; ambos os eventos são emitidos independentes.
...
O mesmo snapshot V4 do pedido que [`order.completed`](/pt/events/order-completed), com três diferenças:
...
Quando o pedido não tinha documento autorizado (foi cancelado antes de o órgão responder, a loja não emite documentos, ou o documento não tem id do provedor), a **chave `fiscal` está ausente** — não `null`:
...
<ResponseField name="cancellation" type="object">
  ...
  <Expandable title="cancellation">
    ...
    <ResponseField name="cancelledBy" type="string | null">
      UUID do usuário que cancelou o pedido. `null` para cancelamentos iniciados pelo sistema.
    </ResponseField>
    ...
    <ResponseField name="cancellationSource" type="string">
      Onde o cancelamento se originou: `backoffice`, `adapter`, `api`.
    </ResponseField>
    <ResponseField name="fiscal" type="object">
      **Presente apenas quando o pedido tinha um documento autorizado** com id do provedor. Carrega `status` e `authority` — veja [O bloco fiscal](#o-bloco-fiscal). Neste evento **não** carrega `history`, `compensates`, `countryCode`, `providerCode` nem `occurredAt`; esses chegam com `order.reversed`.
    </ResponseField>
  </Expandable>
</ResponseField>
...
Sem mudanças na v3. É a numeração fiscal impressa no caixa, um bloco diferente de `cancellation.fiscal`. Quando um cancelamento numera uma nota de crédito, essa nota fica no topo e a fatura desce para `fiscalRepresentation.history`. A referência de campos está na [página v2.2](/pt/events-v2/order-cancelled#data-fiscalrepresentation).
