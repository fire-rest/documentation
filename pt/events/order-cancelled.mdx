...
<Tabs>
  <Tab title="v1 · atual">
    Contrato **atual (v1)**. `order.cancelled` carrega o mesmo snapshot v1 que [`order.completed`](/pt/events/order-completed) — descontos de pedido/produto, impostos granulares, enum `itemType`, `deliveryConfirmationCode` — mais o bloco de auditoria `cancellation`.
  </Tab>
  <Tab title="v0 · descontinuado">
    <Warning>O contrato **v0** está **deprecado** — mantido apenas como referência histórica. Abra aqui: [order-cancelled — v0](/pt/events-v0/order-cancelled).</Warning>
  </Tab>
</Tabs>
