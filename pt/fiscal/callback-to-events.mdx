...
| `rejected`, `denied`, `error` | `rejected`, `denied`, `error` | nenhum — nos eventos, visível apenas em `lastKnown.fiscal.status` do próximo evento daquele pedido |
...
| `authority.countryData` | de `document` | `chaveAcesso`, `numero`, `serie`, `modelo`, `protocolo`, `cStat` |
