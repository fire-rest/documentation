...
description: "Como definir os campos de configuração de cada método —credenciais e ajustes— por marca, loja ou dispositivo no PayBridge (conta)."
---
...
Além de decidir **onde** um método está ligado ([Disponibilidade](/pt/manuals/paybridge/availability)), no PayBridge você define **com quais dados** ele opera. Esses dados —credenciais, identificadores e demais campos— são salvos por nível, combinados do geral ao específico, e são o que o Fire mantém **sincronizado com a DSI** para que depois cada cobrança viaje leve.
