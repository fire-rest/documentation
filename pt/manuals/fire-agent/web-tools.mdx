...
<Note>
  A impressão da cozinha no fire-kds tenta primeiro a URL da **plataforma detectada** (Windows `wss://localhost:9101` versus Android `wss://kds-agent.fire.rest:9101`) e **cai na outra** se essa conexão falhar. Você não escolhe a URL na tela do KDS. Tablets: [Instalação no Android](/pt/manuals/fire-agent/android).
</Note>
