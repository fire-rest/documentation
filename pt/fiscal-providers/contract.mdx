...
| `totals` | não | **O que foi cobrado**, com o detalhamento de impostos. Ver [3.5](#35-client-e-totals) |
...
<Warning>
  **Os valores chegam como números JSON, não como texto**, e com as casas decimais que o valor
  tiver — não com as que o seu regime exige. Na Colômbia são inteiros, porque o peso não usa
  centavos; no Equador vêm com até duas casas.

  E o JSON **não conserva os zeros à direita**: um valor de `8.70` chega como `8.7`, e um de
  `50000.00` como `50000`.

  Por isso, **se o seu identificador é um hash sobre uma string, a formatação é sua.** O CUFE é
  calculado sobre `"50000.00"`, e essa string não existe na requisição: você a monta a partir do
  número. O FIRE não a formata porque não conhece a regra do seu regime, e escolher um formato
  errado deste lado seria pior do que não escolher nenhum.

  O que garantimos é o **valor**: é exatamente o que foi cobrado e impresso.

  **E isso importa especialmente porque você vai consumir os eventos do pedido.** Não é opcional: a
  numeração lhe dá os identificadores, mas **a venda que você emite ao órgão sai do evento** — e de
  lá você volta com o callback. Sem esse circuito, ninguém sabe se o documento foi emitido.

  No evento, o mesmo valor trafega em **outra escala**: inteiro em string, **×10.000**, porque é
  assim que o FIRE o armazena.

  Um exemplo com a Colômbia — **cada país trafega na sua moeda e com os seus impostos, mas a regra
  da escala é a mesma**. Uma venda de **50.000 COP** com IVA de **7.983,19 COP**:

  <CodeGroup>

  ```json NESTA requisição — valores reais
  "totals": [
    {
      "currencyCode": "COP",
      "total": 50000,
  ...
  ```json No evento do pedido — inteiros ×10.000
  "totals": [
    {
      "currencyCode": "COP",
      "total": "500000000",
  ...
  **Divida por 10.000 antes de declarar qualquer valor que tenha tirado do evento**, e use os desta
  requisição para o hash. Os dois caminhos têm de terminar no mesmo número: o que está impresso no
  comprovante.

  Se você tirar o valor do evento sem dividir, declara **500.000.000 COP** por uma venda de **50.000
  COP** — dez mil vezes o montante. O documento fica bem formado e o órgão o aceita.
