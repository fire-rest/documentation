...
<Note>
  Este guia se aplica aos quatro eventos que o Fire emite em produção: `order.completed`, `order.cancelled`, `fiscal.authorized.br`, `fiscal.cancelled.br`. O walkthrough usa `order.completed`; os outros três seguem o mesmo padrão com formatos distintos de `data`.
</Note>
