...
<Info>
  **Um único contrato de numeração, todos os países com gateway.** O Fire resolve internamente
  qual provedor corresponde a cada país. A integração de numeração é a mesma em todos os países
  atendidos pelo gateway (hoje Equador e Colômbia): o que muda é quais campos vêm preenchidos
  na resposta, não a forma de pedir. O Brasil não numera por esta via — emite pelo PlugNotas
  depois da venda; veja [Como o fiscal funciona no Fire](/pt/fiscal/overview).
</Info>
...
| Evento | Quando |
|---|---|
| [`order.invoiced`](/pt/events/order-invoiced) | O órgão **autorizou** o documento — os identificadores do órgão chegam em `countryData` |
...
| **Do país** | `countryData` | O vocabulário do órgão que numerou: `claveAcceso` no Equador, `cufe` na Colômbia. **Percorra, não indexe** |
...
<AccordionGroup>
  ...
  <Accordion title="A Fire não te trava, mas a conta pode" icon="shield-check">
    A Fire **nunca** devolve um erro que te deixa sem saber o que fazer. Falhe o que falhar, a
    resposta traz uma decisão explícita em `policy.numberingFailure.action`.

    O que mudou: essa decisão **nem sempre é seguir**. A conta configura, por vendor, o que
    acontece quando não deu para numerar.

    - **`CONTINUE`** — o padrão, e o comportamento de sempre. Imprima conforme `printing.mode`
      —normalmente um ticket não fiscal— e injete o pedido do mesmo jeito. O comprovante se
      resolve depois.
    - **`REFUND`** — devolva a cobrança no balcão e **não injete o pedido**. Essa venda não
      aconteceu; tem que ser [reportada](/pt/api-reference/lost-sales) ou não fica rastro
      de que houve dinheiro envolvido.

    **Nos dois casos: não retenha a venda nem retente em loop com o cliente na frente.** O
    dinheiro já foi cobrado.

    <Warning>
      **Com `CONTINUE`, nada completa sozinho.** Para conseguir o comprovante é preciso chamar
      de novo com o **mesmo `orderCode`** — a Fire retoma a solicitação e pergunta de novo ao
      provedor. Um `202` que ninguém retenta fica assim para sempre.

      Com `REFUND` é o contrário: **não retente**. Numerar uma venda que você devolveu geraria
      um comprovante de algo que não aconteceu.
    </Warning>
  </Accordion>
...
<Check>Você percorre `countryData` em vez de indexar chaves fixas: quem as define é o país que numerou, e na Colômbia não há `claveAcceso`.</Check>
