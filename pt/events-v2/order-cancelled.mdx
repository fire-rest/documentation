...
<Warning>
  **Descontinuada (v2.2).** Contrato anterior, mantido apenas como referência histórica. A versão atual é [order.cancelled — v3](/pt/events/order-cancelled).
</Warning>
...
```json
{
  ...
  "data": {
    ...
    "marketing": null,  /* igual a order.completed — leva coupons[] quando o pedido teve algum */
...
<Note>Todo bloco acima que apareça elidido ou marcado como *igual a `order.completed`* —`orderLines`, `marketing`, `payments`, `client`, `store`, `fulfillment`, `kds`, `device`, `channel`, `operator`— é exatamente o snapshot que `order.completed` carrega, porque todos os eventos de pedido são montados a partir do mesmo snapshot v4. Isso inclui **`orderLines[].rewardId`** e **`marketing.coupons[]`**. Veja a [referência de campos de `order.completed`](/pt/events/order-completed#referência-de-campos).</Note>
