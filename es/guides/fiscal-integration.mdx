...
<AccordionGroup>
  ...
  <Accordion title="La venta nunca se bloquea" icon="shield-check">
    ...
    `printing.mode: "PROVISIONAL_RECEIPT"`. Imprimís un ticket no fiscal e inyectás la orden
    igual.

    **No retengas la venta ni reintentes en bucle mientras el cliente espera.** El dinero ya
    se cobró; el comprobante se resuelve después.

    <Warning>
      **Nadie la completa solo.** Para conseguir el comprobante hay que volver a llamar con
      el **mismo `orderCode`** — Fire retoma la solicitud y le vuelve a preguntar al
      proveedor. Un `202` que nadie reintenta se queda así para siempre.
    </Warning>
  </Accordion>
