...
<Info>
  **Un solo contrato de numeración, todos los países con gateway.** Fire resuelve por dentro
  qué proveedor corresponde a cada país. La integración de numeración es la misma en todos los
  países que atiende el gateway (hoy Ecuador y Colombia): lo que cambia es qué campos vienen
  llenos en la respuesta, no la forma de pedirla. Brasil no numera por esta vía — emite por
  PlugNotas después de la venta; ver [Cómo funciona lo fiscal en Fire](/es/fiscal/overview).
</Info>
...
| Evento | Cuándo |
|---|---|
| [`order.invoiced`](/es/events/order-invoiced) | El ente **autorizó** el documento — los identificadores del ente llegan en `countryData` |
...
| **Del país** | `countryData` | El vocabulario del ente que numeró: `claveAcceso` en Ecuador, `cufe` en Colombia. **Recorrelo, no lo indexes** |
...
<Check>Recorrés `countryData` en vez de indexar claves fijas: las pone el país que numeró, y en Colombia no hay `claveAcceso`.</Check>
