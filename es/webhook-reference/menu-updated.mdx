...
**Formato:** ISO 8601 con precisión de **segundos** y sufijo `Z` — `"2026-08-04T12:30:00Z"`. Siempre en **UTC**, nunca en la zona de la tienda: el valor es el mismo hecho para todas las tiendas del menú, que pueden estar en países distintos. Para mostrarlo en hora local usa el `timezone` que viaja en `list`. Sin milisegundos — a diferencia de `event.createdAt`, que sí los lleva.
...
No existe `assignedAt` a nivel `list`: el campo describe cuándo la entidad entró al **menú**, no cuándo una tienda empezó a recibirlo.
...
- **`list.schedules`:** ventanas de tiempo en las que el canal está activo para esa tienda. Un array vacío (`[]`) indica que el canal opera 24 horas — sin restricciones.
...
<Info>
  ¿Buscás un webhook más liviano más un fetch a demanda en vez del payload completo?
  Ver [Entrega híbrida](/es/hybrid-delivery/overview) — una alternativa propuesta
  para menús, todavía no implementada.
</Info>
