...
| `rejected`, `denied`, `error` | `rejected`, `denied`, `error` | ninguno — visible solo en `lastKnown.fiscal.status` del siguiente evento de esa orden |
...
| `authority.countryData` | desde `document` | `chaveAcesso`, `numero`, `serie`, `modelo`, `protocolo` |
...
- **No hay guard por proveedor.** Una orden tiene un solo documento. Un `authorized` cuyo `eventId` no coincide con el documento existente de la orden termina en `notFound` y no cambia nada. Un `cancelled` resuelve el documento más reciente de la orden y lo actualiza, sin importar qué proveedor lo emitió — PlugNotas incluido.
