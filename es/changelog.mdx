...
## 25 de septiembre de 2026 — Efectivo entregado y vuelto en los eventos de la orden

Los dos campos de efectivo de `payments.paymentMethods[]` en los eventos que llevan el snapshot V4 de la orden (`order.opened`, `order.completed`, `order.invoiced`, `order.cancelled`, `order.reversed`). Las versiones de los eventos no cambian.

- **`customerCashAmount` es un número y trae el monto real.** Hasta el 24 de septiembre de 2026 viajaba siempre como el texto `"0"`, incluso en pagos en efectivo. Ahora es el efectivo que entregó el cliente: para una cuenta de `22.9` pagada con un billete de `50`, `50`; `0` en los medios que no son efectivo.
- **`changeAmount` figura en la referencia** — el vuelto que entregó la caja, como número: `27.1` en el ejemplo anterior, `0` en los medios que no son efectivo, `null` si la caja no lo mandó. Ya viajaba en los eventos; faltaba en la referencia.
- **Si tu consumidor validaba que `customerCashAmount` fuera texto, ahora falla.** Leelo como número. Los eventos de órdenes inyectadas antes del 24 de septiembre siguen trayendo `"0"`, así que un reenvío de una orden vieja también.

Actualizado en **EN / ES / PT**.

## 16 de septiembre de 2026 — `cStat` vuelve a emitirse, dentro de `countryData`

**Esto corrige lo que dice una entrada posterior.** El 15 de septiembre anunciamos que `cStat` ya no se emitía. Los consumidores lo pidieron de vuelta —sin él una anulación no dice si el ente la aceptó— así que vuelve. Solo `cStat`: `docType`, `dataAutorizacao` y `sefazCancellation` siguen afuera.

- **Brasil: `authority.countryData.cStat`.** El código con el que el ente resolvió **este** documento: `100` en una autorización, `135` en una anulación. Es vocabulario del ente, así que viaja al lado de `chaveAcesso` y `protocolo`, no en la raíz del bloque.
- **La anulación lleva su propio código, no el de la factura.** El `order.reversed` trae `135` —el evento de cancelación registrado en SEFAZ— mientras el `order.invoiced` que lo precedió conserva `100`. Verificado de punta a punta en staging.
- **Viaja tal cual lo manda el proveedor**, `number` o `string`. No normalizamos un tipo que el ente no eligió.
- **Por el callback fiscal ya funcionaba**: todo lo que va dentro de `document` y no es canónico llega a `countryData` sin tocarse. Mandá `cStat` ahí y llega — no hubo que cambiar nada de tu lado.
- **`mensagemSefaz` sigue afuera.** Vuelve solo el código.
- **Los rechazos siguen sin emitir evento.** Un documento `rejected` guarda su código pero no dispara `order.invoiced` ni `order.reversed`, así que para ese caso `cStat` no te llega.

Actualizado en **EN / ES / PT**.

## 15 de septiembre de 2026 — Impresión al enviar del KDS, zoom y Agente Fire Android

La impresión de cocina ahora es una regla de tienda (comanda y/o fiscal en una estación). Las tablets necesitan su propio Agente Fire. El zoom y la reimpresión del operador son otra impresora.

- **[Impresión al enviar](/es/manuals/kds/peripherals-printing)** — **By screen** o **By station**, documentos y timing fiscal. Arma la cola en la pantalla KDS. El dashboard avisa si la estación de impresión no tiene pantalla activa.
- **[Acciones en pantalla](/es/manuals/kds/screen-actions)** — **zoom** de estación/ticket (doble toque al temporizador, tecla **Q**). Si la impresora falla con el fiscal listo, el ticket muestra **Queued: printer failed**.
- **[Tablero de operador](/es/manuals/kds/operator-board)** — **Operator fiscal printer** en el PC del supervisor (no la cola de cocina).
- **[Instalación en Android](/es/manuals/fire-agent/android)** — APK en la tablet del KDS. `kds-agent.fire.rest` debe resolver a `127.0.0.1`. El token y el **nombre** de impresora deben coincidir con fire-kds.
- **[Estaciones](/es/manuals/kds/stations)** / **[Ruteo](/es/manuals/kds/routing)** — el veto de item type en la estación desapareció. Las categorías viven solo en la estrategia de ruteo.

Actualizado en **EN / ES / PT**.

## 15 de septiembre de 2026 — Fechas de la anulación y rechazos

Dos correcciones sobre lo que guarda el [callback fiscal](/es/api-reference/fiscal-callback) y, por lo tanto, sobre lo que llevan [`order.invoiced`](/es/events/order-invoiced) y [`order.reversed`](/es/events/order-reversed). Las versiones de los eventos no cambian.

- **La anulación conserva las fechas de la factura.** `authority.issuedAt` y `authority.authorizedAt` se mantienen como los tenía la factura, aunque el callback `cancelled` mande los suyos. Antes tomaban la hora de la anulación mientras `compensates.issuedAt` seguía apuntando a la factura, así que el mismo bloque contaba dos historias. Para saber cuándo se anuló el documento, usa `authority.cancelledAt`.
- **Un resultado negativo no guarda datos de comprobante.** En `rejected`, `denied` y `error`, Fire ignora `documentNumber`, `providerDocId`, `issuedAt`, `authorizedAt`, los importes, `pdfUrl`, `xmlUrl` y `graphic` aunque el proveedor los mande; se guardan `documentType`, `docSubtype`, las claves del país en `document`, `failure`, `provider` y `metadata`. **Un documento rechazado se leía igual que uno autorizado**, con número y con `authorizedAt`. Un documento que ya tenía datos de comprobante los conserva.

Actualizado en **EN / ES / PT**.

## 15 de septiembre de 2026 — Bloque fiscal estructurado y eventos fiscales nuevos

El bloque fiscal de los tres eventos fiscales ahora es lo que Fire guarda en la orden, con una sola forma para todos los países: el veredicto del ente en `authority`, más el recorrido del documento y a cuál reemplaza. Dos guías nuevas explican el ciclo completo.

- **Breaking — el bloque fiscal de [`order.invoiced`](/es/events/order-invoiced), [`order.cancelled`](/es/events/order-cancelled) y [`order.reversed`](/es/events/order-reversed) es estructurado.** Los campos canónicos pasan a `authority` (`documentType`, `documentNumber`, `issuedAt`, `cancelledAt`…), los importes a `authority.amounts` y los identificadores del ente (`chaveAcesso`, `claveAcceso`, `cufe`, `protocolo`…) a `authority.countryData`. `dataAutorizacao` pasa a ser `authority.authorizedAt`; `docType`, `cStat` y `sefazCancellation` ya no se emiten. Cada ruta anterior tiene exactamente una ruta nueva: ver [Rutas que cambian](/es/events/order-invoiced#rutas-que-cambian-respecto-del-contrato-anterior).
- **Breaking — en una anulación el bloque pasa de `cancellation.metadata.fiscal` a `cancellation.fiscal`**, y `cancellation.metadata` ya no se emite. Si la orden no tenía documento autorizado, la clave `fiscal` **no está** — antes venía `null`, así que una comparación con `=== null` deja de cumplirse.
- **Breaking — el [callback fiscal](/es/api-reference/fiscal-callback) sacó todos los campos canónicos de `document` a la raíz** (`documentType`, `docSubtype`, `documentNumber`, `issuedAt`, `authorizedAt`, `cancelledAt`, `authorizationMode`, `providerDocId`, `totalAmount`, `taxAmount`, `currencyCode`, `pdfUrl`, `xmlUrl`, `graphic`); `document` ahora lleva solo los identificadores del país. Renombres: `countryCode` → `country`, `docType` → `documentType` (`SALE_INVOICE` o `CREDIT_NOTE`, los únicos dos aceptados), `error` → `failure` (con `scope` y `details`), `providerSpecific` → `metadata`, `emittedAt` → `issuedAt`. Un emisor con los nombres anteriores recibe `400` con el campo — en el caso de `docType`, en `authorized` y `cancelled`. En Ecuador se retira `numeroAutorizacion`: el identificador autorizado es `claveAcceso`. `cnpjEmitente` no es parte del contrato.
- **`order.cancelled` lleva el bloque fiscal en todos los países** con documento autorizado; antes era solo Brasil.
- **`history` y `compensates`** — `order.invoiced` y `order.reversed` llevan cada parada del recorrido del documento, de la más vieja a la más nueva. `compensates` apunta al documento que el vigente reemplaza: viene lleno en `order.reversed` y en `null` en `order.invoiced`.
- **`authority.providerIdentity` y `authority.providerMetadata`** — el `provider` y el `metadata` del callback ahora llegan a la orden y a los eventos, con los mismos nombres que en la [numeración fiscal](/es/api-reference/fiscal-documents). `null` si no se mandan; PlugNotas (Brasil) nunca los manda.
- **[Cómo funciona lo fiscal en Fire](/es/fiscal/overview)** — los dos actos (numerar y autorizar), los dos bloques de la orden y los dos caminos por los que un documento llega a Fire.
- **[Del callback a los eventos](/es/fiscal/callback-to-events)** — qué evento sale por cada callback y dónde cae cada campo.

<Warning>Los flujos y handlers que leen las rutas anteriores — `fiscal.chaveAcesso`, `fiscal.protocolo`, `cancellation.metadata.fiscal.*` — leen `undefined` en las versiones nuevas, sin error. Actualízalos antes de depender de estos eventos.</Warning>

<Note>En Brasil `authority.authorizedAt` trae la fecha de la SEFAZ con una hora convencional de medianoche, así que puede quedar antes que `issuedAt`. Para el momento de la venta usa `issuedAt`.</Note>

| Evento | Versión |
|---|---|
| [`order.invoiced`](/es/events/order-invoiced) | v1.2 → **v2** |
| [`order.cancelled`](/es/events/order-cancelled) | v2.2 → **v3** |
| [`order.reversed`](/es/events/order-reversed) | v2 → **v3** |

No es retrocompatible en el bloque fiscal; el resto del snapshot de la orden no cambia. Las versiones anteriores siguen disponibles desde las pestañas de versión de cada evento.

Actualizado en **EN / ES / PT**.

## 4 de septiembre de 2026 — Cupones en las líneas de la orden

Las órdenes ya pueden llegar con cupones. [`order.completed`](/es/events/order-completed) lleva quién financió cada descuento, en la línea y en la orden.

- **`orderLines[].rewardId`** — el código de canje del cupón, o `null`. Los modificadores seleccionados también lo llevan: en un combo el descuento suele estar en los modificadores, no en el contenedor. Hay dos formas válidas y ninguna es un error: `null` con descuento (una rebaja de catálogo, sin cupón) y un código con `discountValue: "0"` (el cupón cambió el precio de lista en vez de descontar).
- **[`data.marketing.coupons[]`](/es/events/order-completed)** — estaba reservado para uso futuro; ahora lleva `{ rewardId, name, metadata }` por cupón. `metadata.scope` es `product`, `order` o `shipping`, y `metadata.amount` es la plata que descontó. Un cupón con `scope: "order"` aparece **solo** acá: su monto llega a las líneas ya repartido entre ellas y nada une las porciones con él.

<Warning>En órdenes de kiosco `totals.discountValue` es el total de **todos** los niveles — el cupón de orden y los descuentos de línea juntos, porque el origen reparte el cupón de orden dentro de las líneas antes de enviar. Sumarlo con la suma de `orderLines[].discountValue` cuenta la misma plata dos veces. Usá uno u otro, no los dos.</Warning>
...
## 3 de septiembre de 2026 — Credenciales Fire del KDS, filas de comandas y categorías de estación

La impresión y la cancelación de cocina necesitan el host y la key correctos de Fire. Las tarjetas pueden ocupar de una a tres filas. Los productos reciben una categoría de ruteo y una miniatura de cocina.

- **[Credenciales Fire API](/es/manuals/kds/fire-credentials)** — default de cuenta frente a override por vendor. La base URL es solo el host de Fire; fire-kds agrega las rutas de print y cancel.
- **[Categorías de estación](/es/manuals/kds/station-categories)** — códigos usados como `item_type` en el ruteo, más miniaturas de producto en la vista de la categoría.
- **[Credenciales Fire del KDS (técnico)](/es/guides/kds-fire-credentials)** — fila de vendor → default de cuenta → env. Print solo lee `FISCAL_PRINT_*`. Cancel puede caer a `FIRE_*`.
- **[Acciones en pantalla](/es/manuals/kds/screen-actions)** — **Filas de comandas** (1–3) reservan espacio para notas. Si Fire rechaza un cancel, el operador ve el rechazo y el detalle de policy; el trabajo queda fallido.

Actualizado en **EN / ES / PT**.

## 3 de septiembre de 2026 — El catálogo: productos, categorías y grupos de modificadores

Tres manuales nuevos para las pantallas que alimentan todo lo demás. Un menú no inventa nada
—elige del catálogo—, así que el trabajo empieza de verdad aquí.

- **[Productos](/es/manuals/backoffice/products)** — los cuatro tipos, los interruptores que
  deciden si algo se vende, el precio, los modificadores, los combos y las sugerencias.
- **[Categorías](/es/manuals/backoffice/categories)** — los bloques con los que el cliente
  recorre la carta, su horario y la exclusividad de canal.
- **[Grupos de modificadores](/es/manuals/backoffice/modifier-groups)** — las preguntas que se
  hacen al pedir, y las opciones que las responden.
- **Las opciones de un modificador son productos del catálogo.** Para ofrecer queso extra tiene
  que existir un producto queso extra; su precio se lee de ese producto, nunca se escribe en el
  grupo.
- **En un grupo solo se ponen dos números: mínimo y máximo.** Obligatorio/opcional y
  única/múltiple se deducen de ellos, así que un grupo no puede contradecirse.
- **La exclusividad de canal se hereda.** Marcar una categoría como exclusiva la saca del Golden
  Menu *junto con sus productos*.
- **Un combo no tiene pestaña de precio**, y el precio que muestra el listado es una referencia
  calculada, no lo que cobras. El precio real sale de la lista de precios del destino.

<Note>
  La pestaña de precios depende del país: en Brasil está simplificada al precio final más la
  categoría fiscal (NCM). Las variantes de producto están previstas pero todavía no tienen
  pantalla.
</Note>

Actualizado en **EN / ES / PT**.

## 3 de septiembre de 2026 — Menús: armar la carta y hacerla llegar al canal

Dos manuales nuevos para el par de pantallas que deciden **qué** se vende y **dónde**: el menú
Golden y los menús custom que heredan de él, y la matriz que pone cada menú en cada tienda, canal
y tipo de fulfillment.

- **[Menús](/es/manuals/backoffice/menus)** — Golden y custom, la herencia, el diálogo de
  creación, las pestañas Structure y Overrides, y las lentes de precio y de día.
- **[Asignación de menús](/es/manuals/backoffice/menu-assignments)** — la matriz tienda x canal x
  fulfillment, la asignación masiva, el Sync y sus advertencias, y la previsualización.
- **El tipo de fulfillment se elige una vez y no se puede cambiar.** Es el único campo del menú
  sin vuelta atrás: equivocarse significa crear el menú de nuevo.
- **Guardar un menú no lo publica — en el editor no hay botón de publicar.** El menú llega a un
  canal recién cuando alguien sincroniza la fila correspondiente en la pantalla de asignaciones.
- **El Sync aplasta el menú fresco, sin versionado.** Manda el menú y la lista de precios tal
  como están guardados en ese segundo; no se puede republicar un estado anterior.
- **Una fila puede quedar en Synced y aun así haber perdido productos.** Un producto sin precio
  en la lista asignada queda fuera del envío en vez de publicarse en cero, y lo único que lo dice
  es el chip de advertencias.

<Note>
  Los canales agregadores reciben sus eventos, pero su resultado todavía no se refleja en el
  estado de sincronización de esta pantalla. La pestaña History del menú también sigue vacía.
</Note>

Actualizado en **EN / ES / PT**.

## 2 de septiembre de 2026 — Agotados: apagar productos durante el servicio

Manual nuevo de la pantalla **Agotados**: sacar un producto de la venta en una tienda cuando se
acaba en cocina, sin editar el menú y sin que nadie tenga que deshacerlo después.

- **[Agotados](/es/manuals/backoffice/out-of-stock)** — el diálogo de apagado, el alcance por
  canal, el motivo obligatorio, la restauración y la pestaña de auditoría.
- **Un apagado nunca sobrevive al cierre de la tienda.** El techo es el fin de la jornada
  operativa, no una cantidad fija de horas: un producto apagado hoy amanece vendiéndose mañana.
- **El alcance es un par de canal y tipo de entrega.** Un producto puede quedar apagado en el
  kiosco y seguir vendiéndose en caja.
- **Como opción de un combo, el producto se sigue pudiendo elegir.** Si se vende dentro de grupos
  de modificadores, el diálogo los nombra y el apagado los registra, pero lo que reciben los canales
  es disponibilidad por producto: una opción no tiene dónde entrar ahí.
- **El motivo es obligatorio y no se edita.** Cambiar la vigencia mueve solo la hora — el motivo
  queda como el registro de por qué se apagó el producto.
- **Publicar el menú cierra el apagado, pero solo si cubrió todo su alcance.** Un apagado de toda la
  tienda sobrevive a una publicación que llegó a un canal, porque en los otros el producto sigue
  fuera de venta de verdad. El desenlace es **Pisado por un sync**.
- **El apagado queda guardado aunque el canal no se entere.** Avisar se informa aparte de guardar, y
  los apagados cuyo aviso no llegó se cuentan en la cabecera y quedan marcados en su fila.

<Note>
  El apagado sí se propaga a los canales de venta: `products/listing` por producto para los canales
  X-Mart y [`product.availability_changed`](/es/webhook-reference/product-availability-changed) por
  canal para los agregadores, en las tres direcciones (apagar, restaurar, vencer). La fila queda como
  registro operativo de Fire —qué se dejó de vender, cuándo y por qué— y nunca entra al snapshot del
  menú: esa separación es deliberada, no una etapa pendiente. Lo que no se propaga son los productos
  que se venden solo como opción de modificador: ninguno de los dos contratos tiene dónde ponerlos.
</Note>
...
- **`categories[n].assignedAt`** y **`products[n].additionalInfo.assignedAt`** — fecha en que la entidad entró al menú, ISO 8601 UTC, sin milisegundos. Es un dato de **membresía**, no de edición: no cambia con ediciones de precio, nombre, imagen u orden; sí cambia (fecha nueva) cuando el ítem se quita del menú y se vuelve a agregar. Casos borde completos en [`menu.updated` → Fecha de incorporación al menú](/es/webhook-reference/menu-updated#fecha-de-incorporación-al-menú). No viaja en [`product.price_updated`](/es/webhook-reference/product-price-updated) ni en [`product.availability_changed`](/es/webhook-reference/product-availability-changed) — esos eventos solo transmiten su delta.
- **[`menu.updated`](/es/webhook-reference/menu-updated) / [`product.updated`](/es/webhook-reference/product-updated) usan `taxInfo`** (no `taxesInfo`), **`type: PRODUCTO`** para ítems estándar y **`modifierGroups[n].type: RADIO`** para grupos de selección única (`CHECKBOX` para múltiple).
