...
## 15 de septiembre de 2026 — Bloque fiscal estructurado y nuevas versiones de los eventos fiscales
...
- **Breaking — el bloque fiscal de [`order.invoiced`](/es/events/order-invoiced), [`order.cancelled`](/es/events/order-cancelled) y [`order.reversed`](/es/events/order-reversed) es estructurado.** Los campos canónicos pasan a `authority` (`documentType`, `documentNumber`, `issuedAt`, `authorizedAt`, `cancelledAt`…), los importes a `authority.amounts` y los identificadores del ente (`chaveAcesso`, `claveAcceso`, `cufe`, `protocolo`…) a `authority.countryData`. En una anulación el bloque sube de `cancellation.metadata.fiscal` a **`cancellation.fiscal`**. Ya no se emiten `docType`, `dataAutorizacao`, `cStat` ni `sefazCancellation`. Cada ruta anterior tiene exactamente una ruta nueva: ver [Rutas que cambian](/es/events/order-invoiced#rutas-que-cambian-respecto-del-contrato-anterior).
- **Breaking — el [callback fiscal](/es/api-reference/fiscal-callback) recibe `documentType`** (`SALE_INVOICE` o `CREDIT_NOTE`, los únicos dos aceptados) en lugar de `docType`; un callback que siga mandando solo `docType` recibe `400`. `documentNumber`, `authorizedAt` y `graphic` van en la raíz, y lo requerido depende del `eventType`. `cnpjEmitente` no es parte del contrato.
- **`history` y `compensates`** — `order.invoiced` y `order.reversed` llevan cada parada del recorrido del documento, de la más vieja a la más nueva, y un puntero al documento que el vigente reemplaza.
- **`authority.providerIdentity` y `authority.providerMetadata`** — el `provider` y el `metadata` del callback ahora llegan a la orden y a los eventos, con los mismos nombres que en la numeración fiscal. `null` si no se mandan; PlugNotas (Brasil) nunca los manda.
- **`order.invoiced` y `order.reversed` salen en todos los países** — el país viaja en `fiscal.countryCode`, no en el nombre del evento.
...
<Warning>Los flujos y handlers que leen las rutas anteriores — `fiscal.chaveAcesso`, `fiscal.protocolo`, `cancellation.metadata.fiscal.*` — leen `undefined` en las versiones nuevas, sin error. Actualizalos antes de depender de estos eventos.</Warning>
