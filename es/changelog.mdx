...
- **Las versiones en inglés y portugués de [`menu.updated`](/es/webhook-reference/menu-updated-v2) / [`product.updated`](/es/webhook-reference/product-updated) tenían nombres de campo desactualizados** — `taxesInfo` ahora es `taxInfo`, `type: PRODUCT` ahora es `type: PRODUCTO`, y `modifierGroups[n].type: single` ahora es `RADIO`. Si tu integración lee esos eventos en inglés o portugués, revisá que coincida con los nombres actuales.
- **`storeId` en la doc en inglés de `menu.updated` decía `store_number`; es el UUID interno** (`stores.id`), misma convención que el resto de los eventos.
