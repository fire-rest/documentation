...
- **`data.fiscalRepresentation`** — serie, secuencial, número de documento y clave de
  acceso, tal como se imprimieron en la caja. **Se omite** (la clave no aparece, no viene
  en `null`) cuando la venta no se numeró: agregadores, países sin representación fiscal,
  o numeración desactivada. Que esté presente **no** significa que el comprobante esté
  autorizado — el veredicto del ente sigue en `lastKnown.fiscal.status`. No cambia nunca,
  ni siquiera después de que el ente autorice o rechace.
