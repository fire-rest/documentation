...
{/*
  Formulario de suscripción, con el estilo de la doc en ambos temas.

  Envía al iframe oculto de abajo en vez de navegar, así el lector nunca sale de
  la página. Buttondown bloquea el AJAX cross-origin y su API key no puede vivir
  en el navegador, así que esta es la única forma de conservar el diseño y al
  lector en su lugar.

  El costo: la respuesta cae en un iframe que no podemos leer por ser de otro
  origen, así que el mensaje de abajo es optimista — dice "revisá tu correo",
  nunca "ya estás suscrito". El doble opt-in y el firewall de Buttondown siguen
  aplicando; su captcha Turnstile no, porque vive en la página de ellos.

  Repetido en en/ y pt/ con los textos traducidos. El `name` del iframe tiene
  que seguir siendo único por página.
*/}
...
<div style={{border: '1px solid rgb(128 128 128 / 0.25)', borderRadius: '0.75rem', padding: '1.25rem', marginBottom: '2.5rem'}}>
  ...
  <form action="https://buttondown.com/api/emails/embed-subscribe/fire-docs" method="post" target="fire-docs-subscribe-es" onSubmit={(e) => { const f = e.currentTarget; setTimeout(() => { f.style.display = 'none'; f.nextElementSibling.style.display = 'block' }, 0) }} style={{display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '0.875rem'}}>
    <input type="email" name="email" required placeholder="tu@empresa.com" style={{flex: '1 1 16rem', minWidth: 0, padding: '0.5rem 0.75rem', borderRadius: '0.5rem', border: '1px solid rgb(128 128 128 / 0.4)', background: 'transparent', color: 'inherit', fontSize: '0.875rem'}} />
    <button type="submit" style={{padding: '0.5rem 1.25rem', borderRadius: '0.5rem', border: 'none', background: '#E6293D', color: '#fff', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer'}}>Suscribirme</button>
  </form>
  <div style={{display: 'none', fontSize: '0.875rem', marginTop: '0.875rem'}}>Ya casi — revisa tu correo y confirma la suscripción.</div>
  <iframe name="fire-docs-subscribe-es" title="Resultado de la suscripción" style={{display: 'none'}} />
</div>
