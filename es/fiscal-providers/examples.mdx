...
Fijate en lo que **no** viaja: ni `accountId`, ni `vendorId` —el tenant sale de la API
key—, ni códigos del SRI, ni referencia al catálogo del proveedor. Y los campos sin
configurar se omiten en vez de viajar en `""`.
...
- **`document` habla ecuatoriano.** `claveAcceso`, `secuencial`, `ambiente` — los nombres del
  SRI, no una traducción. Y no hay campos de otros países: el número de control venezolano
  sencillamente no existe acá.
- **El número visible viene armado en `numeroComprobante`** (`005-004-000000052`). Lo componés
  vos, que conocés la regla —art. 18 del Reglamento— y FIRE lo imprime tal cual, sin
  reformatearlo. Las tres piezas siguen viajando aparte, como las define el SRI, pero son para
  conciliar: nadie las vuelve a unir.
...
- **`graphic.qr` coincide con `document.claveAcceso`.** En Ecuador es así, y la redundancia es
  deliberada: la alternativa es que el punto de venta sepa qué se codifica en cada país.
