...
<Warning>
  ...
  **Para volver al importe real, dividí por 10.000.** Y como la escala son 4 decimales fijos,
  ...
  ```json En el request y en el evento — la misma escala
  "totals": [
    {
      "currencyCode": "COP",
      "total": "500000000",
...
### 6.1 Qué código va en cada caso

`5xx` no es el cajón de todo lo que salió mal. La diferencia con `422` se decide con una sola
pregunta: **el mismo request, mandado otra vez dentro de un rato, ¿puede andar?**

| Situación | HTTP | `retryable` |
|---|---|---|
| Un campo del request falta, sobra o tiene el formato equivocado | `422` | `false` |
| La tienda no está en tu catálogo, o le falta la configuración que necesitás para numerar —identidad fiscal, resolución, clave técnica— | `422` | `false` |
| El ente rechaza por una regla del régimen | `422` | `false` |
| Tu servicio, tu base o el ente no están disponibles ahora | `503` | `true` |
| Se rompió algo tuyo y todavía no sabés qué | `500` | `true` |
| No atendés ese país | `404` | `false` |

<Warning>
  **Que falte configuración es `422`, no `500`.**

  Es el error más caro de confundir, porque el `5xx` nos dice justo lo contrario de lo que
  corresponde hacer. Una tienda sin clave técnica va a fallar igual dentro de un segundo y
  dentro de una semana: no hay nada que esperar. Pero un `5xx` nos hace reintentar —con el
  cliente parado en la caja— y después deja la venta en `FAILED_RETRYABLE`, que significa
  "volvé más tarde" para algo que solo se arregla cargando un dato.

  Y del otro lado queda un `"Internal Server Error"` que no nombra ningún campo. Quien lo lee no
  puede hacer nada más que abrir un ticket — que es exactamente lo que `failure.message` existe
  para evitar.
</Warning>

### 6.2 Un error no manejado también responde el sobre

**El `catch` global es parte del contrato.** Una excepción que no atrapaste sigue siendo la
respuesta a un pedido de numeración, y quien la recibe necesita las mismas dos cosas de siempre:
si tiene sentido reintentar, y por dónde empezar a mirar.

El cuerpo que arma el framework solo no alcanza:

```jsonc
// ❌ Esto no cumple el contrato
{
  "path": "/api/v1/fiscal/co/prekeys",
  "error": "Internal Server Error",
  "status": 500,
  "timestamp": "2026-09-10T14:32:42.418Z"
}
```

No trae `retryable`, así que hay que adivinar. No trae `failure`, así que no dice qué pasó. No
trae `orderCode`, así que ni siquiera afirma de qué venta habla. Es el manejador por defecto de
NestJS —el de Spring Boot es igual con las llaves en otro orden— y llega cuando nadie lo
reemplazó.

Lo mínimo que sí cumple:

```json
HTTP 500

{
  "orderCode": "FUEL-CO-1789050796519",
  "retryable": true,
  "failure": {
    "code": "INTERNAL_ERROR",
    "message": "error interno al calcular el CUFE",
    "details": []
  },
  "provider": { "name": "…", "version": "…", "reference": "req-01H8XK3M4P" }
}
```

<Note>
  **En un `500`, `provider.reference` es lo más valioso del cuerpo.** Es el identificador con el
  que vas a buscar en TUS logs cuando te reportemos el caso. Sin él la conversación empieza con
  "falló el 10 de septiembre a las 14:32 UTC" y sigue con vos buscando a mano.

  Y poné un `failure.code` aunque sea genérico. `INTERNAL_ERROR` es peor que
  `CUFE_COMPUTATION_FAILED`, pero es infinitamente mejor que nada: sobre un código se puede
  alertar, sobre un texto libre no.
</Note>

<Warning>
  **Un `500` con `retryable: true` es una promesa sobre tu idempotencia.** Vamos a reintentar con
  el mismo `orderCode`, y si el error ocurrió DESPUÉS de que consumiste el secuencial, ese
  reintento tiene que devolver el documento que ya numeraste con `reused: true` —ver
  [4](#4-idempotencia)—, no numerar otro.

  Es el caso que explica por qué la llave es `country + orderCode + operation` y no algo que se
  genere al empezar a procesar: cuando se rompe a la mitad, la llave tiene que existir desde
  antes.
</Warning>
...
### 8.1 `graphic` en el callback

**El mismo bloque que [5.3](#53-graphic--lo-imprimible), en la raíz del callback.** Llave-valor,
cada valor el string exacto a codificar, sin interpretar.

```json
{
  "eventId": "59810570-c167-4998-b321-5ac788ca0767",
  "orderId": "8c80aeb1-7de1-4ad4-98b8-ab2121a51c09",
  "eventType": "authorized",
  "countryCode": "EC",
  "providerEventId": "EC:8c80aeb1-…:01",
  "document": { "…": "el bloque de tu país — ver 5.2" },
  "graphic": { "qr": "3108202601179141513200110010200000000221234567819" }
}
```

**Por qué en la raíz y no dentro de `document`.** Por lo mismo que en la numeración: `graphic`
no es un dato del comprobante, es el artefacto que se imprime. Que esté en el mismo lugar en
los dos extremos es lo que permite integrar una vez y leer igual en ambos — si en un lado
estuviera anidado y en el otro no, quien concilie los dos caminos tendría que recordar la
diferencia.

**Es opcional, y así se queda.** Si tu integración no lo manda, todo sigue funcionando como
hasta hoy: ningún callback que hoy es válido deja de serlo.

<Note>
  **Cuándo mandarlo.** Cuando el código imprimible solo lo conocés vos. El caso claro es Brasil:
  el QR de la NFC-e lleva un hash calculado con el **CSC** del emisor, un secreto que solo tiene
  quien emite — no se deriva de la chave y nadie más puede componerlo.

  Donde el valor coincide con un campo que ya mandás —Ecuador, donde es la clave de acceso—
  mandarlo igual le ahorra al punto de venta tener que saber que en Ecuador se codifica la
  clave, en Brasil la URL y en Chile el TED. Es la misma redundancia deliberada de 5.3.
</Note>

<Warning>
  **No lleva simbología, y no es un olvido.** Cómo se dibuja cada código —QR, Code128, PDF417—
  lo declara la plantilla del comprobante, no el proveedor: es una regla del régimen, no de la
  integración. En Ecuador el valor es la clave de acceso y el RIDE la imprime como código de
  barras, no como QR — por eso la llave nombra el **propósito** (`qr`, `barcode`, `ted`) y no
  el símbolo.
</Warning>

**Colombia sigue sin mandarlo**, igual que en la numeración: su QR ya viaja resuelto en
`document.qrCode`. Repetirlo acá sería el mismo dato en dos lugares que pueden discrepar.
