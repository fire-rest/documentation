...
<Warning>
  **Los importes viajan multiplicados por 10.000, en el request de numeración y en el evento de la orden por igual.**

  Todo lo monetario — `totals` en el request de numeración y `data.payments` en el evento —
  viaja como **entero en string, multiplicado por 10.000**. Es la escala con la que FIRE almacena,
  para hacer aritmética con enteros y no arrastrar error de punto flotante al sumar impuestos.
  ...
  **Dividí por 10.000 donde leas un importe** — en el request de numeración o en el evento —
  antes de declararlo al ente o de calcular un hash como el CUFE. Los porcentajes no se escalan:
  una tasa viaja tal cual. Ver [el contrato](/es/fiscal-providers/contract).

  Descubrirlo tarde cuesta caro: si no dividís, declarás **500.000.000 COP** por una venta de
