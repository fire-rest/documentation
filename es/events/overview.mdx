...
| [`order.cancelled`](/es/events/order-cancelled) | Snapshot V4 de orden + bloque `cancellation` de auditoría, con `cancellation.fiscal` (`status`, `authority`) cuando la orden tenía un documento autorizado |
| [`order.invoiced`](/es/events/order-invoiced) | Snapshot V4 de orden + bloque `fiscal`: `authority` (campos canónicos, `amounts`, `countryData`, `graphic`), `history`, `compensates` |
| [`order.reversed`](/es/events/order-reversed) | Snapshot V4 de orden + bloque `cancellation` de auditoría + `cancellation.fiscal` con el documento cancelado, `history` y `compensates` |
...
<CardGroup cols={2}>
  ...
  <Card title="order.invoiced" icon="file-invoice" href="/es/events/order-invoiced">
    La autoridad fiscal autorizó el documento, en cualquier país, vía tu proveedor fiscal.
  </Card>
</CardGroup>
