...
<Note>
  Fire **no** firma con HMAC las peticiones salientes hoy. Autentica usando un Bearer token, API key u OAuth2 client credentials configurado en el nodo HTTP de tu flow. La firma HMAC (`X-Fire-Signature`) está en el roadmap; esta página se actualizará cuando salga.
</Note>
