...
`order.cancelled` dispara cuando se cancela una orden previamente inyectada — desde el backoffice de Fire, un adaptador externo o la API de cancelación. **No** retrae un `order.completed` previo de la misma orden; ambos eventos se emiten independientes.
...
El mismo snapshot V4 de orden que [`order.completed`](/es/events/order-completed), con tres diferencias:
...
Cuando la orden no tenía un documento autorizado (se canceló antes de que la autoridad respondiera, la tienda no emite documentos, o el documento no tiene id de proveedor), **la clave `fiscal` está ausente** — no en `null`:
...
<ResponseField name="cancellation" type="object">
  ...
  <Expandable title="cancellation">
    ...
    <ResponseField name="cancelledBy" type="string | null">
      UUID del usuario que canceló la orden. `null` para cancelaciones iniciadas por el sistema.
    </ResponseField>
    ...
    <ResponseField name="cancellationSource" type="string">
      Dónde se originó la cancelación: `backoffice`, `adapter`, `api`.
    </ResponseField>
    <ResponseField name="fiscal" type="object">
      **Presente solo cuando la orden tenía un documento autorizado** con id de proveedor. Lleva `status` y `authority` — ver [El bloque fiscal](#el-bloque-fiscal). En este evento **no** lleva `history`, `compensates`, `countryCode`, `providerCode` ni `occurredAt`; esos llegan con `order.reversed`.
    </ResponseField>
  </Expandable>
</ResponseField>
...
Sin cambios en v3. Es la numeración fiscal impresa en la caja, un bloque distinto de `cancellation.fiscal`. Cuando una cancelación numera una nota de crédito, esa nota queda arriba y la factura baja a `fiscalRepresentation.history`. La referencia de campos está en la [página v2.2](/es/events-v2/order-cancelled#data-fiscalrepresentation).
