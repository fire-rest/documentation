...
`policy`, `lastKnown` y `fiscalRepresentation` viajan exactamente igual que en `order.completed`. En Brasil (PlugNotas) `fiscalRepresentation` es `null`: no hay paso de numeración.
...
Cuando la orden no tenía un documento autorizado (se canceló antes de que la autoridad respondiera, la tienda no emite documentos, o el documento no tiene id de proveedor), **la clave `fiscal` está ausente** — no en `null`. Acá, una orden de Ecuador numerada en la caja y cancelada antes de que el ente respondiera: `fiscalRepresentation` lleva la factura que se imprimió, `lastKnown.fiscal` todavía dice `processing` y `cancellation` no tiene `fiscal`. No sigue ningún `order.reversed`.
...
```json
{
  ...
  "data": {
    ...
    "fiscalRepresentation": {
      ...
      /* resto del bloque — ver data.fiscalRepresentation abajo */
    },
    ...
    "cancellation": {
      ...
      "cancellationReason": "El cliente pidió cancelar",
      "cancellationNote": null,
      "cancellationSource": "backoffice"
    }
  },
