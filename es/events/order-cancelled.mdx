...
<Tabs>
  <Tab title="v2.1 · actual">
    Estás viendo el contrato **actual (v2.1)** de `order.cancelled`. **v2.1 agrega** `data.policy.deferredPayment` (¿esta orden puede trabajarse antes del pago?) y `data.lastKnown` (estado orientativo de cocina/fiscal), más el evento nuevo [`order.opened`](/es/events/order-opened). Solo aditivo — nada de lo que ya leías cambió. También lleva el motivo de cancelación estructurado que introdujo v2 — `cancellationType`, `cancellationGroup` y `cancellationNote`, siempre presentes (`null` cuando están vacíos).
  </Tab>
