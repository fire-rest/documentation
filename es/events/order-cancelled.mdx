...
<Tabs>
  <Tab title="v1 · actual">
    Contrato **actual (v1)**. `order.cancelled` lleva el mismo snapshot v1 que [`order.completed`](/es/events/order-completed) — descuentos de orden/producto, impuestos granulares, enum `itemType`, `deliveryConfirmationCode` — más el bloque de auditoría `cancellation`.
  </Tab>
