...
<Tabs>
  <Tab title="v1 · actual">
    Estás viendo el contrato **actual (v1)** de `order.opened`. Este evento no tiene v0 — nació con el snapshot enriquecido (`data.policy`, `data.lastKnown`).
  </Tab>
</Tabs>
...
Los bloques restantes — `store`, `client`, `channel`, `orderLines`, `fulfillment`, `kds`, `device`, `operator`, `marketing`, `metadata`, `payments.totals` — son idénticos a `order.completed`. En vez de duplicarlos, mirá la [referencia de campos de `order.completed`](/es/events/order-completed#field-reference).
