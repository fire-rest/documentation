...
Valores posibles de `fiscal.status`: `pending`, `processing`, `fiscal_graphic`, `authorized`, `cancelling`, `cancelled`, `rejected`, `denied`, `error`. Fire además lleva dos estados internos para órdenes sin documento todavía — esos se reportan acá como `null`, para no filtrar contabilidad interna dentro de tu contrato.
