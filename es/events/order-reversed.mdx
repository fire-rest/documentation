...
description: "Solo Brasil — SEFAZ confirmó la cancelación de un documento fiscal previamente autorizado vía tu proveedor fiscal."
---

<Tabs>
  <Tab title="v1 · actual">
    Contrato **actual (v1)**. El snapshot de orden sigue el contrato v1 de `order.cancelled` — descuentos, impuestos granulares, enum `itemType`, `deliveryConfirmationCode` — más el bloque `cancellation` y los datos `sefazCancellation`.
  </Tab>
  <Tab title="v0 · deprecado">
    <Warning>El contrato **v0** está **deprecado** — se mantiene solo como referencia histórica. Ábrelo acá: [order-cancelled — v0](/es/events-v0/order-cancelled).</Warning>
  </Tab>
</Tabs>

`fiscal.cancelled.br` dispara cuando SEFAZ confirma la cancelación de un documento fiscal (NFC-e o NF-e) que había sido previamente autorizado. Es la contraparte SEFAZ-confirmada de [`order.cancelled`](/es/events/order-cancelled): la orden se cancela primero dentro de Fire (disparando `order.cancelled`), Fire después pide la cancelación en SEFAZ vía tu proveedor fiscal, y `fiscal.cancelled.br` dispara solo cuando SEFAZ estampa el protocolo de cancelación.

## Condición de disparo

Fire emite `fiscal.cancelled.br` una vez por cancelación fiscal, la primera vez que **todo** lo siguiente es verdadero:

- La orden está en una tienda brasileña con `storeFiscalConfig.enabled === true`
- Un documento fiscal había sido previamente autorizado (es decir, `fiscal.authorized.br` se emitió antes)
- Se envió una solicitud de cancelación a tu proveedor fiscal
- tu proveedor fiscal reporta confirmación SEFAZ de la cancelación
...
| Latencia relativa a `order.cancelled` | Usualmente segundos; puede ser minutos si SEFAZ está degradado |
| Precondición | Un `fiscal.authorized.br` previo para el mismo `orderId` |
...
Este es el único bloque que es único de `fiscal.cancelled.br`. Cada otro campo se comparte con [`order.cancelled`](/es/events/order-cancelled) — consulta esa página para los campos de auditoría de cancelación.
...
Para una orden brasileña fiscal-enabled, espera los cuatro eventos arriba. Para órdenes brasileñas **sin** autorización fiscal (porque la orden se canceló antes de la emisión fiscal, o fiscal estaba deshabilitado), solo dispara `order.cancelled` — no `fiscal.cancelled.br`.
...
- **No proceses `fiscal.cancelled.br` sin `order.cancelled` primero.** Disparan en orden en flujo normal, pero entrega fuera de orden es posible. Si recibes `fiscal.cancelled.br` para un `orderId` que no tienes registrado como cancelado, loguéalo y crea el registro desde el bloque `cancellation` de este evento — no botes el evento.
