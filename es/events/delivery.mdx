...
- **Sin garantía de orden entre tipos de evento para la misma orden.** `fiscal.authorized.br` puede llegar antes o después de `order.completed` para el mismo `data.orderId`. No asumas que uno precede al otro.
