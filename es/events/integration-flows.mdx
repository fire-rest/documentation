...
description: "Cómo tu sistema se suscribe a eventos de Fire. Un flow es un grafo de nodos que configuras en el dashboard — su nodo HTTP es dueño de la petición que llega a tu endpoint."
---
...
Esta página cubre el modelo end to end: reglas de matching, estructura del grafo del flow, qué controla el nodo HTTP y cómo el body template referencia el trigger data.

## Cómo funciona el matching

Cuando dispara un evento de negocio dentro de Fire, el runtime consulta cada flow activo cuyos criterios matchean:
...
| `triggerType` | Igual al tipo de evento (`order.completed`, `order.cancelled`, `fiscal.authorized.br`, `fiscal.cancelled.br`) |
| `storeCodes` | Vacío (matchea todas las tiendas) o contiene el código de tienda del evento |
| `isActive` | `true` |
...
Una flow definition es un grafo guardado como JSON. El nodo trigger es el punto de entrada; los nodos siguientes consumen el trigger context.
...
```ts
{
  ...
  nodes: Array<{
    ...
    position: { x, y },         // layout del dashboard
    data: { /* config específico del nodo */ },
  }>,
  edges: Array<{
    id: string,
    source: string,             // node id
    target: string,
    sourceHandle?: string,       // p. ej. "200", "_error"
  }>,
}
```
...
| Internal nodes | Llama servicios internos de Fire (emit fiscal, status update, etc.) — usado para orquestación cross-tenant |
...
Cada template de nodo puede referenciar el **trigger context**, generado por el runtime cuando el flow corre:
...
Fire actualmente no firma peticiones salientes con HMAC. Autentica configurando credenciales en el nodo HTTP del flow:
...
Cada fila de flow lleva `accountId` y `vendorId` — el runtime nunca corre un flow a través de tenants. El aislamiento se hace a nivel de BD (modelo FLAT: columnas `account_id`, `vendor_id` en `integration_flows`). Cuando configuras un flow, queda con scope a tu tenant por default; no puedes suscribirte a eventos de otro account.

## Activación, versionado y dry-run

- Los flows tienen un flag `is_active` — toggle para arrancar o parar de recibir eventos sin borrar el flow.
- Cada save crea una nueva `version`. Las versiones pasadas se conservan para auditoría; solo la última versión activa corre.
- El botón **Probar Flujo** del dashboard corre un flow con un payload sintético (`GENERIC_SAMPLE_PAYLOAD` o un escenario por país) sin afectar data real — perfecto para verificar body templates antes de pasar a producción.
