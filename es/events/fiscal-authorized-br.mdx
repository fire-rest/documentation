...
description: "Solo Brasil — SEFAZ autorizó el documento fiscal (NFC-e o NF-e) de una orden vía tu proveedor fiscal."
---

`fiscal.authorized.br` dispara cuando SEFAZ (la autoridad tributaria brasileña) autoriza el documento fiscal asociado a una orden. Lo emite el pipeline fiscal brasileño que se integra con tu proveedor fiscal como proveedor de documentos.

Este evento es **separado de** [`order.completed`](/es/events/order-completed): la orden se paga primero (`order.completed`), después Fire pide la emisión fiscal vía tu proveedor fiscal, y `fiscal.authorized.br` dispara solo cuando SEFAZ devuelve un protocolo de autorización.

## Condición de disparo

Fire emite `fiscal.authorized.br` una vez por documento fiscal, la primera vez que **todo** lo siguiente es verdadero:
...
- tu proveedor fiscal reporta autorización SEFAZ (status transiciona a `authorized` / `cStat` es el código SEFAZ "autorizado")

| | |
|---|---|
| Cobertura | **Solo Brasil** — el sufijo `.br` es significativo; no hay equivalente para otros países hoy |
| Tipos de documento | `nfce` (factura de consumidor) y `nfe` (factura B2B) |
| Llave de idempotencia | `event.id` |
| Dispara más de una vez | No, salvo en reintentos |
| Latencia relativa a `order.completed` | Usualmente segundos; puede ser minutos si SEFAZ está degradado |

## Qué hay en `trigger.data`

Mismo snapshot V4 que [`order.completed`](/es/events/order-completed) más un bloque top-level **`fiscal`** llevando las referencias del documento autorizado por SEFAZ.
...
<ResponseField name="fiscal" type="object">
  ...
  <Expandable title="fiscal">
    <ResponseField name="status" type="string">
      Status del documento. Para este evento, siempre `"authorized"`. Otros valores que puedes ver en tu proveedor fiscal (`pending`, `processing`, `rejected`, `denied`) **no** disparan `fiscal.authorized.br` — solo el estado terminal autorizado.
    </ResponseField>
...
- **No hay eventos fiscales para otros países.** Solo Brasil hoy. Otros países' pipelines fiscales existen en el codebase (esquemas CO/EC/CL/AR/VE) pero no están cableados para disparar flow triggers todavía.
...
<CardGroup cols={2}>
  ...
  <Card title="fiscal.cancelled.br" icon="file-circle-xmark" href="/es/events/fiscal-cancelled-br">
