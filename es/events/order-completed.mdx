...
<Tabs>
  <Tab title="v1.1 · actual">
    Estás viendo el contrato **actual (v1.1)** de `order.completed`. **v1.1 agrega** `data.policy.deferredPayment` (¿esta orden puede trabajarse antes del pago?) y `data.lastKnown` (estado orientativo de cocina/fiscal), más el evento nuevo [`order.opened`](/es/events/order-opened). Solo aditivo — nada de lo que ya leías cambió.
  </Tab>
...
**Se omite** —la clave no aparece, no viene en `null`— cuando la venta no se
numeró: agregadores, países sin representación fiscal, o comercios con la
numeración desactivada. Ramificá por presencia de la clave.
