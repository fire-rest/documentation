...
Cada vez que Fire envía un evento, usa ese secret para calcular una firma del cuerpo de la petición y la adjunta como header `X-Fire-Signature`, junto con un header `X-Fire-Timestamp`. Tu sistema realiza el mismo cálculo sobre el body que recibe y compara el resultado:
...
<Note>
  El header que lleva la firma es `X-Fire-Signature` por default, pero es
  configurable por integración — si el dashboard muestra un nombre distinto para tu
  integración, usa ese.
</Note>
...
<Steps>
  <Step title="Lee el header de firma">
    Extrae el valor del header `X-Fire-Signature` de la petición entrante. Tiene la
    forma `sha256=<hex digest>` — es un prefijo, no un hex plano.
  </Step>
  <Step title="Recalcula la firma">
    Usando tu secret de webhook, calcula HMAC-SHA256 del **body crudo** de la petición (antes de cualquier parseo JSON), y antepón `sha256=` al hex resultante.
  </Step>
  <Step title="Compara">
    Compara el valor calculado (con el prefijo `sha256=`) contra el valor completo del header usando una comparación de tiempo constante. Si coinciden, procesa el evento. Si no, devuelve `400` y descártalo.
  </Step>
</Steps>
...
Cada entrega también lleva un header `X-Fire-Timestamp` (Unix seconds — momento en que
Fire firmó la petición). Fire no impone hoy una ventana de tolerancia del lado
servidor: si querés protección contra una petición capturada y reenviada después,
valida que el timestamp esté dentro de una tolerancia que elijas (p. ej. 5 minutos)
