...
<ParamField body="client.billingInformation" type="object">
  **El destinatario de la factura, y es el que manda.** Fire arma el comprador del comprobante
  leyendo primero `billingInformation` (`govIdType`, `govIdNumber`, `businessName`, `email`,
  `address`) y recién después los campos de `client`. Existe aparte porque la factura puede ir a
