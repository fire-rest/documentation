...
<Tabs>
  <Tab title="Brasil (BR)">
    ...
    | `cnpjEmitente` | string \| null | | **No es parte del contrato.** Se acepta y pasa intacto a `countryData`; Fire nunca lo lee |
...
| Body del callback | Evento: `fiscal.authority` (o `cancellation.fiscal.authority`) |
|---|---|
...
| `country` | `fiscal.countryCode` (fuera de `authority`); si la orden ya tenía país fiscal, se conserva el guardado |
| `occurredAt` | `fiscal.occurredAt` (fuera de `authority`) |
| `provider`, `metadata`, `failure`, `providerEventId` | no pasan al evento |
