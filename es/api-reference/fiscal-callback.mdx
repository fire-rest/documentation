...
<ParamField body="providerEventId" type="string" required>
  El id propio de tu proveedor para esta entrega. Se guarda para auditoría/forense — **no es la llave de idempotencia**. Fire deduplica por `(orderId, eventId)` — un `eventType` distinto sobre el mismo par devuelve `409`, salvo `fiscal_graphic` — así que podés enviar un `providerEventId` nuevo en cada reintento sin crear duplicados. Usa el ID nativo del evento del proveedor si está disponible; si no, un UUID.
</ParamField>
