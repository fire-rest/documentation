...
<ParamField body="eventId" type="string" required>
  UUID para correlación. **Fire valida que referencie un evento que Fire emitió para esta misma orden** — ecoa el `event.id` de un envelope V4 que recibiste (el de la emisión, o el de la cancelación para callbacks `cancelled`). Un `eventId` que no corresponda a un evento de Fire para esa orden se rechaza con `400` antes del `202`. Nunca inventes este valor.
</ParamField>
