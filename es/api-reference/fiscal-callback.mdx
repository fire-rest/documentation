...
| `issuedAt` | string (ISO 8601) \| null | | Fecha/hora UTC en que se emitió el documento. **Antes se llamaba `emittedAt`** — ver el aviso abajo |
...
<Warning>
  **Cambio de nombre: `emittedAt` → `issuedAt`**

  Este campo pasó a llamarse `issuedAt`, que es como se llama **el mismo hecho** en la
  respuesta de numeración fiscal. Eran dos nombres para lo mismo, y esa diferencia obligaba
  a saber por qué camino había entrado el documento para poder leer su fecha.

  **Si todavía mandás `emittedAt`, no perdés el dato**: se sigue aceptando al ingestar. Pero
  no está declarado en el contrato y va a dejar de aceptarse, así que migrá al nombre nuevo.
</Warning>
