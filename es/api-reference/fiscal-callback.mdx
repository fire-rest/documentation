...
<ParamField header="Authorization" type="string">
  Alias legado: la misma API key de Fire, como `Bearer pk_live_...`. Prefiere `x-api-key`; si llegan los dos, se usa `x-api-key`.
</ParamField>
...
<ParamField body="providerEventId" type="string" required>
  El id propio de tu proveedor para esta entrega. Se guarda para auditoría/forense — **no es la llave de idempotencia**. Fire deduplica por `(orderId, eventId)` — un `eventType` distinto sobre el mismo par devuelve `409`, salvo `fiscal_graphic` y los reintentos después de un fallo (ver [Reintentos y anulación rechazada](#reintentos-y-anulación-rechazada)) — así que podés enviar un `providerEventId` nuevo en cada reintento sin crear duplicados. Usa el ID nativo del evento del proveedor si está disponible; si no, un UUID.
</ParamField>
...
Fire acepta **exactamente dos** valores en `documentType`. No hay un tercer tipo: cualquier otro valor devuelve `400`.

| `documentType` | Qué es | `eventType` del callback | Operación de numeración |
|---|---|---|---|
| `SALE_INVOICE` | El documento de la venta: factura, NFC-e / NF-e, boleta… | `authorized` | `INVOICE` |
| `CREDIT_NOTE` | El documento que anula una venta | `cancelled` | `CANCEL` |

Cómo llama cada régimen a su documento va en `docSubtype`, y ese campo es **abierto**: `nfce`, `nfe` (Brasil), `factura` (Ecuador, Colombia), `boleta` (Chile), `factura_a` (Argentina) y cualquier otro nombre que use tu autoridad. Fire no lo valida contra una lista ni decide nada con él.

<Note>
  No lo confundas con el [catálogo de tipos de documento](/es/api-reference/fiscal-document-types): ese endpoint lista los documentos de **identificación del comprador** (`CEDULA`, `CPF`, `NIT`…), no tipos de documento fiscal.
</Note>
...
<ParamField body="authorizedAt" type="string">
  Fecha/hora UTC (ISO 8601) en que el ente **aprobó** el documento — un hecho distinto de `issuedAt`. Pueden separarse por segundos (autorización en línea) o por horas (contingencia autorizada cuando vuelve la conexión). Opcional; si no viene, los consumidores de los eventos ven `authority.authorizedAt: null`. En una cancelación usa `cancelledAt`: un callback `cancelled` **conserva el `issuedAt` y el `authorizedAt` que el documento ya tenía** —son los de la factura— y solo usa los que mandes si Fire no tenía ninguno.
</ParamField>
  ...
  **Antes se llamaba `error`**, y ahora lleva la misma forma que la respuesta de numeración fiscal.
  ...
  <Expandable title="failure">
    ...
    <ParamField body="details" type="array">
      Detalle campo por campo, cuando el ente lo da: una lista de `{ "field": "…", "issue": "…" }`, ambos opcionales.
    </ParamField>
  </Expandable>
</ParamField>

<ParamField body="metadata" type="object">
  Bag free-form para extras específicos del proveedor (respuesta raw, IDs internos, etc.). No validado. Llega a la orden y a los eventos como `authority.providerMetadata`, tal cual y opaco — igual que en la numeración fiscal. Ausente o `{}` queda en `null`. No mandes `null`: omítelo o manda `{}` — `null` devuelve `400`.
...
<Tabs>
  <Tab title="Brasil (BR)">
    ...
    | `cStat` | int / string \| null | | Código de estado de SEFAZ: `100` autorizado, `135` cancelación registrada, `865` rechazado. Llega a los eventos como `authority.countryData.cStat` |

    El ejemplo de abajo es un payload `authorized` completo: cada campo de raíz que muestra es **requerido** para ese evento, salvo `provider` y `metadata`, que son opcionales y llegan a la orden como `authority.providerIdentity` y `authority.providerMetadata` — ver [Qué es requerido, por evento](#qué-es-requerido-por-evento).
...
`fiscal_graphic` reporta que tu proveedor está entregando la **representación gráfica** — lo que imprime la caja para el documento. Es un **estado propio**, no un resultado:
...
Para estados no autorizados, omite `document` (o mándalo en `null`) y provee `failure`. El `country` sigue aplicando (valida el routing); el documento no se requiere porque no hay artefacto autorizado — y `documentType`/`docSubtype` tampoco, porque no hay comprobante que tipificar.

**Un resultado negativo no guarda datos de comprobante.** Si mandas `documentNumber`, `providerDocId`, `issuedAt`, `authorizedAt`, importes, `pdfUrl`, `xmlUrl` o `graphic` con `rejected`, `denied` o `error`, Fire los ignora: solo se guardan `documentType`, `docSubtype`, las claves del país en `document`, `failure`, `provider` y `metadata`. Un documento que ya tenía datos de comprobante los conserva.
...
### Reintentos y anulación rechazada

**Una emisión fallida se puede reintentar.** Si reportaste `rejected`, `denied` o `error` y después reintentás ante el ente, mandá el nuevo resultado con el **mismo `eventId`** — el de la emisión. Fire lo acepta y el flujo sigue normal: si el nuevo resultado es `authorized`, se despacha `order.invoiced`. Si el reintento vuelve a fallar, el motivo nuevo reemplaza al anterior.

```text
rejected (eventId E1)  →  authorized (eventId E1)   ✓ se aplica, sale order.invoiced
rejected (eventId E1)  →  error (eventId E1)        ✓ se guarda el motivo nuevo
```

**Anulación rechazada por el ente.** Cuando Fire te pide anular una factura autorizada (evento `order.cancelled`), la orden queda "anulando". Si el ente **no acepta la anulación**, mandá `rejected`, `denied` o `error` con el `eventId` de **esa** anulación y el motivo en `failure`. Fire lo interpreta como anulación fallida: **la factura sigue vigente** (la orden vuelve a `authorized`, con el motivo en `error`) y no se despacha ningún evento. Si después reintentás y el ente la acepta, mandá `cancelled` con el mismo `eventId` y se despacha `order.reversed`.

```text
order.cancelled (eventId C1)  →  rejected (C1)   → la factura sigue autorizada
                              →  cancelled (C1)  → anulada, sale order.reversed
```

<Note>
  Un `rejected` sobre una factura autorizada que **no** se está anulando sigue significando lo mismo que siempre: el ente rechazó la factura.
</Note>
...
<ResponseField name="duplicate" type="boolean">
  `true` cuando este `(orderId, eventId)` ya se ingresó **con el mismo `eventType`** — se devuelve el registro existente y no se re-encola nada. `false` para un evento nuevo. Un `eventType` distinto sobre el mismo par nunca llega a este campo: devuelve `409`, salvo `fiscal_graphic`.
</ResponseField>
...
Fire deduplica callbacks por **`(orderId, eventId)`** — *no* por `providerEventId` (que podés regenerar libremente). El mismo par reenviado con el **mismo** `eventType` es un replay benigno; el mismo par con un `eventType` **distinto** devuelve `409` — un intento de colgar una acción sobre el evento de otra — salvo `fiscal_graphic`, que puede compartir el `eventId` con su resultado.
...
| Mismo `(orderId, eventId)` con el mismo `eventType` reenviado (cualquier `providerEventId`) | **`202`** · `duplicate: true` — se devuelve el registro existente, **no** se re-encola |
...
| Un fallo (`rejected`, `denied` o `error`) y luego otro desenlace con el **mismo `eventId`** (ej. `authorized` tras reintentar) | **`202`** — es el reintento de la misma emisión, se aplica |
| El mismo fallo reenviado con el **mismo `eventId`** (ej. un segundo `rejected` con otro motivo) | **`202`** · `duplicate: false` — se procesa y se guarda el motivo nuevo. Un fallo no dispara eventos, así que reprocesarlo es inocuo |
...
Cuando la orden ya tiene documento, el callback se asocia a él por `eventId`. Un callback con otro `eventId` válido termina en `notFound`, no en `regression`; reenviar el `eventId` del documento con un estado anterior es un reenvío del mismo par. Ninguno de los dos cambia el documento.
