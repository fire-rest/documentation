...
<Note>
  **Qué reglas corren acá, y cuáles no.**

  Corren las que se responden **con la orden**: su estado, el estado fiscal, el país, si el
  comprador se identificó, los minutos desde la autorización, y las que haya configurado la
  cuenta.

  **No corre la que exige la nota de crédito.** La política de cancelación tiene una regla
  base que niega cuando falta —y que sí aplica al `POST /cancel`—, pero acá sería pedirle a
  este endpoint que la nota exista antes de emitirla: es justamente lo que está por crear.

  Tampoco corre la del día de negocio, por la misma razón: es una condición para cancelar la
  orden, no para anular el comprobante.

  La pregunta que se hace acá es **«¿se puede anular esta venta?»**, no «¿se puede cancelar
  esta orden?». Esa la contesta
  [Elegibilidad de cancelación](/es/api-reference/cancellation-eligibility).
</Note>
