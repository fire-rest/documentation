...
<ResponseField name="canCancel" type="boolean">
  La respuesta. Lee la sección de `pending` más abajo antes de tratar un `true` como garantía.
</ResponseField>
...
<ResponseField name="pending" type="string[]">
  Verificaciones caras que **no** se corrieron: `CREDIT_NOTE` y/o `BUSINESS_DAY`. Vacío cuando ya se verificó todo.
</ResponseField>

### `pending` — por qué un `true` no es una garantía

El preflight corre solo la fase barata. Comprobar la nota de crédito cuesta una llamada al proveedor fiscal, y el día de negocio una consulta — sobre una lista de órdenes serían una por fila. Por eso el preflight las omite y las declara en `pending`. Un `canCancel: true` con `pending` no vacío **todavía puede terminar en rechazo** al cancelar de verdad: la cancelación real sí corre esas verificaciones.
