...
<ParamField body="eventId" type="string" required>
  UUID de correlación (paridad con el callback fiscal). **Fire valida que referencie un evento que Fire emitió para esta misma orden** — ecoa el `event.id` del envelope con el que Fire despachó la orden a tu KDS. Un `eventId` que no corresponda a un evento de Fire para esa orden se rechaza con `400` antes del `202`. Nunca inventes este valor.
</ParamField>
