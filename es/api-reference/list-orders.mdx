...
`revision` es la versión de la orden: arranca en `1` y sube uno cada vez que se [corrige](/es/api-reference/update-order). Es lo que mandás como `expectedRevision` al corregirla. El cobro no la mueve.
