...
Cuando una orden se cobra **después** de abrirse, la plata puede entrar por partes — el efectivo
ahora, la tarjeta cuando el cliente vuelve. `status` y `paymentStatus` no pueden expresar eso: dicen
`OPEN` y `PENDING` tanto para una orden que no pagó nada como para una que todavía debe 15,90 de
35,90.

| la orden | `status` | `paymentStatus` | `settlement.status` |
| --- | --- | --- | --- |
| abierta, sin cobrar nada | `OPEN` | `PENDING` | `pending` |
| **cobrados 20,00 de 35,90** | `OPEN` | `PENDING` | **`recorded`** |
| una pieza se rechazó | `OPEN` | `PENDING` | `declined` |
| cobrada por completo | `COMPLETED` | `SUCCEEDED` | `settled` |

Leé `settlement` para saber **cuánto** se cobró, y `payments` para saber **con qué**.
...
`status` es uno de `pending`, `recorded`, `declined`, `settled`. `paidSoFar` y `total` usan la misma
escala ×10000 que `totals` — arriba, 20,00 cobrados de 35,90. `tenderCount` cuenta solo las piezas
aprobadas; las rechazadas van en `declinedCount`.

`amountMismatch: true` quiere decir que **lo cobrado todavía no llegó al total**. En una orden de pago
diferido ese es el estado normal a mitad del cobro, no un error.
