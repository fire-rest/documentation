...
**Campos disponibles**: `id`, `orderCode`, `orderExternal`, `accountId`, `vendorId`, `storeId`, `stationId`,
...
El DTO de order refleja **todas las columnas de la tabla `orders`**. Dos de ellas traen el ciclo de
vida de la orden más allá de Fire:

| Campo | Qué trae |
|---|---|
| `kitchen` | Espejo del KDS — `{ status, occurredAt, stationName, history[] }`: el recorrido de la orden en cocina (`preparing` → `ready` → `dispatched`). |
| `aggregator` | Espejo del agregador — `{ channelCode, status, occurredAt, history[] }`: el recorrido de delivery reportado por Rappi / Uber / Didi. |
| `orderExternal` | Tu propio id de orden, canónico y único por vendor. |

`channel` es el único campo renombrado: es la columna `catalog_id`, que guarda el código de canal.
`deletedAt` siempre viene `null` — las órdenes borradas nunca se devuelven.
