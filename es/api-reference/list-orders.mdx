...
`metadata` siempre tiene la **misma forma amplia** — contiene la unión de los identificadores de todos
los países — pero **solo se poblan los campos correspondientes a `fiscal.countryCode`; el resto queda
en `null`**. Leé `fiscal.countryCode` para saber qué identificadores esperar.
...
identificadores específicos de cada país que se muestran abajo se agregan encima; los identificadores
de los **otros** países están presentes pero en `null`.
