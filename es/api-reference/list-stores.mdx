  ...
  <Expandable title="store">
    ...
    <ResponseField name="fiscal" type="object | null">
      ...
      <Expandable title="fiscal">
        ...
        <ResponseField name="secondaryGovIdType" type="string | null">Tipo de identificación fiscal secundaria (BR: `INSCRICAO_ESTADUAL`).</ResponseField>
        <ResponseField name="secondaryGovIdNumber" type="string | null">Número de identificación fiscal secundaria.</ResponseField>
        <ResponseField name="metadata" type="object | null">Extras específicos por país. Hoy solo BR está poblado — ver [El bloque fiscal](#el-bloque-fiscal).</ResponseField>
      </Expandable>
    </ResponseField>
...
`metadata` **varía según el país** — el país se discrimina por `location.countryCode` (no hay un
campo `country` dentro de `fiscal`). Hoy solo Brasil pobla `metadata`:
